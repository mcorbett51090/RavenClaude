export const meta = {
  ...
  phases: [
    ...
    { title: "Verify", detail: "Adversarial per-claim verification (vote count per sourceQuality)" },
...
const rcRead = await agent(
  "Use the Read tool to read the file at path `" + RC_PATH + "`. " +
...
if (!rcRead || !rcRead.found || !rcRead.content || rcRead.content.enabled !== true) {
  ...
  if (raw.task_class) {
    ...
    const classified = await agent(
      "## Run classifier\n\n" +
      "You are a one-shot classifier for a deep-research workflow. Classify the research question below into one of three task classes:\n\n" +
    ...
    const rationale = classified ? classified.rationale : "classifier returned null; defaulting to general"
    ...
    try {
      ...
      await agent(
        "Write the following JSON to `.ravenclaude/runs/run-classifier/run-" + tsSecs + ".json` using the Write tool. " +
        "Create parent directories if needed. Content:\n\n" +
        JSON.stringify({ task_class: taskClass, rationale, tiers: runCfg.tiers, reasoning: runCfg.reasoning }, null, 2),
        { label: "rc-audit-emit" }
      )
    } catch {}
  }
...
log("run-config: enabled=" + runCfg.enabled + " task_class=" + (runCfg.task_class || "n/a") + " rationale=" + (runCfg.rationale || "").slice(0, 80))
...
const MCP_FETCH_PREFIX = (runCfg.use_specialized_mcp && runCfg.primary_source_host === "learn.microsoft.com")
...
const VERIFY_PROMPT = (claim, v, voteMax) =>
  "## Adversarial Claim Verifier (voter " + (v + 1) + "/" + voteMax + ")\n\n" +
  "Be SKEPTICAL. Try to REFUTE this claim. ≥" + REFUTATIONS_REQUIRED + "/" + voteMax + " refutations kill it.\n\n" +
  "## Research question\n" + QUESTION + "\n\n" +
  "## Claim under review\n\"" + claim.claim + "\"\n\n" +
...
if (rankedClaims.length === 0) {
  return {
    ...
    run_config: { enabled: runCfg.enabled, task_class: runCfg.task_class, rationale: runCfg.rationale },
  }
}
...
const voted = (await parallel(
  rankedClaims.map((claim, claimIdx) => () => {
    ...
    return parallel(
      Array.from({ length: voteCount }, (_, v) => () =>
      ...
      const voteStr = (valid.length - valid.filter(v => v.refuted).length) + "-" + valid.filter(v => v.refuted).length
      log("\"" + claim.claim.slice(0, 50) + "…\": " + voteStr + (abstained > 0 ? " (" + abstained + " abstain)" : "") + (escalated ? " [escalated]" : "") + " " + (survives ? "✓" : "✗"))
      ...
      claimTierAudit.push({
        ...
        escalated_to: escalated ? ((runCfg.tiers && runCfg.tiers["verify_judgment"]) || "balanced") : null,
        final_verdict: survives ? "survive" : "killed",
      })

      return { ...claim, verdicts: valid, refutedVotes: valid.filter(v => v.refuted).length, survives }
    })
  })
)).filter(Boolean)
...
if (runCfg.enabled && claimTierAudit.length > 0) {
  try {
    ...
    await agent(
      "Write the following JSONL content (one JSON object per line) to `.ravenclaude/runs/" + runId + "/claim_tier_audit.jsonl` using the Write tool. Create parent directories if needed. Content:\n\n" +
