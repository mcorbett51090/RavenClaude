...
| `worktree-clean.sh <slug>\|--status\|--all [--force]` | Safely remove worktrees — refuses a dirty tree without `--force`; `--status` lists each tree clean/DIRTY/**IGNORED**/**UNKNOWN**; `--all` removes only the clean ones. UNKNOWN means `git status` itself failed for that tree, and is refused even with `--force`. IGNORED means only ignored files are present (`.env`, `node_modules/`) — `--porcelain` is silent on those, so they used to read as clean; refused without `--force`, honoured with it (Gate 216). |
