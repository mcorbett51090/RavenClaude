...
| `worktree-clean.sh <slug>\|--status\|--all [--force]` | Safely remove worktrees — refuses a dirty tree without `--force`; `--status` lists each tree clean/DIRTY/**UNKNOWN**; `--all` removes only the clean ones. UNKNOWN means `git status` itself failed for that tree, and is refused even with `--force` (Gate 216). |
