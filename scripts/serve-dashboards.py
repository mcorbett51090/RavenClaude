...
CODEX_EMITTER = REPO_ROOT / "scripts" / "emit-codex-config.py"
PROJECT_TARGET = REPO_ROOT
...
# forged Host enables DNS-rebinding. Any request whose Origin isn't ours, or whose
# Host isn't known, is rejected; on top of that a state-changing request must carry
# an Origin at all (see _state_change_origin_ok — a browser always sends one on a
# POST, so only a non-browser client can omit it).
...
class DashboardHandler(SimpleHTTPRequestHandler):
    ...
    def _handle_norns(self):
        ...
        Read-only; same Origin/Host CSRF guard as /__read. (Mirror of the bundled
        plugin server's /__norns with REPO_ROOT → PROJECT_ROOT — kept in lockstep
        per the dashboard-server-parity gate.)"""
        if not self._local_request_ok():
            self.send_error(403, "refused: cross-origin or non-local Origin/Host")
            return
        from urllib.parse import parse_qs, urlparse
        ...
        # Defend the plugin name: it indexes a path, so reject anything that
        # isn't a bare plugin directory name (no separators / traversal).
        if not plugin or "/" in plugin or "\\" in plugin or ".." in plugin:
            self.send_error(400, "invalid plugin name")
            return
        self._json(200, _read_norns(REPO_ROOT, plugin))
    ...
    def _handle_streams(self):
        ...
        Read-only; same Origin/Host CSRF guard as /__read. (Mirror of the bundled
        plugin server's /__streams — _read_streams takes project_root, so the two
        copies are byte-identical; Gate 32 checks endpoint NAMES, edit BOTH.)"""
        if not self._local_request_ok():
            self.send_error(403, "refused: cross-origin or non-local Origin/Host")
            return
        self._json(200, _read_streams(REPO_ROOT))
    ...
    def _handle_reserve(self):
        ...
        (Mirror of the plugin server's /__reserve — edit BOTH; Gate 32 checks names.)"""
        if not self._local_request_ok():
            self.send_error(403, "refused: cross-origin or non-local Origin/Host")
            return
        self._json(200, _read_reserve(REPO_ROOT))
    ...
    def _handle_reserve_override(self):
        ...
        try:
            ...
            return
        code, payload = _write_reserve_override(REPO_ROOT, body)
    ...
    def do_OPTIONS(self):
        if self.path in ("/__save", "/__run", "/__reserve-override"):
    ...
    def do_POST(self):
        ...
        # (/__save, /__run, /__classify), so the strict Origin requirement goes
        # here rather than in _local_request_ok() — which also gates read-only
        # GETs that legitimately carry no Origin. See _state_change_origin_ok.
        if not self._local_request_ok():
            self.send_error(403, "refused: cross-origin or non-local Origin/Host")
            return
