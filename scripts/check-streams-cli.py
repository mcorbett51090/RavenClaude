...
def check_readonly_summary(ops, capori) -> list[str]:
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        ops.append_event(d, sid, label="secret", terms=["secret"], word_count=3,
                          session_id="sess-z", ts="2026-06-23T00:01:00Z")
        ...
        (sdir / "history.jsonl").write_text(
            json.dumps({"schema_version": 1, "kind": "x", "stream_id": sid,
                        "summary": PHRASE}, sort_keys=True) + "\n", encoding="utf-8")
...
def check_session_close(ops) -> list[str]:
    ...
    with tempfile.TemporaryDirectory() as d:
        r = subprocess.run(
            ["bash", str(HOOK)],
            input='{"session_id":"s1"}', text=True, capture_output=True,
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        r = subprocess.run(
            ["bash", str(HOOK)],
            input='{"session_id":"' + PHRASE.replace(" ", "_") + '"}', text=True,
            capture_output=True,
            env={"CLAUDE_PROJECT_DIR": d, "CLAUDE_PLUGIN_ROOT": plugin_root, "PATH": _path()},
        )
        if r.returncode != 0:
...
def must_fail_traversal(ops) -> int:
    """Disable the guard (raw join) and assert the crafted id escapes the root."""
    with tempfile.TemporaryDirectory() as d:
        root = ops.streams_root(d)
        root.mkdir(parents=True, exist_ok=True)
        bad = "../escaped-stream"
        # The guarded path refuses; the RAW join (no is_safe_slug) escapes.
        raw_target = (root / bad).resolve()
        root_resolved = root.resolve()
        escaped = raw_target != root_resolved and root_resolved not in raw_target.parents
        if escaped:
            print("  must-fail-traversal: raw join ESCAPES the root (guard has teeth)")
            return 0
        print("  must-fail-traversal: ERROR — raw join did not escape (guard vacuous?)",
              file=sys.stderr)
        return 1
...
def main(argv: list[str]) -> int:
    ...
    if errs:
        ...
        return 1
    print("  slug-anti-traversal OK | read-only summary OK | session-close fail-safe + no-egress OK")
    return 0
