...
- [ ] **Regenerate the interactive portal:** `python3 scripts/generate-index-dashboard.py`. Commit the updated `index.html` (at the repo root) along with the rest of the change — CI's index.html freshness gate (Gate 97) will fail otherwise. _(The former `repo-guide.html` was retired and its role folded into `index.html`; there is no `scripts/generate-repo-guide.py` anymore.)_
