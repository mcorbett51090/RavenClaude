...
| [`data-tier-pick-the-azure-database.md`](./data-tier-pick-the-azure-database.md) | Primary diagnostic — when an app needs a managed DB on Azure and someone reaches for "the one we always use," check the workload shape first. | The data engine (SQL / Cosmos / PostgreSQL / Storage) is one of the hardest decisions to reverse; pick by access pattern, not team habit. |
...
| [`migration-assess-then-iac.md`](./migration-assess-then-iac.md) | Pattern — strong default for any brownfield/portal-built estate; deviate only with a written reason. | Hand-rehosting in the portal with "IaC it later" produces drift from day one and "later" never comes; assess first, then land the workload into IaC. |
