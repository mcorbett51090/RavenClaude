...
- **Canonical / knowledge** (high trust, follow without disclaimer): [`knowledge/frontend-engineering-decision-trees.md`](knowledge/frontend-engineering-decision-trees.md) (rendering-strategy per route, where-state-lives, server-vs-client component, data-fetching, optimistic-update, which-global-store, slow-render triage, form library) and [`knowledge/styling-and-bundle-decision-trees.md`](knowledge/styling-and-bundle-decision-trees.md) (styling-approach selection, bundle-size-regression triage) and [`knowledge/browser-extension-engineering.md`](knowledge/browser-extension-engineering.md) (Manifest V3 extension engineering — two Mermaid trees: where logic lives across content script / MV3 service worker / injected page script / popup·options, and the least-privilege permissions progression; plus messaging, storage, cross-browser, and the store review/publishing pipeline — MV3 specifics are `[verify-at-use]`-volatile). **Traverse the relevant Mermaid tree top-to-bottom before choosing** — the proactive complement to the Capability Grounding Protocol. Capability-map rows are `[verify-at-use]` — re-check against the vendor before quoting.
...
| 7 | **skills/hooks/commands/templates** | **Coverage sufficient + extended** — 6 skills, 4 commands, 4 templates, 1 advisory hook cover architecture, component craft, state/data, rendering strategy, performance, accessibility, and (new 2026-06-22) Manifest V3 browser-extension engineering. The new decision trees + script + extension skill extend reach without a 5th agent (team-growth-as-knowledge house rule). |
...
- **browser-extension value-add (2026-06-22)** — net-new coverage for **Manifest V3 browser extensions**: `knowledge/browser-extension-engineering.md` (2 Mermaid decision trees — where logic lives, and the least-privilege permissions progression — plus MV3 essentials, cross-context messaging, storage, Chrome/Edge/Firefox WebExtensions portability, and the store review/publishing pipeline), the `build-browser-extension` skill, and 3 best-practices (`request-the-narrowest-extension-permissions`, `keep-the-mv3-service-worker-stateless-and-ephemeral`, `isolate-content-scripts-from-the-page`). The extension's popup/options **UI seams to the existing react/state/perf agents** — this layer owns only the MV3-distinctive parts. MV3 specifics marked `[verify-at-use]` (volatile).
