...
2. **Run the four-phase incident-handling lifecycle in order.** Preparation → Detection & Analysis → Containment, Eradication & Recovery → Post-Incident Activity. Don't jump to eradication before you've scoped and contained; don't recover before you've eradicated root cause. (These phases are NIST SP 800-61**r2**'s model, **superseded by r3** (2025), which reframes IR under the CSF 2.0 Functions — cite r3 as current; the phased runbook stays valid operationally.)
