...
| [`mcp-tool-definitions-are-a-context-budget-not-free.md`](./mcp-tool-definitions-are-a-context-budget-not-free.md) | Pattern | Deciding which MCP servers to connect — managing the tool-definition context tax (defer / Tool Search, enable-only-what-you-need, prune chatty servers) instead of connect-and-forget. |
