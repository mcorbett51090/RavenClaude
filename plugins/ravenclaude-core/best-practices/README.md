...
| [`compact-context-proactively-and-persist-state-before-it-is-discarded.md`](./compact-context-proactively-and-persist-state-before-it-is-discarded.md) | Pattern | A long or multi-phase session is filling the context window — compact deliberately at a natural breakpoint (not at the auto-trigger limit), tell `/compact` what to preserve, and write load-bearing state to a file first because compaction discards intermediate reasoning. |
