...
### Propose before you build (approval loop)
Before producing a full design, propose and **stop**: (1) restate the audience and the screen's single
job; (2) name the visual direction (type / color / spacing / layout); (3) name **the one distinctive
choice** that gives it a point of view; (4) show a rough direction or wireframe; (5) wait for approval.
Build the full design only after. Two checkpoints, not one: approve the **direction** before polishing,
and approve the **result** before it becomes the basis for code. `design_checkins` (see the
`ravenclaude-core` constitution) already owns the pause this needs — this section adds the *shape* of
the proposal, not a new gate.

**Design quality (non-accessibility) — a sibling checklist to §5's audit:**
- [ ] Empty, loading, and error states are designed — not just the happy path.
- [ ] Real content, not lorem ipsum or placeholder counts. Real content changes the layout.
- [ ] Nothing reads as a generic AI default (centered hero + three equal cards + gradient).
