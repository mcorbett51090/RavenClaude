...
When the engagement is **memory-store-shaped** — an agent that must remember something across sessions, an auto-memory or `MEMORY.md` scheme, a memory-tool file path, a vector store standing in as memory, or a summarize-and-discard compaction design — consult [`../../memory-engineering/skills/choose-memory-paradigm/SKILL.md`](../../memory-engineering/skills/choose-memory-paradigm/SKILL.md) before answering. It runs the baseline-first gate (the no-memory **and** the cheap-lexical-retrieval baselines must lose on measured numbers before a memory system is justified) and then selects the paradigm — raw context / flat retrieval / LLM-extraction / agentic — across the four write-path axes (construction, storage, retrieval, mutability). Its sibling [`../../memory-engineering/skills/map-memory-surface/SKILL.md`](../../memory-engineering/skills/map-memory-surface/SKILL.md) answers the next question: which surface holds the bytes, and who executes the write.

**Spawn pointer:** if the engagement is memory-store-shaped beyond a single design call — retention and erasure policy, poisoning exposure, or write-path economics all in play — surface `memory-engineering/memory-architect-lead` to the Team Lead as the specialist to dispatch. You do not spawn it yourself (see Boundaries below); you name it, and the Team Lead routes.
