...
def _marketplace_root() -> Path:
    """Repo root: fixtures stay at marketplace-root tests/fixtures/classify-claim/.

    This file now lives at plugins/ravenclaude-core/scripts/; walking up finds
    the marketplace checkout. A consumer install has no fixtures — --self-test
    then fails closed (FileNotFoundError), which is correct.
    """
    here = Path(__file__).resolve().parent
    for parent in here.parents:
        if (parent / "tests" / "fixtures" / "classify-claim").is_dir():
            return parent
    return here.parents[2]


REPO_ROOT = _marketplace_root()
