...
def _fmt_rules(rules: list[str], cap: int = 6) -> str:
    if not rules:
        return "none"
    # Each rule string is read verbatim from .claude/settings.json /
    # settings.local.json — a committed, repo-controlled file — so it is untrusted
    # banner input exactly like the design-project / run-config fields. A rule
    # carrying a newline + a literal </ravenclaude-capabilities> would otherwise
    # close the untrusted-data frame early and render injected text as trusted
    # prose (verified break-out class, 2026-07 review). Route every rule through
    # the same _sanitize_banner_field() the sibling fields already use.
    sanitized = [s for r in rules[:cap] if (s := _sanitize_banner_field(r, cap=80))]
    if not sanitized:
        return "none"
    suffix = f", +{len(rules) - cap} more" if len(rules) > cap else ""
    return ", ".join(sanitized) + suffix
