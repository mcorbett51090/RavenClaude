...
def _sanitize_reasoning(raw: str, untrusted_inputs) -> str:
    ...
    # Precompute the reasoning's 10-grams as a set so the per-field scan is
    # O(len(field)) with O(1) lookups instead of O(len(field) * len(reasoning))
    # substring searches — question/context are read unbounded from stdin JSON,
    # so a very large attacker-influenced field must not drive a quadratic scan.
    # `x in reasoning_10grams` is exactly equivalent to `x in sanitized` for the
    # 10-char windows below: a 10-char substring is present iff it is one of the
    # reasoning's 10-grams — so the withhold decision is byte-identical.
    reasoning_10grams = {sanitized[i : i + 10] for i in range(len(sanitized) - 9)}
    for u in candidates:
        if len(u) < 10:
            continue
        ...
        if any(u[i : i + 10] in reasoning_10grams for i in range(len(u) - 9)):
