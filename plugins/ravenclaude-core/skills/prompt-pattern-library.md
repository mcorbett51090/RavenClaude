...
**Example block:** see [`structured-output.md`](./structured-output.md) for the canonical template. Inherit by reference, not by copy — agents that copy the block end up drifting from the canonical shape.
...
Full pattern: [`../skills/scenario-retrieval.md`](../skills/scenario-retrieval.md).
```
...
**What it is.** Any research-bearing answer cites the source (URL, file path, doc section) AND labels the source's tier (Tier 1 Consensus / 2 Strong-but-Contextual / 3 Divergent / 4 Emerging / 5 Deprecated) per [`researcher.md`](./researcher.md).
...
See [`environment-discovery.md`](./environment-discovery.md) for auto-discovery
...
only orchestrator. See [`spawn-team.md`](./spawn-team.md).
```
...
A new agent missing ≥2 required patterns scores ≤3 on Dimension 3 (Capability Grounding) of the [`agent-quality-rubric`](./agent-quality-rubric.md).
...
- [`agent-quality-rubric.md`](./agent-quality-rubric.md) — Dimension 3 grades agents against this catalog.
- [`structured-output.md`](./structured-output.md) — pattern 3's canonical reference.
- [`researcher.md`](./researcher.md) — pattern 6's tier schema.
- [`scenario-retrieval.md`](./scenario-retrieval.md) — pattern 4's full mechanics.
- [`environment-discovery.md`](./environment-discovery.md) — pattern 7's auto-discovery skill.
