...
| Categories | **5 shell** (`shell_readonly`, `shell_remote_mutate`, `shell_code_exec`, `shell_local_mutate`, `shell_package_install`; ALLOW/EDIT/DENY) + **6 tool-shape** (`file_edit_project`, `file_edit_global`, `file_read_project`, `file_read_global`, `network_read`, `mcp_tools`; **ALLOW/DENY-only**, v0.38.0–v0.39.0) | `network_write`; the deterministic MCP `mcp.allowed_servers` allowlist (§MCP identity) |
...
- **All file / network / MCP shapes are live (v0.39.0); two pieces remain.** File edits + reads (project & global), `WebFetch`/`WebSearch` → `network_read`, and `mcp_tools` are all reviewed when toggled on. A `..`/`~`/outside-project path resolves to the stricter `file_edit_global` / `file_read_global` (never silently to the project category). Not yet live: **`network_write`** (POST/PUT/DELETE web shapes) and the deterministic **`mcp.allowed_servers` allowlist** (design §MCP identity) — until the allowlist ships, the three MCP server-identity concerns (`mcp.unknown-server` / `mcp.unverified-server` / `mcp.tool-shadowing`) are seat-judged rather than deterministically denied.
