...
| Categories | **6 command** (5 shell + `network_write` — curl/wget/gh writes, v0.40.0; ALLOW/EDIT/DENY) + **6 tool-shape** (`file_edit_project`, `file_edit_global`, `file_read_project`, `file_read_global`, `network_read`, `mcp_tools`; **ALLOW/DENY-only**, v0.38.0–v0.39.0) | — (all 12 categories live as of v0.40.0; the MCP `mcp.allowed_servers` allowlist shipped v0.41.0) |
...
## Hardening EDIT (feature-flagged, default ON)
...
- Adding a transform is AppSec-reviewed (same bar as a catalog concern). Flag defaults **ON** after AppSec enable GO (2026-09-16); set `hardening_edit: false` to opt out (House Rule 3).
...
- **MCP server allowlist (§MCP identity, v0.41.0; dashboard UI v0.42.0).** Declare trusted MCP servers from the dashboard's **Command-review panel → Trusted MCP servers** field (writes `command_review.mcp.allowed_servers` into `comfort-posture.yaml`), or by hand in `.ravenclaude/thing.yaml`:
  ...
  ```yaml
  mcp:
    allowed_servers: [github, atlassian] # the <server> in mcp__<server>__<verb>
  ```

  (Precedence: `comfort-posture.yaml command_review.mcp` > `thing.yaml mcp` > none.) When an allowlist **is** configured, a **write** verb (anything not `get_`/`list_`/`read_`/`search_`/`describe_`/`fetch_`) from a server **not** on the list is denied **pre-LLM** (cite `mcp.unverified-server`) — no panel, no cost, can't be bypassed. Reads, and any verb from a listed server, fall through to the normal panel. The allowlist is **opt-in**: with none configured, the three MCP server-identity concerns (`mcp.unknown-server` / `mcp.unverified-server` / `mcp.tool-shadowing`) stay seat-judged and nothing is newly blocked. `mcp.tool-shadowing` is always seat-judged — detecting a name collision needs the live tool registry, which the single-call hook payload doesn't carry. Changing the allowlist invalidates any cached MCP verdict (it folds into `config_hash`).
