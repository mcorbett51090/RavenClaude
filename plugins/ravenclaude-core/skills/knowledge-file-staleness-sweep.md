...
This skill is the operational arm of the Researcher meta-skill ([`researcher.md`](./researcher.md)). Researcher decides *what* needs updating; this skill is *how* to find it systematically.
...
For each row with `Disposition: refresh` or `release-block`, draft a Focused Task brief for the named re-verifier per [`spawn-team.md`](./spawn-team.md) Step 4. The brief includes:
...
After running the sweep, produce a brief report + the queue file, then emit the Structured Output Protocol JSON block per [`structured-output.md`](./structured-output.md):
...
- [`researcher.md`](./researcher.md) — the meta-skill this sweep operationalizes. Tier 1-5 schema lives there.
...
- [`plugin-release-checklist.md`](./plugin-release-checklist.md) — wires the pre-release gate into the release process.
- [`spawn-team.md`](./spawn-team.md) — how the Team Lead routes the re-verification work after the queue lands.
