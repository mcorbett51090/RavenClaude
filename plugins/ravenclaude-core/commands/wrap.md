...
- **`.ravenclaude/runs/<session>/dispatch-ledger.jsonl`** — one counts-only line per sub-agent dispatch (tier, brief/report words, flags). Read it rolled up: `bash plugins/ravenclaude-core/bin/rc dispatch-summary`. A high frontier share on read-heavy work, a run of `frontier_readonly` flags, or briefs over cap is the "we paid Opus prices for grep" lesson — a scenario shape in its own right, and the cost line the engagement summary should carry. (Written only on Claude Code with a posture file present; absent elsewhere — say so rather than estimate.)
