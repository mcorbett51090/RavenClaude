...
- **`knowledge/claude-code-permissions.md` auto-mode facts were stale** (re-verified 2026-09-23 against code.claude.com/docs/en/permission-modes). Auto mode is not a research preview — on Pro/Max/Team it is now the built-in starting permission mode (v2.1.228+/v2.1.233+); it is available on the Anthropic API, Claude Platform on AWS, Amazon Bedrock, Google Cloud's Agent Platform, Microsoft Foundry, and gateway sessions, not Anthropic-API-only; the model gate differs by provider (Opus 4.6+/Sonnet 4.6+/Fable on the Anthropic API and Claude Platform on AWS; the narrower Sonnet 5/Opus 4.7+/Fable elsewhere); `Monitor` allow rules are also dropped on entry since v2.1.236; and `defaultMode: "auto"` in project-level settings still does not take effect (only user-level or managed settings). The always-prompt mode is now labeled "Manual" in the CLI/VS Code/JetBrains/desktop app (config value unchanged).
- **`knowledge/dynamic-workflows.md` carried a stale "research preview" label and stale runtime caps.** The current Claude Code workflows doc has no research-preview label. The concurrency cap (previously stated as a flat ≤16) is now configurable via `CLAUDE_CODE_WORKFLOW_MAX_CONCURRENT_AGENTS` (1–256, v2.1.269+); a single `parallel()`/`pipeline()` call is capped at 4,096 items; a size guideline (`workflowSizeGuideline`, default `medium`/`small` on Pro) advises Claude on scale; and a run now pauses rather than failing when it hits a usage limit (v2.1.271+). Also noted: plugins can ship a workflow natively in a `workflows/` directory — RavenClaude currently bundles `rc-deep-research`/`two-panel-plan-review` as skills instead; migrating is a tracked follow-up, not done here.
- **`knowledge/model-catalog.json`'s Legacy-model correction was investigated but NOT applied.** Opus 5.5 (GA 2026-09-22) and Fable 5.1 (GA 2026-09-01) are independently confirmed current, with Opus 5/Fable 5/Opus 4.8 now Legacy — but bumping `current.opus`/`current.fable` requires editing tribunal-seat substrate files (`hooks/thing-orchestrator.sh`, `scripts/thing-decision.py`, and several other `plugins/ravenclaude-core/scripts/*` files) that the command-review tribunal's own self-tamper floor (`xc.tribunal-self-disable`) denies to an automated session — confirmed by a live, denied edit attempt this session (Sága log `thing-2026-09-23T16-37-59Z-8002.json`). Per this repo's own documented precedent for this exact operation (see the "opus5-model-catalog-bump" proposal, 2026-08-07), this bump needs either a maintainer session with a working `dev_repo_exempt` ownership check or a proposal doc for manual application — left untouched (reverted after the denial) rather than shipped half-consistent.
...
A new always-on skill, `skill-index`, indexes every skill shipped by every marketplace plugin — so a
plugin that's currently disabled still has a findable, re-enableable entry instead of being silently
invisible. Generated by `scripts/generate-skill-index.py`, freshness-gated by CI as Gate 282. See
[`knowledge/concepts/skill-index-self-reference-convergence.md`](knowledge/concepts/skill-index-self-reference-convergence.md)
for a real mechanism quirk found while building it: the generator indexes its own output, so a first
write always undercounts by one entry — run it twice (or once, then `--check`, then once more) before
committing.

**Migration:** none — a new additive skill; nothing in a consumer's installed plugin changes on
`/plugin marketplace update` until they invoke `skill-index`.
