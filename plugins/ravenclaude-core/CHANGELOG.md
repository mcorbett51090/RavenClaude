...
- **`guard-destructive.sh` `git push -f` pattern over-matched a branch name ending in `-f`.** `git push origin feature-f` was blocked as a force-push because `.*-f` matched the trailing `-f`. The pattern now requires `-f` to be a standalone flag; `-f` / `--force` force-pushes are still blocked. (Fixed safely — the guard only ever fails closed.)
- **`thing-decision.py` self-disable guard missed non-canonical falsy `thing` values.** `_posture_write_disables` only caught `off`/`false`/`no`/`False`, but `thing_enabled_for` treats `thing: 0` / `"0"` / `disabled` / `none` as off too — so a tribunal-disabling Write of one of those slipped the guard. It now mirrors `thing_enabled_for`'s truthiness exactly. Also: the seat/panel timeout config now excludes `bool`, so `seat_timeout_seconds: true` is no longer coerced to a 1-second timeout (`isinstance(True, int)`).
- **`rc-deep-research.js` eval stats under-counted verify agents** (both byte-identical copies). The verify-phase agent count used a flat `voted.length * VOTES_PER_CLAIM` that ignored per-claim vote fan-out and escalation votes; it now accumulates a real `verifyAgentsFired` counter. Baseline (uniform policy, no escalation) is unchanged, so the eval baseline holds; Gate 52 untouched. Stats-only — no change to research behavior.
- **`two-panel-plan-review.js` could mislabel lens results** (both copies) when a panel agent returned null: `digestPanel` indexed `lenses[i]` after `filter(Boolean)` compacted the array. Each result is now paired with its lens key before filtering.
