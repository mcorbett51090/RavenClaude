#!/usr/bin/env bash
# Gate 223 — the two assumption layers.
#
...
echo ""
if [ "$fails" -eq 0 ]; then
  echo "Gate 223 PASS — inference-as-observation fires on uncited causal claims and stays silent on cited/described/escaped/prescriptive/out-of-scope; ask-on-ambiguity fires on the under-specified shape only, never blocks, and never egresses the prompt. Both teeth halves confirmed."
  exit 0
else
  echo "Gate 223 FAIL — $fails subtest(s) failed."
  exit 1
fi
