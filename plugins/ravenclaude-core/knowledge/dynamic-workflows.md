...
3. **Bundled in a skill** — put the `.js` files in a skill folder and reference them from `SKILL.md`; prompt Claude to treat them as a **template**, not a verbatim script. _RavenClaude now ships both workflows this way:_ [`skills/rc-deep-research/`](../skills/rc-deep-research/SKILL.md) and [`skills/two-panel-plan-review/`](../skills/two-panel-plan-review/SKILL.md) each bundle their `.js` alongside a `SKILL.md` that frames it as a template — **so consumers get a runnable `/rc-deep-research` and `/two-panel-plan-review` on `/plugin install`** (the consumer path). The project-level `.claude/workflows/*.js` copies still exist for this dev repo (option 1); the bundled skills are how the workflows reach consumers.

## What RavenClaude already ships

| Workflow | Patterns | Notes |
|---|---|---|
| [`rc-deep-research`](../skills/rc-deep-research/SKILL.md) | fan-out-and-synthesize + adversarial verification | **Bundled as a plugin skill** ([`skills/rc-deep-research/`](../skills/rc-deep-research/SKILL.md), `.js` + `SKILL.md`) — consumers get a runnable `/rc-deep-research` on install; the project-level [`.claude/workflows/rc-deep-research.js`](../../../.claude/workflows/rc-deep-research.js) copy remains for this dev repo. Renamed from `deep-research` (2026-06-04) because Claude Code now ships a **bundled `/deep-research`** skill-workflow. The docs confirm project>personal precedence on a name clash but are **silent on bundled-vs-project** — so "ours shadows the bundled one" is `[unverified]`; the rename removes the dependency on that unknown. Carries the substrate adapter (`adaptive-run-classifier` / `.ravenclaude/run-config.json`) for per-phase model-tier routing (disabled + safe when that file is absent). |
| [`two-panel-plan-review`](../skills/two-panel-plan-review/SKILL.md) | fan-out-and-synthesize + adversarial verification | **Bundled as a plugin skill** ([`skills/two-panel-plan-review/`](../skills/two-panel-plan-review/SKILL.md), `.js` + `SKILL.md`) — consumers get a runnable `/two-panel-plan-review` on install; the project-level [`.claude/workflows/two-panel-plan-review.js`](../../../.claude/workflows/two-panel-plan-review.js) copy remains for this dev repo. Two independent expert panels review a strategic plan then a build plan. Its lens/severity/routing rubric is **private to this workflow** — it is NOT shared with FORGE (different input contracts: this reviews a pre-written plan; FORGE turns a raw idea into a plan). `[corrected 2026-07-15]` |
| [`/forge`](../commands/forge.md) (static) | classify-and-act + adversarial verification + tournament-style tiebreak | The gated **planning** pipeline — a *static* harness, not a dynamic workflow. It does **not** reach for the two-panel rubric: that rubric is private to the two-panel workflow and deliberately not shared. `[corrected 2026-07-15]` |
