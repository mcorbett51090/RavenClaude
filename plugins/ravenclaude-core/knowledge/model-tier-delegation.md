...
## Multi-hop delegation — may a called agent call agents?

**Possible: yes. Enabled here: no, by declaration. Desirable: not as a default.** The full determination, with every platform fact dated, is [`docs/decisions/2026-09-14-nested-dispatch-determination.md`](../../../docs/decisions/2026-09-14-nested-dispatch-determination.md); this is the operating summary.

The platform nests **by default, three layers below the main conversation** (`CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH`; `1` turns it off) `[docs-verified 2026-09-14; observed live the same day on Claude Code 2.1.271 — main → coord1 → coord2 → leaf completed, a 4th layer found `Agent` silently absent from the 3rd-layer subagent's toolset, five runs, decision doc § 7]`. "Sub-agents do not spawn other sub-agents" is therefore a **house rule**, and the layer that actually enforces it is not prose — it is each agent's `tools:` allow-list: omit `Agent` and the subagent cannot dispatch. Two platform facts decide where the rule has to live:

- **`Agent(type)` is ignored in a subagent definition.** `tools: Agent(scout)` does not mean "may dispatch scout"; it means "may dispatch anything, at any tier". A declaration cannot *scope* nested dispatch, only grant or withhold it.
- **The built-in `general-purpose` / `claude` types hold every subagent tool, `Agent` included.** So a Team Lead that dispatches `general-purpose` has dispatched something that can nest, whatever this roster declares.

Why the default is *no*, in this file's own terms: every nested hop is a brief written and a report re-read **at the caller's tier, where the Team Lead cannot see it** — only the top summary returns. The intermediate report is exactly what `report_over_cap` exists to catch, consumed by a subagent that receives no advisory and wrote no `Max output` line. The tier of the whole subtree is whatever the caller chose, and the caller is a mid-or-fast model making the tier decisions the first table reserves for the frontier orchestrator. Recovery runs sideways (same tier, longer brief — the pay-twice shape) instead of up the ladder. And `Agent` in `tools:` is an unscoped grant, so the blast radius of an agent that has it is the blast radius of `general-purpose`.

The one shape the determination sanctions — **a frontier-tier parent fanning out to fast-tier, read-only leaves that cannot themselves dispatch** (the platform's own "reviewer → a verifier per finding" example) — is allowed **per agent, by name, with a reason** in [`tests/fixtures/nested-dispatch-exemptions.json`](../../../tests/fixtures/nested-dispatch-exemptions.json). A mid/fast parent, or a third layer, is not exemptable. No exemption is on file.

| Layer | What it does | Binds? |
| --- | --- | --- |
| **Gate 289** [`scripts/check-nested-dispatch.py`](../../../scripts/check-nested-dispatch.py) | fails any `agents/*.md` whose `tools:` grants `Agent` / `Agent(...)` / `Task` / `"*"` without a reasoned exemption; a stale or reasonless exemption fails; an empty roster is not a pass | **yes** — on the *declaration*, in CI |
| **`handoff-tax-meter`** `nested_dispatch` | hooks fire inside subagents and the input then carries the caller's `agent_id` / `agent_type`; the ledger records the caller and a `depth` (1 = main thread; live, hooks fire child-first so the nested line carries the floor `2` marked `depth_is_lower_bound`, and `dispatch-summary` re-resolves the exact layer by walking the caller chain over the complete ledger); the `nested_dispatch` flag is **recorded, not spoken** — a hook that fires inside a subagent hands its `additionalContext` to *that subagent*, never to the Team Lead, and live the spoken version derailed the caller's report — so the `nesting` row of `dispatch-summary` is where the orchestrator reads it. A nested child may also come back `async_launched` (2 of 5 live runs, `run_in_background` unset): its report never reaches the hook and its caller may return before it finishes | no — it is how the three vectors the gate cannot reach (built-ins, a fork, a consumer's project-local agent) become visible |
| **`CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH=1`** | the consumer's off-switch, in `settings.json` `env` | yes, on the consumer's host — and it also disables the sanctioned shape |
| [`guard-recursive-spawn.sh`](../hooks/guard-recursive-spawn.sh) | the `PostToolUse` grep over agent *prose* that warns when a definition reads as if it spawns | no — it was the only guard before 2026-09-14, and a grep over prose cannot see a `tools:` line |

**The follow-up that would change the verdict:** a `PreToolUse(Agent)` hook *can* scope what the declaration cannot — the input carries the caller's `agent_id` / `agent_type` and the child's `subagent_type` / `model`, so a hook could deny any nested dispatch whose caller is not frontier or whose child is not a read-only fast type. That would turn the sanctioned shape from a reviewed exemption into an enforced envelope. Not built: no agent needs it yet, and a *blocking* hook on the dispatch path is a design check-in under the tribunal's high-blast rule, not a pass-3 addition.
...
|---|---|---|
| [`handoff-tax-meter.sh`](../hooks/handoff-tax-meter.sh) (`PostToolUse` on `Agent`) | per dispatch: `subagent_type`, requested vs `resolvedModel`, tier, brief words, report words, `totalTokens`, `totalToolUseCount`, duration, over-cap flags; and — because hooks fire inside subagents too — the **caller** (`caller_agent_id` / `caller_agent_type`, `nested`, reconstructed `depth`) when a called agent called an agent | `.ravenclaude/runs/<session>/dispatch-ledger.jsonl` (+ an advisory to the Team Lead on an over-cap report or brief, an un-pinned frontier dispatch of a read-only type, or a nested dispatch) |
| `bash plugins/ravenclaude-core/bin/rc dispatch-summary` (= `handoff-tax-meter.py --summary`) | the ledger rolled up: dispatches by tier, frontier share, `frontier_readonly` count, over-cap briefs/reports, median brief/report words, the token lower bound, and a `nesting` row (nested count, which callers, deepest layer) | stdout, for `/wrap` and the retrospective — the "cost per completed task" line comes from here, not from a feeling |
...
| `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH` | settings `env` | `3` (platform default since v2.1.219) | How many subagent layers may sit below the main thread. `1` turns nesting off — the consumer's switch for the vectors Gate 289 cannot reach (built-in `general-purpose`, a fork, a project-local agent). See § "Multi-hop delegation". |
