...
The platform nests **by default, three layers below the main conversation** (`CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH`; `1` turns it off) `[docs-verified 2026-09-14]`. "Sub-agents do not spawn other sub-agents" is therefore a **house rule**, and the layer that actually enforces it is not prose — it is each agent's `tools:` allow-list: omit `Agent` and the subagent cannot dispatch. Two platform facts decide where the rule has to live:
...
| **`handoff-tax-meter`** `nested_dispatch` | hooks fire inside subagents and the input then carries the caller's `agent_id` / `agent_type`; the ledger records the caller and a reconstructed `depth` (1 = main thread; the caller's own depth + 1 when the caller was spawned in this ledger; else `2` marked `depth_is_lower_bound`), the advisory names the caller, the layer and the off-switch, and `dispatch-summary` prints a `nesting` row | no — it is how the three vectors the gate cannot reach (built-ins, a fork, a consumer's project-local agent) become visible |
