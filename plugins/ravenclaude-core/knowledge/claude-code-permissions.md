...
A subagent's `tools:` frontmatter bounds **which tools** it may call, but it does **not** bound the **permission mode** those tool calls run under. Anthropic's docs state it plainly: **"When using `bypassPermissions`, all subagents inherit this mode and it cannot be overridden."** There is currently **no shipped mechanism** to restrict a subagent below the parent session's mode — the per-subagent knobs floated for it (`subagentPermissionMode`, `allowSubagentBypass`, a `subagentBypassAllowlist`) are all **proposed, not implemented** (#20264, open as of 2026-07-01 — verify before citing any of them as available).
