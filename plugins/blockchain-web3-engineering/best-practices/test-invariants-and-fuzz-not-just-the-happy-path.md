# Test invariants and fuzz — not just the happy path.
...
## The rule
Unit tests on expected inputs miss the adversarial state an attacker searches for; specify invariants (total supply conserved, no value created, access holds) and fuzz/property-test them, because the exploit lives in the input you didn't write a test for.
...
The §4 failure mode: acting as if "test invariants and fuzz — not just the happy path." weren't true — the most common way an analysis quietly misleads the practitioner who acts on it. The plugin's advisory hook flags a deliverable that reads as if this rule were ignored.
