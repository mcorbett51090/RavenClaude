...
## Recent GA capabilities (weekly sweep, verified 2026-06-19; re-verified 2026-07-08 against [Graph what's-new](https://learn.microsoft.com/graph/whats-new-overview))
...
- **New least-privileged permissions for user create/update — GA (July 2026)** `[verify-at-use]`. [`User.Create`](https://learn.microsoft.com/graph/api/user-post-users) is now the **least-privileged** permission to create a user, and [`User.ReadUpdate.All`](https://learn.microsoft.com/graph/api/user-update) the least-privileged permission to update one — both narrower than the `User.ReadWrite.All` / `Directory.ReadWrite.All` previously required, so prefer them per house-opinion §1 (least-privilege). Confirmed on the v1.0 `Update user` permissions table (`User.ReadUpdate.All` listed as least-privileged for both delegated + application). Source: Graph what's-new "July 2026: New and generally available — Users" (re-verified 2026-07-08).
