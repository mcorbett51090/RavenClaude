...
description: "Use for shaping, paging, batching, and hardening Microsoft Graph API calls — OData query shaping ($select/$filter/$expand), advanced query (ConsistencyLevel: eventual), paging via @odata.nextLink, $batch, delta queries, and throttling/429 + Retry-After resilience."
