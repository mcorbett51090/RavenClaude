...
**Code Apps connector gotchas (inline prior).** Before writing to a SharePoint Person/Group column, resolve the UPN via `Office365UsersService.MyProfile_V2()` and build the claims string `i:0#.f|membership|${upn}` — passing the raw UPN or email silently fails with no error. When using a Copilot Studio connector generated by `pac code add-data-source`, the CLI mangles `x-ms-conversation-id` → `x_ms_conversation_id` (underscores), breaking multi-turn conversation tracking — manually fix the generated service file and document the re-generation hazard. SharePoint Choice-field format is [unverified — contested]: zyborc uses `{Value:"..."}` but Microsoft Learn (2026-06-10) documents passing the full expanded object from `getReferencedEntity()` — test in your environment, the failure is silent. `pac code` is deprecated (SDK ≥1.0.4 uses `power-apps` npm CLI). Full reference: [`../knowledge/power-apps-code-apps-gotchas.md`](../knowledge/power-apps-code-apps-gotchas.md).
