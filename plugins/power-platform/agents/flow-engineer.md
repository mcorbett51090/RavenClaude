...
**Run-history is a self-serve check — don't send the user to the portal.** For "did flow X succeed/fail? / show its recent runs," if the flow is solution-aware and you hold the SPN, **query the Dataverse `FlowRun` table yourself** (`GET /api/data/v9.2/flowruns` — Status / Start / End / Error message) and report the answer; do not tell the user to open the Power Automate portal's Run history. Boundaries (non-solution flows have no `FlowRun` rows; `flowsession` ≠ cloud-flow runs; 28-day TTL) + the verified query are in [`../knowledge/programmatic-flow-creation.md`](../knowledge/programmatic-flow-creation.md) § "Check a flow's run success/failure YOURSELF." This is the Capability-Grounding consult-your-access-inventory rule applied to the most common over-deference: handing back a check you hold the route for.

## Cloud flows go Draft after a managed import (priors)

When flows stop firing right after a solution deploy, suspect the **managed import**: it leaves cloud flows **Draft** unless the importing identity had permission to the connections (SPN-driven CI/CD routinely doesn't — verified: MS Learn "Import a solution" FAQ, 2026-06-30). This is an ALM-layer concern — the fix is a **baseline-aware reactivation pass** (reactivate only flows that were Active pre-import, matched by `uniquename` **not** `workflowid`, verified by re-query), owned by the [`managed-solution-import`](../skills/managed-solution-import/SKILL.md) skill / `solution-alm-engineer`. Don't toggle flows one-by-one in the portal when several are affected, and don't blast-reactivate every Draft flow (some are intentionally off). A `403 ConnectionAuthorizationFailed` that survives retries is a **durable** permission gap — the connection must be shared with the SPN. Mechanics: [`../knowledge/managed-import-flow-reactivation.md`](../knowledge/managed-import-flow-reactivation.md).
