...
## Data-store selection (priors)

Before modeling a single table, when the user asks "should this be Dataverse / SharePoint / a list / Excel?" (or hands you a flat-file workbook to "turn into an app"), **traverse the `## Decision Tree: Data store — Dataverse vs Dataverse for Teams vs Lists vs Excel` section in [`../knowledge/datastore-and-integration-decision-trees.md`](../knowledge/datastore-and-integration-decision-trees.md) top-to-bottom before choosing.** Decide by enterprise-security/ALM/non-Teams-surface need → relational need → scale/licensing. Excel is a migration *source*, never a backend. Only once the tree lands on a Dataverse leaf do the within-Dataverse trees (column type, relationship, security mechanism, logic placement) apply. Adjacent rule: [`../best-practices/data-store-dataverse-vs-sharepoint-vs-teams.md`](../best-practices/data-store-dataverse-vs-sharepoint-vs-teams.md).
...
## Error attribution (priors)

A 401/403/404 reading a Dataverse table is an **access / auth / routing** problem — it is **not** evidence that a field or table is absent. Schema existence is a metadata fact, independent of whether the principal can read the data, so never report a "missing field" off an access error: verify **schema** (entity metadata / `RetrieveEntity`) and **access** (security role, row sharing, column-level security, env/DLP) as two separate checks. Only a `400 Could not find a property named '<x>'` (on an otherwise-authorized request) speaks to a column name. Decision table + the verify-both-axes discipline: [`../knowledge/dataverse-http-error-attribution.md`](../knowledge/dataverse-http-error-attribution.md). This is the Dataverse worked example of the core [Claim Grounding & Source Honesty protocol](../../ravenclaude-core/CLAUDE.md) (don't state a behavioral conclusion from an unverified inference).

## REST-first debugging for Dataverse-backed scoring (priors — BMA-CSP Lesson 15, 2026-06-04)

**Diagnosing a Dataverse-backed scoring report where the report layer says "0" / "BLANK" for every entity** → don't open the model in Power BI Desktop first; don't iterate by deploying. The Dataverse Web API (and the Fabric REST `executeQueries` against the consuming semantic model) returns the verbatim error envelope the portal hides. The five-minute diagnosis that closes silent-zero scoring bugs is `GET https://yourorg.crm.dynamics.com/api/data/v9.2/cr<n>_questions?$select=cr<n>_questionnumber,cr<n>_questiontext&$top=50` to confirm the actual string values in the column the scorer DAX filters on, then comparing against the literals in the scorer measures. The string-literal mismatch is the canonical silent-zero failure mode — same shape as the category-name lesson ([`../knowledge/dax-category-name-mismatch-zero-scores.md`](../knowledge/dax-category-name-mismatch-zero-scores.md)), applied to question numbers and any other dimension-string assumption.

**Auth chain priors:** the bearer-token decision tree for Dataverse Web API is unchanged ([`../knowledge/dataverse-token-acquisition.md`](../knowledge/dataverse-token-acquisition.md) — client-credentials → `az account get-access-token` → reuse PAC's MSAL cache → interactive). The Fabric REST analogue uses `--resource https://analysis.windows.net/powerbi/api` instead of the org URL. Full REST debugging playbook + endpoint cheat-sheet (Fabric `executeQueries`, refresh history, workspace items + the Dataverse-Web-API analogue): [`../knowledge/pbir-fabric-rest-debugging.md`](../knowledge/pbir-fabric-rest-debugging.md). When the scoring DAX has been pinned to design-doc assumptions that don't match the live Dataverse data, REST is the FIRST diagnostic move, not the last — production-observed across multiple dead-end loops 2026-06-04 on the BMA-CSP-Risk-Scoring engagement.

## Solution layering — the "Active-dependency" managed-export trap (priors)

When a **managed** solution import fails on a `MissingDependency` to `solution="Active"` (`canResolveMissingDependency="False"`), do **not** delete and recreate the table/column "to move it out of the Active layer." Every unmanaged component is *always* in the default/Active solution (verified: [`CreateEntityRequest.SolutionUniqueName`](https://learn.microsoft.com/dotnet/api/microsoft.xrm.sdk.messages.createentityrequest.solutionuniquename) — "all tables are automatically added to the default solution"), so relocation is a non-goal that can't be achieved and the delete is wasted effort. The real lever is the `RootComponentBehavior` on the **existing** component: `AddSolutionComponent` with `DoNotIncludeSubcomponents=false` (⇒ behavior **0 = Include Subcomponents**) makes the managed export self-contained — **in place, no delete**. Create-with-solution-context (`MSCRM.SolutionUniqueName` header / `?solutionUniqueName=`) adds named-solution membership but `[unverified — observed, not documented]` lands at `behavior=1` (shell), so the `behavior=0` follow-up is the load-bearing step. Full lesson + Microsoft-Learn citations: [`../knowledge/dataverse-solution-layering-active-dependency.md`](../knowledge/dataverse-solution-layering-active-dependency.md). This is the Dataverse worked example of the core [verify-the-load-bearing-assumption-before-a-high-impact-activity](../../ravenclaude-core/CLAUDE.md) discipline — the deletes were an irreversible activity launched on an unverified premise one doc-read would have falsified.
