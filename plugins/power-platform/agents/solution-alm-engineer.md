...
## Reactivate cloud flows after a managed import (priors)

A managed `pac solution import` leaves cloud flows **Draft** whenever the importing identity (your SPN) lacks permission to the connection — the platform auto-reactivates only when the flow was exported-On AND its connection references are bound AND the importer can use those connections (verified: MS Learn "Import a solution" FAQ, 2026-06-30). SPN-driven CI/CD routinely fails that third condition, so flows land Draft and automations silently die while the import reports **success**. **Run an explicit, baseline-aware reactivation pass after every managed import:** capture the Active flows *before* the import keyed by **`uniquename`, not `workflowid`** (a managed import can recreate the row with a new GUID — GUID-matching silently skips it), reactivate only those left Draft, PATCH `{statecode:1,statuscode:2}` via the Dataverse Web API, and **verify by re-query** (a `204` is not proof of activation). A `403 ConnectionAuthorizationFailed` is two problems: transient (binding still propagating → retry with backoff) vs durable (SPN lacks connection permission → **share the connection with the SPN**; retry won't fix it). Keep pac flags lean — `--activate-plugins` + `--settings-file`; `--publish-changes`/`--force-overwrite` are **discouraged for managed imports** (opt-in only). Full playbook + the shipped script: [`../skills/managed-solution-import/SKILL.md`](../skills/managed-solution-import/SKILL.md); mechanics: [`../knowledge/managed-import-flow-reactivation.md`](../knowledge/managed-import-flow-reactivation.md); rule: [`../best-practices/alm-reactivate-flows-after-managed-import.md`](../best-practices/alm-reactivate-flows-after-managed-import.md).
