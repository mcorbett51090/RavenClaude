...
- **For ANY PBIP/PBIR report symptom** (spinner, blank visual, zero measures, missing rows, wrong color, first-deploy fail, filter ignored, deployment variable no-op) → read [`knowledge/pbip-report-fast-triage.md`](../knowledge/pbip-report-fast-triage.md) FIRST, before editing any report file. Follow its source chain: Tier 1 our captured reference (always works, zero network/MCP) → Tier 2 official MS via cite-and-WebFetch (the `learn.microsoft.com/...` article + `github.com/microsoft/json-schemas` schema repo) → Tier 3 the Learn MCP **only if your org permits MCPs** → Tier 4 data-goblin (GPL, re-express only). Verify any property against a Desktop-saved `.pbip` file. Never guess-and-check on report metadata. The router has two tables: (a) symptom→cause→fix-location (infinite-spinner / zero-vs-blank / missing-rows / wrong-color / first-deploy-fail / filter-wrong-scope / deployment-var-ignored) and (b) a formatting/theming decision rule (chrome→`visualContainerObjects`; data content→`objects`; silent no-op→wrong suffix or unknown property name→check `pbir-enhanced-reference §9`).
...
- **Designing or deploying PBIP artifacts in Fabric across environments** (parameterizing lakehouse GUIDs, PQ parameters, connection strings; deployment-pipeline rules; Variable Libraries; REST; first-deploy failures) → read [`knowledge/pbip-fabric-deployment-variables.md`](../knowledge/pbip-fabric-deployment-variables.md) before authoring any PBIP CI/CD pipeline. The load-bearing two-layer model: **Layer A** = source-controlled variables parameterized via `parameter.yml` before/during deploy (`find_replace`/`key_value_replace`/`spark_pool`/`semantic_model_binding`); **Layer B** = post-deploy runtime variables (deployment-pipeline parameter rules, Variable Libraries, REST) applied after the artifact is live. The canonical first-deploy failure: expecting pipeline parameter rules on the **first deploy** (they don't exist until the artifact lands in the target stage). The **#839 gotcha**: `find_replace`/`key_value_replace` silently skip `.platform` files — preprocess `.platform` before `publish_all_items()` for display-name parameterization (PR #840 in flight `[verify-at-use]`). **PBIR constraint**: PBIR reports cannot deploy through Fabric deployment pipelines — route via code-first deploy instead `[verify-at-use]`. Toolchain: **`iemejia/fabio`** (Rust agent-native CLI, v0.20.0 2026-06-10, structured JSON output + piping — the right primitive for agent-driven PBIP deploy; license `[verify-before-use]`); **`NatVanG/fab-inspector`** v3.3.0 for pre-deploy validation; **`kerski/fabric-dataops-patterns`** for `git diff`-scoped deploy + synchronous refresh gate; `powerofbi.org` lakehouse-JSON pattern for Data Pipeline parameters. Scout run 2026-06-10.

## Declarative visualization (Deneb / Vega-Lite / SVG)

When a standard Power BI visual can't serve a chart type — diverging bars, dumbbell plots, heatmaps, small-multiples with custom marks, annotated lines — use **Deneb** (the certified Vega/Vega-Lite custom visual) for Power BI, or **SVG-in-DAX** via the Image visual for lightweight static shapes. The cross-surface spec-authoring method, security rules, and starter templates live at [`../../ravenclaude-core/skills/declarative-visualization/SKILL.md`](../../ravenclaude-core/skills/declarative-visualization/SKILL.md); the full grammar + surface→delivery map + security model: [`../../ravenclaude-core/knowledge/declarative-visualization.md`](../../ravenclaude-core/knowledge/declarative-visualization.md). **Security is load-bearing:** run `lint.py` on any spec before committing — `data.url`, remote `transform.lookup`, custom `loader`, and SVG `<script>`/`on*` are forbidden (Gate 101). SVG-in-DAX note: the Image visual rasterizes SVG, blocking script execution — but omit `<script>` anyway for defense-in-depth; DAX string limit ~32,767 chars; prefix with `"data:image/svg+xml;utf8," &`. [Verify SVG char limit + rasterization behavior against current Power BI docs.]

## Visual craft

Report layout is visual craft, not just DAX correctness — information hierarchy (KPI cards above the fold, trend visuals mid-page, drill-down matrices/tables lowest), one dominant KPI per page with supporting cards stepping back, whitespace over borders, and color reserved for status/conditional-formatting semantics rather than decoration. This is the cross-plugin canon (not Power-BI-specific — applies to any dashboard stack): [`../../data-platform/knowledge/dashboard-visual-craft-2026.md`](../../data-platform/knowledge/dashboard-visual-craft-2026.md). Distinct from this file's own report-mechanics knowledge above — that answers *does it render correctly*; this answers *does it read as premium*.
...
**When a visual renders BLANK (layout perfect, content invisible) — diff it against a known-good exemplar, don't guess-and-deploy.** The layout linter answers "is the geometry valid?", never "will it render?" — a tile can sit at perfect coordinates yet come back blank because its render skeleton is wrong for its type (wrong query role, an unknown object key like `calloutValue` on a legacy `card`, a missing `$id` on a `cardVisual`). Run the referee's **`parity` gate** — `{"surface":"pbir","parity":{"candidate":"<failing>/visual.json","reference":"<confirmed-working>/visual.json"}}` — to get the structural delta in one determinate check instead of a deploy cycle. **Consult [`knowledge/pbir-enhanced-reference.md`](../knowledge/pbir-enhanced-reference.md) §1 + the `calloutValue`/`cardVisual` callouts FIRST** (the card-family role matrix and the "renders blank when authored programmatically" facts are already captured there) — the parity gate is the backstop for when a render still surprises you. The proven count/score-tile pattern is `card` + `Values` role + `labels`/`categoryLabels` (Measure colors OK).
