...
- **The "you must do the portal work" that's actually false.** "I can't build a main form for this table — you'll have to do that in the maker portal" is the model-driven twin of the flow trap above, and equally **false**. A main form is a `systemform` record (`type=2`) with a writable `formxml`/`formjson` body: create it via the Dataverse Web API (`POST systemforms` + `MSCRM.SolutionUniqueName`, or the `CopySystemForm` action then `PATCH formxml`) and `PublishXml`, or via the solution FormXml round-trip — verified against Microsoft Learn's [Customize forms in model-driven apps](https://learn.microsoft.com/power-apps/developer/model-driven-apps/customize-entity-forms) (2026-06-11). The over-generalization usually leaks from the *custom-page* `.pa.yaml`-is-read-only fact, which applies to **canvas custom pages only** — it does **not** extend to forms, views, or sitemaps. The agent already ships the full how-to in [`skills/dataverse-web-api/resources/forms-ui.md`](skills/dataverse-web-api/resources/forms-ui.md); §5's checklist (check the plugin's own skills first) catches this before the refusal leaves the agent. The privilege gate (System Customizer) is a *grant-the-role* ask, not a portal fallback.
...
The hooks are **advisory by default** (print to stderr, don't block the edit). To enforce, set `POWER_PLATFORM_STRICT=1` in the env — a hook then exits **2** (the only code Claude Code's PreToolUse contract treats as blocking; `exit 1` is a non-blocking error it silently swallows). To wire them into a consumer project's `.claude/settings.json`, see [`hooks/README.md`](hooks/README.md).
...
| [`knowledge/dataverse-web-resource-html-gotchas.md`](knowledge/dataverse-web-resource-html-gotchas.md) | Building or reviewing an **HTML/JS web resource page** embedded in a model-driven app — or debugging one that fails *silently* (spinner that never clears, every KPI `0`, a `404` the portal swallows). Four production gotchas (Copilot session on Contoso Insight, reconciled against learn.microsoft.com 2026-06-11): **(1)** slash-named web resources are served with `%2F`-encoded slashes (one flat path segment), so relative `../` references break — the robust fix is `Xrm.Utility.getGlobalContext().getWebResourceUrl(name)` (MS *recommends* relative + *warns against* root-anchored `/WebResources/...`, so don't blindly "always use absolute"); **(2)** `Xrm.WebApi.retrieveMultipleRecords` wants the **logical** name (`prefix_onsite`), NOT the entity-set name (`prefix_onsites`) — inverse on Power Pages `$pages.webAPI`; **(3)** verify every column against `EntityDefinitions/Attributes` (paging `@odata.nextLink`) before `$select` — never trust a spec doc (the Power Platform instance of *verify-the-load-bearing-assumption*); **(4)** `pac solution import` runs 25+ min [field-observed] — for sitemap-only changes PATCH `sitemaps({id}).sitemapxml` over the Web API + `PublishAllXml`, with `%2F`-encoded SubArea URLs. Owned by `model-driven-engineer`; companion to the [`dataverse-web-resources`](skills/dataverse-web-resources/SKILL.md) skill. Production lesson 2026-06-11. |
...
| [`knowledge/power-apps-code-apps-gotchas.md`](knowledge/power-apps-code-apps-gotchas.md) | Implementing or debugging a **SharePoint Person/Group column write**, a **Copilot Studio multi-turn conversation**, or any **Code App SharePoint Choice-field write** from a Power Apps Code App. Covers: (1) People-Picker UPN ≠ email — requires claims string `i:0#.f|membership|${upn}` built via `Office365UsersService.MyProfile_V2()` (silent failure if omitted; Office365Users connector is **premium**); (2) Copilot Studio connector code-generator bug — `pac code add-data-source` mangles `x-ms-conversation-id` → `x_ms_conversation_id` (underscores), breaking multi-turn conversation tracking (manual fix in generated service; re-generation hazard documented); (3) SharePoint Choice-field payload [unverified — contested]: zyborc documents `{Value:"..."}` wrapper; Microsoft Learn (fetched 2026-06-10) shows full expanded object from `getReferencedEntity()` — silent failure either way if wrong; (4) SDK/CLI state: `pac code` deprecated in favor of `power-apps` npm CLI (SDK ≥1.0.4) [verified Microsoft Learn 2026-06-10]. Source: Alexander Siedler (zyborc, `power-apps-code-apps-skill`, 2026-03); subsumption checked 2026-06-10 against `microsoft/power-platform-skills` (rc-deep-research `wf_e0543b9a-dde`). Owned by `power-fx-engineer` when the work is a Code App. [verify-at-use] |
...
| [`knowledge/pbip-report-fast-triage.md`](knowledge/pbip-report-fast-triage.md) | **For ANY PBIP/PBIR report symptom** — the fast-triage debug hub that routes from observed symptom → correct knowledge file in 1–2 steps. Two tables: (a) symptom→cause→fix-location keyed on what the agent observes (infinite-spinner / zero-vs-blank / missing-rows / wrong-or-missing-color / first-deploy-fail / filter-wrong-scope / deployment-var-ignored), each with a file-level link + §-as-text pointer to the authoritative fix; (b) a formatting/theming property-location decision rule (chrome→`visualContainerObjects`; data content→`objects`; silent no-op→wrong suffix or unknown property→`pbir-enhanced-reference §9`) that generalises to new bugs, not just the 6 known ones. Also contains the graceful-degradation source chain: Tier 1 our captured reference (zero network/MCP — always works); Tier 2 official MS non-MCP (cited direct `learn.microsoft.com/...` URLs + `github.com/microsoft/json-schemas` — fetch via WebFetch without any MCP); Tier 3 the Learn MCP (optional accelerator — only if your org permits MCPs); Tier 4 data-goblin `pbir-format/references` (GPL, re-express only). **MCP-optional by design: works where all MCPs are banned.** Owned by `power-bi-engineer`. Added 2026-06-10. |
