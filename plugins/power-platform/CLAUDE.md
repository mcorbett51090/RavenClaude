...
The `flow-engineer`, `solution-alm-engineer`, `dataverse-architect`, and `power-platform-admin` agents each carry compact inline priors that summarize the relevant lesson from each knowledge file; the full files in `knowledge/` are the source of truth and get re-read on demand. The `pcf-developer`, `power-fx-engineer`, and `power-pages-engineer` agents carry the decision-tree-traversal prior for `pcf-react-fluent-platform-libraries.md` (and `pcf-developer` additionally carries the Fluent v9 theming pointer); `copilot-studio-engineer` carries the traversal prior for `copilot-agents-2026.md`; `power-bi-engineer` carries the traversal prior for `pbir-enhanced-report-loading.md`.
