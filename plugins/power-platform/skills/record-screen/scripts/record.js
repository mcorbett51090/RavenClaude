...
async function serve() {
  ...
  function saveFrame(base64Data) {
    ...
    fs.writeFileSync(path.join(recordingState.tempDir, name), Buffer.from(base64Data, "base64"));
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      // --- Status ---
        ...
        const connected = extensionWs && extensionWs.readyState === WebSocket.OPEN;
        if (recordingState) {
          ...
          res.end(JSON.stringify({ status: "idle", extensionConnected: connected }));
        }
      } else {
        res.statusCode = 404;
...
function apiRequest(method, urlPath, body = null, timeoutMs = 120_000) {
  return new Promise((resolve, reject) => {
    ...
    req.on("error", () => {
      reject(new Error('Cannot connect to server. Run "node record.js serve" first.'));
    });
    req.on("timeout", () => {
      req.destroy();
...
async function showStatus() {
  ...
  if (result.status === "recording") {
    ...
    console.log(`EXTENSION: ${result.extensionConnected ? "connected" : "disconnected"}`);
  } else {
