...
>
> **Platform status — PBIR is now the DEFAULT report format (Jan 2026 rollout).** New reports created in Power BI Desktop / Service now default to PBIR; **editing and saving an existing report auto-converts it to PBIR**; PBIR-Legacy support is slated to end at **GA (planned 2026 — exact date `[verify-at-build]`; do not quote a specific GA month)**. Practical upshot for this reference: PBIR authoring is no longer an opt-in/preview path — treat it as the live default, but keep version-specific schema claims `[verify-at-use]` since the format is still moving toward GA. Source: Power BI blog, "PBIR will become the default Power BI report format" (retrieved 2026-06-16). See the triage hub [`pbip-report-fast-triage.md`](pbip-report-fast-triage.md) for symptom routing.
>
...
> **Observation — `kpi` can render blank even when byte-identical to a working twin `[low confidence — single environment, 2026-06-22]`.** In one Fabric workspace (BMA-CSP-Risk-Scoring V3), a `kpi` tile authored to match a confirmed-working V2 `kpi` (same `Indicator` role, same Literal `indicator.fontColor`) rendered blank, while the V2 tile rendered fine in the same workspace at the same time. Switching the tile to **`card` + `Values` role + `labels`/`categoryLabels`** (the V2-proven count/score-tile pattern) fixed it. This may have been a transient masked by the parallel switches, or a V3-context difference not yet isolated — treat it as unexplained, not as "`kpi` is broken". **If a `kpi` fails despite byte-parity with a working exemplar, check the semantic model for a TMDL parse error first** — a broken measure/TMDL can make specific visual types silently fail while others still render. Prefer `card` for count/score tiles regardless. Note the boundary: the [`visual-feedback-loop`](../../ravenclaude-core/skills/visual-feedback-loop/SKILL.md) `parity` gate helps only when the candidate and exemplar are **not** byte-identical (it flags role/key/`$id` *divergence*); a byte-identical-yet-blank tile returns `pass` from the gate — its cause is outside skeleton structure (TMDL parse error or a transient Fabric issue), so diagnose there.
