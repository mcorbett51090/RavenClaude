...
> **Last reviewed:** 2026-06-11. Source: a production managed-ALM session (BTCSI-style DEV-unmanaged → TEST/PROD-managed pipeline; org/repo identifiers removed). A managed export failed on a hard dependency on the **Active** solution; the fix attempt **deleted and recreated 19 entities — twice** before the real, no-delete fix was found. This file captures the verified root cause so the next agent skips the rabbit hole. Refresh when the Dataverse Web API metadata surface or the `AddSolutionComponent` message shape changes.
>
