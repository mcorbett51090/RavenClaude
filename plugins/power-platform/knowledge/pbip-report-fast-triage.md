...
| **Missing rows** — some entities/submissions absent from the report with no warning | M Query `Table.RemoveRowsWithErrors` silently drops workbooks that lack a specific named table; OR Dataverse lookup display field not split at load boundary | [`knowledge/pbir-m-query-pitfalls.md`](pbir-m-query-pitfalls.md) | §1 (Excel/folder drop) + §2 (Dataverse lookup) |
