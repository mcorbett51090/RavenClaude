# Shift left — cheapest to fix in design, costliest after release.
...
## The rule
Color, contrast, focus order, target size, heading structure, and alt-text intent are nearly free to get right in design and code review, and expensive to retrofit across a shipped product; bake the criteria into the design system and the definition-of-done, not a pre-launch audit scramble.
...
The §4 failure mode: acting as if "shift left — cheapest to fix in design, costliest after release." weren't true — the most common way an analysis quietly misleads the practitioner who acts on it. The plugin's advisory hook flags a deliverable that reads as if this rule were ignored.
