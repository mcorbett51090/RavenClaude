# Automated tools catch only a fraction — manual + AT testing is required.
...
## The rule
Automated scanners reliably detect roughly a third of WCAG issues [unverified — training knowledge] and miss the human-judgment criteria entirely (focus order, meaningful alt text, name/role/value correctness); a clean axe scan is a starting point, never a conformance claim.
...
The §4 failure mode: acting as if "automated tools catch only a fraction — manual + AT testing is required." weren't true — the most common way an analysis quietly misleads the practitioner who acts on it. The plugin's advisory hook flags a deliverable that reads as if this rule were ignored.
