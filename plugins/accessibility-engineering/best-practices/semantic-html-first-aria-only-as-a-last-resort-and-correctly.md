# Semantic HTML first, ARIA only as a last resort — and correctly.
...
## The rule
A native <button>, <a>, <label>, <nav>, or heading carries role, state, and keyboard behavior for free; ARIA adds none of that behavior and a wrong role/state is worse than no ARIA ("No ARIA is better than bad ARIA"). Reach for ARIA only when no native element fits.
...
The §4 failure mode: acting as if "semantic HTML first, ARIA only as a last resort — and correctly." weren't true — the most common way an analysis quietly misleads the practitioner who acts on it. The plugin's advisory hook flags a deliverable that reads as if this rule were ignored.
