/**
 * KV-backed IdempotencyStore for the Stripe FRAMEWORK tier.
 *
 * Next.js/Astro API routes are routinely deployed to a serverless/edge
 * runtime (Vercel Functions, Cloudflare Pages Functions) that can recycle —
 * or cold-start — between webhook deliveries, exactly like the static
 * tier's function. An in-process Set (see
 * ../../shared/idempotency-store's InMemoryIdempotencyStore) would silently
 * lose state and re-run side effects on a Stripe retry (CLAUDE.md §2 #3).
 * This file backs the shared IdempotencyStore interface with an EXTERNAL
 * key-value store reached over HTTP.
 *
 * Running this app on a long-lived Node process instead (self-hosted,
 * Docker, a VM that never recycles)? You may swap this for
 * `InMemoryIdempotencyStore` from the shared contract, or better, your
 * existing database — but confirm the process truly never recycles before
 * making that trade.
 *
 ...
 * dependency, so it works unmodified from a Node API route or an edge route.
 */
