/**
 * Static-checkable tests for the Stripe FRAMEWORK tier webhook route.
 * Run with: npx tsx --test templates/stripe/framework/tests/webhook.test.ts
 * (or wire into Vitest/Jest — the same `node:test`-shaped assertions port
 * with minimal adaptation, which is what most Next.js/Astro projects use).
 *
...
const PUBLISHABLE_KEY = "pk_test_not_a_real_key_0000000000000000000000000";
...
function buildDeps() {
  const provider = new StripeFrameworkProvider({
...
test("a missing signature header is rejected", async () => {
  ...
  const response = await handler(
    new Request("https://example.com/api/stripe/webhook", { method: "POST", body: payload }),
  );
  assert.equal(response.status, 400);
});
