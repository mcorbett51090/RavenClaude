/**
 * ShopifyProvider — static tier. Implements the shared PaymentProvider
 * contract (../../shared/payment-provider.ts). Shopify is CAPABILITY-
 * INVERTED relative to Stripe/Square: it owns the catalog AND checkout, and
 * checkout is hosted-only, so this provider omits authorize/capture/refund/
 * cancel entirely rather than stub them (CLAUDE.md §2 #1, capabilities.ts).
 */
import type {
  PaymentProvider,
  CreateCheckoutInput,
...
export interface ShopifyProviderConfig extends ShopifyConfig {
  /** Webhook signing secret from the Shopify admin. Server-side only — never bundle this. */
...
export class ShopifyProvider implements PaymentProvider {
   ...
   * effect on what the buyer is actually charged, which Shopify computes
   * server-side from the variant. Passing a display estimate here is fine;
   * passing a wrong one cannot under- or over-charge anyone.
   */
  ...
  // these exist — see PaymentProvider's doc comment.
}
