# Forecast spend and alert on anomalies.
...
## The rule
A forecast turns cloud cost from a monthly surprise into a managed number, and an anomaly alert catches a runaway resource in hours instead of on the invoice. Budget against the forecast; alert on the deviation.
...
The §4 failure mode: acting as if "forecast spend and alert on anomalies." weren't true — the most common way an analysis quietly misleads the practitioner who acts on it. The plugin's advisory hook flags a deliverable that reads as if this rule were ignored.
