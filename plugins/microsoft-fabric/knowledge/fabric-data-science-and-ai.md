...
- **Fabric Data Agents — GA (March 2026).** Conversational, **read-only** NL Q&A over Lakehouse / Warehouse / Power BI semantic models / KQL / SQL & mirrored DBs / ontology / Microsoft Graph; internally applies **Azure OpenAI Assistant APIs** as its underlying engine (Microsoft's `concept-data-agent.md` still describes it this way). Enforces the **requesting user's** permissions (least-privilege), tenant/workspace policy, and **Purview** governance (DLP, access policies); read-only connections only; optional Azure AI Content Safety. **CI/CD + diagnostics are GA.** Configure with custom instructions/examples. Integrates with **Microsoft Foundry (Foundry IQ)**, **Copilot Studio** (embed as a skill), and **M365 Copilot**. → seam to `azure-cloud` (see [`azure-ai-foundry.md`](../../azure-cloud/knowledge/azure-ai-foundry.md)) and `power-platform` (Copilot Studio).
  - **⛔ Programmatic/external consumption note (added 2026-09-23, Microsoft-Learn MCP, High confidence):** the above describes the agent's *internal* engine, which is separate from how a client *consumes* a published agent. OpenAI sunset the **Assistants API** on **2026-08-26**; the Python client SDK and the Fabric Data Agent External Client repo that used `beta.assistants`/`beta.threads` no longer work. Consumption now goes through the **MCP endpoint** (`https://learn.microsoft.com/fabric/data-science/data-agent-mcp-server` — streamable HTTP, Fabric bearer token, one MCP tool per published agent, **no built-in conversation management** — callers must orchestrate multi-turn state themselves) or the **Fabric OpenAI Responses client** (migration available since 2026-08-11; only the querying code changes, not agent creation/config/publish). Also note: responses returned via the Python client SDK "might be sent outside of Fabric's compliance boundary or geographic region" — flag this to `ravenclaude-core/security-reviewer` on any cross-geo-sensitive engagement.
...
- **AI functions — GA (Nov 2025).** Row-level LLM enrichment in notebooks/Spark (`ai.embed()`, sentiment, entity-extraction, summarize, generate); **multimodal (preview, March 2026)** — images/PDFs via `aifunc.load` / `ai.infer_schema`; default concurrency 200; Azure OpenAI + Foundry models (incl. Claude/LLaMA).
