...
Research-sweep **addition** (Tier-A weekly news sweep) — **Claude Sonnet 5 (`claude-sonnet-5`) went GA 2026-06-30** and the capability map had zero mention of it while still naming **Sonnet 4.6 as "the balanced default"** — now a false assertion the routing ladder + context-budget planner consume. Updated `knowledge/model-selection-and-2026-capability-map.md`: added a **Sonnet 5** lineup row as the new balanced default (demoting Sonnet 4.6 to "prior — superseded, still valid, not retired"); updated the routing-ladder paragraph, the adaptive-thinking / 1M-context / structured-output capability-status rows, the header `Last reviewed` (→ 2026-07-05), and the source list. Facts grounded on the **first-party [Sonnet 5 doc](https://platform.claude.com/docs/en/about-claude/models/whats-new-sonnet-5) (fetched cleanly 2026-07-05)**: 1M context / 128k out; **$3/$15** standard (unchanged from 4.6), **intro $2/$10 through 2026-08-31**; **adaptive thinking ON by default** (manual extended thinking → 400); **non-default sampling params → 400**; **new tokenizer ≈ +30% tokens** for the same text (a real FinOps + context-budget caveat, called out explicitly); first Sonnet-tier with real-time cybersecurity safeguards (refusal → HTTP 200 `stop_reason:"refusal"`). Framed as **superseding Sonnet 4.6 only — not Opus 4.8 or Fable 5**, and "default in Claude Code" carries `[verify-at-use — fast-moving surface]`. The marketplace description's model-right-sizing example was refreshed Sonnet 4.6 → Sonnet 5. **Intentionally NOT touched:** the ~40 illustrative `claude-sonnet-4-6` code examples across `best-practices/` and the 6 agent personas (per house opinion #14 — "ids live in the dated capability map, not baked into personas"; Sonnet 4.6 remains a valid id). Routed through two expert panels (usefulness → **USEFUL/unanimous**; detailed review → **APPROVE-WITH-FIX**, fixes applied: explicit pricing + 128k-output + model-id/GA-date stated, anti-overclaim line, `[verify-at-use]` on the Claude-Code-default + intro-pricing-expiry). **Migration:** none — knowledge-file content only.

## [0.9.7] — 2026-07-02

### Fixed

- **`check-claude-app-anti-patterns.sh` STRICT mode now actually blocks.** `CLAUDE_APP_STRICT=1` exited `1` (a non-blocking error Claude Code swallows) instead of `2` (the PreToolUse deny code), so the documented blocking mode was inert. Fixed to `exit 2`. (Autonomous 3-panel repo review, P1.)
