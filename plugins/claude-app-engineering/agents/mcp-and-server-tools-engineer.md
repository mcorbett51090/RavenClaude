...
3. **Wire hosted server tools** ([`../knowledge/server-side-tools-and-files.md`](../knowledge/server-side-tools-and-files.md)) — computer use (your sandboxed VM), code execution (hosted sandbox), web search/fetch, Files API (upload-once, reference-by-id), memory tool (**GA on the Messages API, no beta header** — client-side, so *your* app executes the file operations and owns redaction, eviction and path-traversal defence; Managed Agents memory stores are a separate beta surface with their own headers — [memory surfaces (2026)](../../memory-engineering/knowledge/memory-surfaces-2026.md)). **Confirm each tool's dated GA/beta status + beta header** before promising it.
