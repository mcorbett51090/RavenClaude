...
The hard part of a Claude app is rarely the first prompt — it's the decisions: Messages API or Agent SDK or Managed Agents? which model? how do I make the cache hit? structured output? MCP server or in-process tool? how do I keep the bill sane and the app up under 429s? This plugin encodes those as a team of advisory specialists backed by a, retrieval-dated, citation-grounded knowledge bank — important because the platform ships monthly.
...
Plus a knowledge bank (build-surface decision tree + dated capability map + caching/tools/MCP/server-tools/Agent-SDK/evals/FinOps playbooks), templates, and an advisory hook (house opinions).
