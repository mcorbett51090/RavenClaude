# The power budget IS the spec — battery life gates the design.
...
## The rule
For a battery or harvested device, average current draw determines battery life and therefore product viability; build the duty-cycled current profile (active mA × active fraction + sleep mA × sleep fraction) first and let it constrain the MCU, radio, and sampling choices — not the other way round.
...
The §4 failure mode: acting as if "the power budget IS the spec — battery life gates the design." weren't true — the most common way an analysis quietly misleads the practitioner who acts on it. The plugin's advisory hook flags a deliverable that reads as if this rule were ignored.
