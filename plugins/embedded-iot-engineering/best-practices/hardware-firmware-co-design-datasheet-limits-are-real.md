# Hardware/firmware co-design — datasheet limits are real.
...
## The rule
Firmware lives inside the silicon's constraints: clock/voltage operating points, peripheral throughput, ADC settling, flash endurance/wear, and brown-out behavior are datasheet facts you design around — verify against the specific part and revision, and measure on the real board (§3 #8).
...
The §4 failure mode: acting as if "hardware/firmware co-design — datasheet limits are real." weren't true — the most common way an analysis quietly misleads the practitioner who acts on it. The plugin's advisory hook flags a deliverable that reads as if this rule were ignored.
