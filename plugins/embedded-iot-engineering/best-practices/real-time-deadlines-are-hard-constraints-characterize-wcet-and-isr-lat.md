# Real-time deadlines are HARD constraints — characterize WCET and ISR latency.
...
## The rule
A real-time system is correct only if it also meets its timing; characterize worst-case execution time and interrupt latency for the critical path, and verify the schedule (rate-monotonic / deadline) holds under worst case — average-case timing is not a deadline guarantee.
...
The §4 failure mode: acting as if "real-time deadlines are HARD constraints — characterize WCET and ISR latency." weren't true — the most common way an analysis quietly misleads the practitioner who acts on it. The plugin's advisory hook flags a deliverable that reads as if this rule were ignored.
