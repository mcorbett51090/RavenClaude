...
findings=()
if grep -nEi "select\\s+\\*\\s+from" "$scan_target" >/dev/null 2>&1; then
  findings+=("SELECT * — name the columns (avoids wide-row reads, breaks on schema change, blocks covering-index use).")
fi
if _rc_pcre_match "$scan_target" "create\\s+index\\s+(?!concurrently)"; then
  findings+=("CREATE INDEX without CONCURRENTLY — on a live table this takes a blocking lock; use CONCURRENTLY.")
fi
if _rc_pcre_match "$scan_target" "(alter\\s+table\\s+\\S+\\s+add\\s+column\\s+.*not\\s+null(?!.*default)|set\\s+not\\s+null)"; then
...
if grep -nEi "where\\s+\\w+\\s*\\(\\s*\\w+\\s*\\)\\s*=|where\\s+(lower|upper|date)\\s*\\(" "$scan_target" >/dev/null 2>&1; then
