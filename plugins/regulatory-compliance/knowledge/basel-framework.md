...
>
> **Sourcing caveat:** this file is authored from **well-established, training-stable BCBS framework knowledge** (the three-pillar architecture, the capital stack, the Pillar-1 minimum ratios, the buffers, LCR/NSFR, the leverage ratio, the output floor) — these are foundational and corroborated across the BCBS consolidated framework, national rulebooks, and the jurisdiction files already in this plugin. **The jurisdiction-specific *implementation dates and percentages* in §7 are the fast-moving layer** and are marked `[unverified — confirm against the primary rulebook/policy statement]` where they could have moved since the Jan-2026 knowledge boundary. Resolve those pins against the BCBS standard (bis.org/basel_framework) or the national instrument before they gate advice. Ratios and buffer levels in §2–§5 are the BCBS minimums and are stable.
>
...
The floor **phases in** (BCBS canonical schedule, post-COVID one-year deferral): **50% (2023) → 55% → 60% → 65% → 70% → 72.5% (2028)**. **Jurisdictions vary the start year and may run additional transitional reliefs** — confirm the applicable phase-in step in the implementing instrument (see §7). `[unverified — confirm the exact per-year step and end-state date in the binding rulebook]`
...
| **EU** | **CRR3 / CRD6** | **1 Jan 2025** (FRTB own-funds piece **delayed to 1 Jan 2026** by EU delegated act) | `[unverified — confirm the FRTB go-live wasn't deferred again]` |
| **UK** | **Basel 3.1** (PRA — **PS1/26**) | **1 Jan 2027** | Pinned across the plugin's `uk-pra-specialist`; lands together with the **Strong & Simple SDDT** regime (PS4/26). `[unverified — confirm 1 Jan 2027 held]` |
| **US** | **"Basel III Endgame"** (final rule) | Proposed Jul 2023; **re-proposal under revision** | **Fast-moving / uncertain** — scope and calibration were materially contested and the timeline slipped past the original 2025 target. `[unverified — fast-moving; confirm current status before advising a US bank]` |
...
> **Why this table is pinned so heavily:** the *substance* of Basel 3.1 is stable, but **which year a given bank actually adopts which piece** is the single most error-prone fact in this domain — jurisdictions defer, phase, and carve out differently. **Never quote an implementation date to a specific firm without confirming it against that jurisdiction's current policy statement / rulebook.** If you can WebFetch, pull the primary instrument; if a date here is `[unverified]`, resolve it before it gates advice.
