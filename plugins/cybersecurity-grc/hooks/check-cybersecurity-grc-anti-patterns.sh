...
if grep -qiE "statement of applicability|\bSoA\b|annex\s*a" "$scan_target" 2>/dev/null; then
  if grep -qiE "(not[ -]applicable|\bN/?A\b|excluded)" "$scan_target" 2>/dev/null \
    && ! grep -qiE "(justif|rationale|reason|because|residual\s+risk)" "$scan_target" 2>/dev/null; then
...
if grep -qiE "risk\s*register|likelihood|inherent\s+risk|residual\s+risk" "$scan_target" 2>/dev/null; then
  if ! grep -qiE "(control|treatment|mitigat|accept|transfer|avoid)" "$scan_target" 2>/dev/null; then
  ...
  if ! grep -qiE "(owner|accountable|responsible)" "$scan_target" 2>/dev/null; then
...
if grep -qiE "operating[ -]effective|operates effectively|control is in place|control operates" "$scan_target" 2>/dev/null; then
  if ! grep -qiE "(evidence|artifact|log|screenshot|export|sample|test\s+result|monitor)" "$scan_target" 2>/dev/null; then
