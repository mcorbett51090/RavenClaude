# Validate online with A/B — offline gains don't always transfer.
...
## The rule
Offline NDCG improvements can fail to move CTR or conversion because the judgment list, position bias, or the metric diverges from user intent; confirm an offline win with an online A/B before declaring victory.
...
The §4 failure mode: acting as if "validate online with A/B — offline gains don't always transfer." weren't true — the most common way an analysis quietly misleads the practitioner who acts on it. The plugin's advisory hook flags a deliverable that reads as if this rule were ignored.
