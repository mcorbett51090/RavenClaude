# Relevance is measured, not vibed.
...
## The rule
NDCG, MRR, and precision@k against a judgment list are the defensible signal; 'these results look better to me' is one person's session, not a metric. Quantify ranking quality before and after every change, or you're tuning blind.
...
The §4 failure mode: acting as if "relevance is measured, not vibed." weren't true — the most common way an analysis quietly misleads the practitioner who acts on it. The plugin's advisory hook flags a deliverable that reads as if this rule were ignored.
