# Skilled Trades Contracting best-practice docs

Named, citable rules for the `skilled-trades-contracting` plugin's 4 specialist agents and any consultant running a skilled trades contracting engagement. Each file is one rule — read, applied, and cited whole. Grounded in this plugin's `knowledge/` bank and its agent constitutions.

---

## Index

_18 rules. Each file is one named, citable rule._

| Doc | Status | Use when |
|---|---|---|
| [`estimate-to-a-fully-loaded-labor-rate-not-a-wage.md`](./estimate-to-a-fully-loaded-labor-rate-not-a-wage.md) | Absolute rule | Any skilled trades contracting deliverable where this question is in play — read, applied, and cited whole. |
| [`price-service-on-a-flat-rate-book-not-guessed-hours.md`](./price-service-on-a-flat-rate-book-not-guessed-hours.md) | Absolute rule | Any skilled trades contracting deliverable where this question is in play — read, applied, and cited whole. |
| [`billable-hour-efficiency-is-the-fields-master-number.md`](./billable-hour-efficiency-is-the-fields-master-number.md) | Absolute rule | Any skilled trades contracting deliverable where this question is in play — read, applied, and cited whole. |
| [`first-time-fix-and-callback-rate-are-margin-not-just-quality.md`](./first-time-fix-and-callback-rate-are-margin-not-just-quality.md) | Pattern | Any skilled trades contracting deliverable where this question is in play — read, applied, and cited whole. |
| [`material-cost-is-the-real-cost-plus-waste-plus-markup-name-a.md`](./material-cost-is-the-real-cost-plus-waste-plus-markup-name-a.md) | Pattern | Any skilled trades contracting deliverable where this question is in play — read, applied, and cited whole. |
| [`the-truck-is-a-profit-center-with-a-utilization-number.md`](./the-truck-is-a-profit-center-with-a-utilization-number.md) | Pattern | Any skilled trades contracting deliverable where this question is in play — read, applied, and cited whole. |
| [`quote-close-rate-and-average-ticket-are-the-sales-levers.md`](./quote-close-rate-and-average-ticket-are-the-sales-levers.md) | Pattern | Any skilled trades contracting deliverable where this question is in play — read, applied, and cited whole. |
| [`date-and-source-any-wage-material-or-market-figure.md`](./date-and-source-any-wage-material-or-market-figure.md) | Absolute rule | Any skilled trades contracting deliverable where this question is in play — read, applied, and cited whole. |
| [`overhead-allocation-rate-must-be-built-before-pricing-any-job.md`](./overhead-allocation-rate-must-be-built-before-pricing-any-job.md) | Absolute rule | Any pricing review; annual rate rebuild; new flat-rate book development. |
| [`maintenance-agreements-are-a-recurring-revenue-engine-not-just-a-service-feature.md`](./maintenance-agreements-are-a-recurring-revenue-engine-not-just-a-service-feature.md) | Pattern | Service contractors; recurring revenue review; customer retention analysis. |
| [`job-costing-closes-the-loop-between-estimate-and-actual.md`](./job-costing-closes-the-loop-between-estimate-and-actual.md) | Absolute rule | Any job margin review; estimating accuracy improvement; P&L investigation. |
| [`good-better-best-option-presentation-raises-average-ticket-without-more-calls.md`](./good-better-best-option-presentation-raises-average-ticket-without-more-calls.md) | Pattern | Service technician training; average ticket improvement; flat-rate book design. |
| [`subcontractor-markup-must-cover-coordination-risk-and-warranty-exposure.md`](./subcontractor-markup-must-cover-coordination-risk-and-warranty-exposure.md) | Pattern | Any job with subcontracted scope; sub markup review; risk pricing. |
| [`technician-certification-and-licensing-are-a-pricing-floor-not-just-compliance.md`](./technician-certification-and-licensing-are-a-pricing-floor-not-just-compliance.md) | Pattern | Technician onboarding; pricing strategy; business positioning review. |
| [`dispatch-sequence-is-a-revenue-decision-not-just-a-logistics-problem.md`](./dispatch-sequence-is-a-revenue-decision-not-just-a-logistics-problem.md) | Pattern | Dispatch protocol design; revenue per call optimization. |
| [`warranty-call-tracking-is-a-quality-and-cost-control-system.md`](./warranty-call-tracking-is-a-quality-and-cost-control-system.md) | Pattern | Callback reduction program; technician quality review; job cost investigation. |
| [`change-order-discipline-protects-margin-on-every-construction-job.md`](./change-order-discipline-protects-margin-on-every-construction-job.md) | Absolute rule | Any construction or new-install job; scope creep investigation. |
| [`seasonal-demand-curves-shape-pricing-staffing-and-cash-flow-planning.md`](./seasonal-demand-curves-shape-pricing-staffing-and-cash-flow-planning.md) | Pattern | Annual business planning; staffing decisions; cash-flow forecasting. |

## See also
- [`../knowledge/`](../knowledge/) — the research-grounded knowledge bank these rules distill.
- [`../CLAUDE.md`](../CLAUDE.md) — the team constitution (house opinions §3, anti-patterns §4, output contract §6).
