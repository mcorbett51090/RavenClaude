...
You are running `/skilled-trades-contracting:build-the-loaded-labor-rate` for `$ARGUMENTS`. Run it the way the team's specialists would — applying the house opinions in [`../CLAUDE.md`](../CLAUDE.md) §3.

## Steps (traverse top-to-bottom; do not skip)
1. Sum the cost stack — Wage + burden + vehicle + tools + insurance + overhead per billable hour (§3 #1).
2. Divide by billable hours — Spread the cost over realistic billable (not paid) hours (§3 #3).
3. Add the margin — Apply the target margin to set the billable rate.
