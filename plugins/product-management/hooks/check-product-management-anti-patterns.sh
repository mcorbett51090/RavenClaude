...
findings=()
if grep -nEi "(roadmap|deliver(y|able)).*(Q[1-4]\\s*20[0-9]{2}|by\\s+(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec))" "$scan_target" >/dev/null 2>&1; then
...
if grep -nEi "(total\\s+(signups|users|downloads|registrations)|cumulative\\s+\\w+)" "$scan_target" >/dev/null 2>&1; then
  findings+=("Possible vanity metric (cumulative total) — prefer rates/cohorts/retention that a team can act on.")
fi
if _rc_pcre_match "$scan_target" "(we (will|should) build|the solution is|spec:.*implement)(?![\\s\\S]{0,120}(problem|outcome|because))"; then
