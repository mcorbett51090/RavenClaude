...
| **G3** | Design system & tokens | `visual-designer` (primary) + `frontend-implementer` | `design-tokens-scaffolding`, `brand-guidance-authoring`, `design-system-audit`, `card-tile-ui` | G2 | **G4** | Token JSON (light+dark) + audited design-system spec + brand-guidance.md |
...
- **Acceptance criteria.**
  ...
  - [ ] Card-sort run (**8–12 participants**, per [`information-architecture`](../../skills/information-architecture/SKILL.md) §4) if this is a nav restructure rather than greenfield. **Before wireframing, browse the relevant categories in CollectUI's ~100-item Daily UI pattern taxonomy** (<https://collectui.com>, grouped by page-type work in [`knowledge/design-sources/collectui.md`](../../knowledge/design-sources/collectui.md)) so the page inventory is checked against current real-world range, not one remembered layout — an inspiration input to G2/G4, never a substitute for the card-sort itself. **Fallback when no human panel is available** (e.g. a solo-founder engagement with no research budget): substitute a **synthetic-persona / stakeholder-proxy sort** — sort the content against the G1 audience personas and named stakeholders — and record it under the standard waiver (owner + reason + condition: `no user panel — validated by proxy, re-run with real participants if budget appears`). The waiver mechanism (§5) explicitly covers this criterion; a nav restructure never *silently* skips validation.
...
- **Entry.** G2 closed.
- **Dispatch.** `visual-designer` (primary) + `frontend-implementer` (co-owner, code-wiring half) → [`design-tokens-scaffolding`](../../skills/design-tokens-scaffolding/SKILL.md), [`brand-guidance-authoring`](../../skills/brand-guidance-authoring/SKILL.md), [`design-system-audit`](../../skills/design-system-audit/SKILL.md), and [`card-tile-ui`](../../skills/card-tile-ui/SKILL.md) where the surface is card-driven.
- **Acceptance criteria.**
  ...
  - [ ] A generation-time **brand-guidance artifact** (`brand-guidance.md`) exists alongside the token JSON. Owning skill: [`brand-guidance-authoring`](../../skills/brand-guidance-authoring/SKILL.md). Modeled on Vercel's `design.md` pattern — see [`knowledge/design-sources/vercel-design-md.md`](../../knowledge/design-sources/vercel-design-md.md) and the disambiguation in [`knowledge/design-md-token-interop.md`](../../knowledge/design-md-token-interop.md) (same-filename-different-job as the Google Labs `DESIGN.md` token-export format — don't conflate them). **Fail-closed on structure only, never on taste:**
    1. `brand-guidance.md` exists in the gate ledger beside `design-tokens.json`.
    2. All 7 required sections present, in order.
    3. Typeface count ≤ 2.
    4. Every catalogue row has a resolved `override.status`; every non-`enforced` row has a non-empty `rationale`.
    5. Zero adjectival rules (the observable-statement check).
    A build with no real brand surface (a pure web-app archetype, authenticated-only tool) records `N/A — <one-line reason>` per §1's binding N/A discipline — this is the pipeline's existing sanctioned way to not-run a criterion without a silent skip. **No judgment of whether the aesthetic is *good* enters this gate** — that is [`brand-polish-checklist.md`](../../skills/brand-guidance-authoring/reference/brand-polish-checklist.md)'s job, and it stays advisory.
...
[ ] G3  Primitive→semantic tokens (W3C JSON); light + dark, no residue; contrast pre-checked ≥4.5:1 / 3:1; design-system-audit clean; brand-guidance.md exists (7 sections, ≤2 typefaces, overrides resolved, zero adjectival rules) or N/A recorded
