...
- **Acceptance criteria.**
  ...
  - [ ] **(Recommended, not yet mandatory)** A generation-time **brand-guidance artifact** exists alongside the token JSON — the tokens answer "what are the values," this answers "how does an agent reliably apply them": voice/hierarchy/layout rules written as **observable, checkable statements** (never adjectival — "let evidence tables use the full available width," not "keep it clean"), plus a **named-anti-pattern catalogue** the model can pattern-match against. Modeled on Vercel's `design.md` pattern (measured 91→39 mechanical-failure reduction with the file present vs. absent) — see [`knowledge/design-sources/vercel-design-md.md`](../../knowledge/design-sources/vercel-design-md.md) and the disambiguation in [`knowledge/design-md-token-interop.md`](../../knowledge/design-md-token-interop.md) (same filename as the Google Labs token-export format, different job — don't conflate them). Not yet a fail-closed criterion because no owning skill formalizes its authoring today; treat this checkbox as a standing invitation to close that gap, not a loop-back trigger.
