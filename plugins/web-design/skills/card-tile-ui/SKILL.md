...
7. **Gentle motion only.** Cards lift `translateY(-2px)` + deepen one shadow step on hover, ~0.18s ease. No scale, no glow pulses. Honor `prefers-reduced-motion`. If a brief wants more than this restrained lift, that's a different job (a memorable marketing-site motion beat) — reach for a single-purpose, per-effect-engine approach the way [ReactBits](https://reactbits.dev) does (CSS-only for a simple effect, one heavier library only where the effect actually needs it — see [`../../knowledge/design-sources/reactbits.md`](../../knowledge/design-sources/reactbits.md)) rather than reaching for a house animation library and then trying to restrain it back down to this pattern's motion budget.
