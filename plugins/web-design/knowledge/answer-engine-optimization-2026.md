...
2. **Structured data (JSON-LD) — `FAQPage` demoted, not the priority.** Google **stopped showing FAQ rich results in Search on 2026-05-07** (Search Console's FAQ report, Rich Results Test support, and the API data were removed by August 2026); Google's own 2026-05-15 AI-search guide states structured data is not required for generative AI Search features and there's no special markup for them. `FAQPage` remains valid schema.org markup and is harmless to keep, but it is **no longer a Google SERP driver and its AI-citation benefit is unproven** — don't lead a GEO pitch with it. Layer `Article`, `HowTo`, `Product`, `Organization`, `Person` as relevant; these carry the real structured-data value. (This overlaps the technical-SEO skill's schema work — coordinate.)
...
**Correction (retrieved 2026-09-23):** the FAQPage guidance above was corrected against Google's own removal announcement (FAQ rich results stopped showing 2026-05-07; Search Console report / Rich Results Test / API data removed by August 2026) and Google's 2026-05-15 AI-search guide (no special markup required for AI Search features) — both cross-corroborated by multiple trade-press readings (Search Engine Journal, Search Engine Land) since the primary Google Search Central pages were egress-blocked this session.
