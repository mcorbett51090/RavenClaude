...
**Clone the craft, swap the identity.** Asked to "build it like `<reference site>`", drive [`design-clone`](../../ravenclaude-core/skills/design-clone/SKILL.md): it captures the reference's design schema — spacing/type scales, grid/breakpoints, elevation, component recipes — into sanitized tokens + fixed-structure component scaffolds wired to the **target's own** brand, with the reference's identity (logo, signature color) structurally unable to reach the output. **Fidelity is browser-gated, up front:** the apply path is an offline, declared-CSS transform — a structural clone of the design *system*, never pixel-fidelity. Verifying the re-skin actually *looks* like the reference is the render→compare loop's `ssim_score` gate ([`visual-feedback-loop`](../../ravenclaude-core/skills/visual-feedback-loop/SKILL.md)), present only when a browser tool is; offline it degrades to a structural sanity check ("declares the same design system") and reports fidelity **unverified**. Trade dress is **not** cleared here — route a distinctive whole-composition clone to `security-reviewer`.
