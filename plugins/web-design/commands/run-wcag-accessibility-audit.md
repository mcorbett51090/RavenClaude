---
description: Run a WCAG 2.2 AA accessibility audit the way it's actually tested — tab the whole page with the mouse unplugged, check semantic-HTML-before-ARIA, verify contrast against the displayed background, confirm visible focus and 24px (AA) / 44px (AAA) targets, and check reduced-motion and forced-colors fallbacks.
...
4. **Confirm visible focus and target size** (`a11y-visible-focus-and-target-size`): never bare `outline: none` — shape a `:focus-visible` ring; interactive targets meet at minimum the WCAG 2.2 **AA floor, SC 2.5.8, 24×24 CSS px** (44×44 CSS px is SC 2.5.5, **AAA** — a stretch goal / platform-HIG default for primary controls, not the AA audit floor), and the focused element isn't obscured by sticky chrome (SC 2.4.11).
