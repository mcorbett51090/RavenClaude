...
3. **Keep INP under 200ms by breaking up long tasks** (`perf-keep-inp-under-200ms`): find long main-thread tasks (DevTools / long-animation-frames API / web-vitals attribution), paint a pending state *before* the heavy work, then yield with a guarded `scheduler.yield()` (falling back to `setTimeout(fn, 0)` on Safari, which doesn't support it) and process in chunks; defer/async non-critical JS; move one unavoidable heavy computation to a Web Worker.
