...
11. **Third-party scripts are debt — with ONE bounded, standing exception.** Every third-party script (analytics, chat, A/B, fonts) costs perf + privacy + reliability. Catalogue them, audit them, retire them. **Exception (added 2026-07-21, `dual-analytics-standard`):** the **dual-analytics house pair — GA4 (Consent Mode v2, `analytics_storage:'denied'` default) + Cloudflare Web Analytics** — ships as **ONE bundled "house analytics" inventory line** in every new site/dashboard, wired *placeholder-until-provisioned* (each self-activates only in a production build when its **valid, non-placeholder** ID/token is set; empty ⇒ zero markup, zero network; each independently PROD-gated AND valid-id-gated). A named exception, not a repeal — every *other* third-party script is still audited one at a time, and a **third** analytics tool is still forbidden. Guardrails (do not weaken): **(a)** Cloudflare is the always-on baseline; **GA4 is wired-but-opt-in** and **never auto-fires on an authenticated/internal surface** without sign-off + URL/query redaction. **(b)** Cloudflare needs no consent banner; **GA4-with-consent-mode still needs a CMP in the EU** — never claim "no cookie banner" as a blanket dual claim. **(c)** with consent default-denied + no CMP, GA numbers are modeled for **all** traffic — **Cloudflare is the source of truth for raw pageviews; GA4 is for events/conversions.** **(d)** the GA Measurement ID + CF token are **PUBLIC identifiers, not secrets** — commit to config / `NEXT_PUBLIC_*`, never a secret store. **(e)** SRI-exempt (versionless); header CSP where possible, `<meta>` CSP + a documented no-enforced-integrity exemption on GitHub Pages. **(f)** exempt from the "pick one" *count*, not the CWV budget (`<50ms` TBT, no LCP regress — measured). Full reference implementation: `skills/third-party-script-hygiene/SKILL.md` §8–9.
...
1. **Check available skills first** — `design-system-audit`, `card-tile-ui`, `accessibility-review`, `core-web-vitals-tuning`, `seo-technical-audit`, `information-architecture`, `conversion-design`, `content-audit`, `design-tokens-scaffolding`, `brand-guidance-authoring`, `third-party-script-hygiene`, `fluent-react-implementation`, `static-site-implementation`, and the `gold-standard-website-pipeline` orchestration pipeline (for a whole new-website build), plus the core `frontend-coder` capabilities.
...
| [`skills/brand-guidance-authoring/SKILL.md`](skills/brand-guidance-authoring/SKILL.md) | `visual-designer`, `frontend-implementer`, `content-strategist` | Generation-time brand-voice contract (`brand-guidance.md`) — named aesthetic, ≤2 typefaces, tiny palette, 4px spacing scale, radius/motion stance, plus a 10-item named anti-pattern catalogue banning "AI slop" defaults (generic-sans brand faces, indigo gradients, 3-card grids, card-in-card, stock-photo 100vh heroes) — and a fresh-context brand-polish checklist. Wired into `gold-standard-website-pipeline` G3 as a fail-closed, structure-only criterion |
...
| [`knowledge/design-md-token-interop.md`](knowledge/design-md-token-interop.md) | Deciding whether a brand/token deliverable should also ship a `DESIGN.md` — Google Labs' alpha spec for handing design tokens + visual-identity rationale to a coding agent in one file. Same W3C DTCG token vocabulary as `design-tokens-scaffolding`; an optional, opt-in export at the `assemble-brand-book` handoff, never a pipeline change. Dated 2026-09-01. |
...
| [`knowledge/web-platform-capabilities-2026.md`](knowledge/web-platform-capabilities-2026.md) | Performance + platform — 2026 Core Web Vitals thresholds (LCP/INP/CLS; per Web Almanac 2025, LCP is most-failed on mobile overall at 62% good, INP is weakest on JS-heavy/high-traffic sites), Speculation Rules API, bfcache, `fetchpriority`, native `<dialog>` (modal) vs Popover (always non-modal), WCAG 2.2 + the EU Accessibility Act. The dated freshness anchor. Owned by `performance-engineer` + `web-architect`. Dated 2026-05-28. |
...
| [`knowledge/design-sources/README.md`](knowledge/design-sources/README.md) | Deciding whether to pull a new pattern/technique from outside RavenClaude, or refreshing what's already been pulled. Dated, sourced notes on 5 named external repos/sites (`astro-frontend-developer-skill`, Vercel's `design.md` post, reactbits.dev, collectui.com, `ui-ux-pro-max-skill`) plus a 12-source broad sweep (Astro ecosystem, component libraries, design-token case studies, motion, accessibility pattern references). The raw-source layer beneath `design-references.md` (whole-site exemplars) and `gold-standard-website-references-2026.md` (agentic tooling exemplars). Dated 2026-09-01. |
...
| [`../ravenclaude-core/skills/design-clone/SKILL.md`](../ravenclaude-core/skills/design-clone/SKILL.md) | Cloning a reference site's design *craft* (spacing/type scales, grid, elevation, component recipes) onto a target's own brand — "make it look like `<site>`" — while structurally blocking the reference's identity (logo / signature color) from the output. "Clone the craft, swap the identity." **Fidelity is browser-gated** (the render→compare `ssim_score` gate); offline it degrades to a structural "declares the same design system" check, and trade dress is not cleared here. The apply-side canon behind each agent's **Design-schema mimicry** section. Owned by core; consumed here by `visual-designer` + `frontend-implementer`. |
...
- **Runnable checker** — [`skills/brand-guidance-authoring/scripts/brand_guidance_lint.py`](skills/brand-guidance-authoring/scripts/brand_guidance_lint.py) (stdlib only, Python 3.8+) makes `gold-standard-website-pipeline` G3's five brand-guidance structural facts mechanical and CI-gateable — file exists, all 7 sections present in order, typeface count ≤ 2, every anti-pattern catalogue row's `override` resolved, zero adjectival rules in the project's own prose. It is a **structural checker, not a taste judge** — a file that passes can still look bad; that judgment stays with `brand-polish-checklist.md`, which stays advisory. `--self-test` proves each of the 5 checks independently trips on a targeted mutation. Owned primarily by `visual-designer`.
...
- **`brand-guidance-authoring` — closing the generation-time brand-guidance gap (2026-09-02).** New 14th skill (4 files: `SKILL.md`, a 10-item `AP-01`–`AP-10` anti-pattern catalogue, a `brand-guidance.md` template, a fresh-context brand-polish checklist) closes the TODO `knowledge/design-sources/vercel-design-md.md` and `knowledge/design-md-token-interop.md` both named — a Vercel-style generation-time brand-voice contract had no owning skill. Wired into `gold-standard-website-pipeline` G3 as **fail-closed on 5 structural facts only, never on taste** (artifact exists · 7 sections present · ≤2 typefaces · every catalogue row's `override` resolved · zero adjectival rules), with the pipeline's existing `N/A — <reason>` discipline as the escape for builds with no real brand surface. The checklist stays advisory at G3 and G5. Built via `/forge` (two divergent cross-model panels, reconciled at synthesis — full record in `.ravenclaude/runs/forge/web-design-taste-system/plan.md`). The one live, deliberately unresolved tension: `AP-01` bans a generic-sans face (Inter included) as a project's *brand-voice* face by default, while `knowledge/design-references.md`'s own Synthesis section still recommends Inter as a general-case display face — both stand, each project resolves it via `AP-01`'s per-row `override`, and neither file is silently edited to match the other (see that file's new "Live tension" subsection). A cross-plugin pointer also landed in `ravenclaude-core/knowledge/visual-feedback-loop.md`'s "weaker on subtle brand polish" caveat, naming this checklist as what that caveat now points at — a self-check, not a new objective stopping signal.
- **`brand-guidance-authoring` follow-ups (2026-09-02), from the FORGE plan's own §7 "named follow-ups" list.** Two of three built; one deliberately declined with the reason recorded rather than forced:
  - **Built — `scripts/brand_guidance_lint.py`**, the plan's own "single highest-value follow-up": a stdlib structural checker for G3's 5 facts, matching the `contrast_ratio.py` precedent (a shipped, runnable, consumer-project tool — not wired into this repo's own `audit-gates.sh`, exactly as `contrast_ratio.py` isn't). Ships a `--self-test` proving each of the 5 checks trips independently on a targeted mutation and does **not** false-positive on the template's own `<...>` placeholder instructional text (which legitimately names banned words as examples of what not to write — this repo's own recurring "source-scan gates match prose" trap, caught by the linter's own first self-test run against its own template).
  - **Built — the `brand-identity-studio` reciprocal cross-reference.** `brand-book-assembly/SKILL.md`'s Delegation line now names `brand-guidance-authoring` as the machine-checkable companion produced at the same handoff point; `vercel-design-md.md`'s TODO for this seam is marked closed.
  - **Declined — a `covers[]` concept-inventory entry for the catalogue.** The plan's follow-up #2 proposed this as "a staleness clock," but on inspection `scripts/concepts.py` is scoped specifically to the Learn-tab / product-inventory system for `ravenclaude-core`'s own mechanisms (every existing `covers[]` entry lives under `ravenclaude-core/knowledge/concepts/` and covers a `ravenclaude-core` artifact) — creating an entry for a `web-design` content catalogue would be a category error, surfacing an aesthetic-taste list in a tab meant to teach how the harness itself works. The real need (this catalogue's facts aging over time) is already covered by the plugin's existing `knowledge-file-staleness-sweep` skill; no new machinery built.
