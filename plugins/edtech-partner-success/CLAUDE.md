...
- **"How's my book looking?"** → `learning-analytics-analyst` (current health-score snapshot) + `edtech-partner-success-manager` (qualitative pulse for the bottom-quartile partners) in parallel; Team Lead synthesizes.
- **"Partner X has gone quiet"** → `edtech-partner-success-manager` (immediate touchpoint draft) + `partner-profile-curator` (re-read the durable record for what we promised and what they cared about) + `success-playbook-designer` (which recovery play applies).
...
- **"Renewal motion for partner X starts next month"** → `success-playbook-designer` (select / refresh the renewal play) → `edtech-partner-success-manager` (execute the touchpoints) → `qbr-composer` (the renewal QBR if applicable).
- **"Onboarding plan for new partner X"** → `partner-profile-curator` (start the durable record) → `edtech-partner-success-manager` (30/60/90 plan) → `learning-analytics-analyst` (which metrics to instrument from day 1).
...
1. **Check available skills first** — `partner-health-scoring`, `health-report-dashboard`, `cs-platform-integration`, `success-plan-authoring`, `qbr-composition`, `rostering-data-quality`, `advocacy-program-design`, `ferpa-comms-translation`, `adoption-sequencing-k12`, `implementation-90-day-arc`, `partner-training-program-design`, `renewal-play-design`, `expansion-play-design`, `recovery-play-design`, `executive-sponsor-mapping`, plus the core PSM skill in `ravenclaude-core`.
...
|---|---|---|
| [`skills/partner-health-scoring/SKILL.md`](skills/partner-health-scoring/SKILL.md) | `learning-analytics-analyst` | Signal selection (adoption depth, usage breadth, sentiment, business outcomes); weighting; **half-life / decay design**; red-flag triggers; threshold-to-play mapping |
| [`skills/health-report-dashboard/SKILL.md`](skills/health-report-dashboard/SKILL.md) | `learning-analytics-analyst`, `edtech-partner-success-manager` | Self-contained **Power-BI/Tableau-style portfolio report** (KPI cards, health-band donut, 12-week trend, peer-cohort range, score-component drill-downs, red-flag surfacing, sortable/filterable per-partner table). Ships a demo (synthetic data) + regenerates from real data via `scripts/generate-bi-report.py` (replace `bi-report/data.json`). The visual companion to the `health-score-dashboard` spec; FERPA: synthetic identifiers only, partner-level never student-level. |
| **v0.5.0** [`skills/cs-platform-integration/SKILL.md`](skills/cs-platform-integration/SKILL.md) | `learning-analytics-analyst`, `edtech-partner-success-manager`, `qbr-composer` | **Back-end integration contract** for a centralized CS dashboard sitting on Planhat (CS platform) + Salesforce (CRM) + Snowflake (warehouse / product usage) + Zapier (glue) + Granola (AI meeting notes — stubbed). Documents which system feeds which `bi-report/data.json` field, the sync-freshness thresholds that drive the source-freshness banner, the SFDC-Account-ID identity spine joining records across systems, and the Granola → Planhat flow as a placeholder until the user defines it. Recommended day-1 implementation order included. Pairs with `health-report-dashboard` (rendering) + `partner-health-scoring` (the score math). |
| [`skills/success-plan-authoring/SKILL.md`](skills/success-plan-authoring/SKILL.md) | `edtech-partner-success-manager` | What a good 30/60/90 / quarterly success plan looks like; measurable success criteria; ownership; cadence |
| [`skills/qbr-composition/SKILL.md`](skills/qbr-composition/SKILL.md) | `qbr-composer` | The QBR playbook — data → narrative → deck → talk track → followups. Includes the no-commitments-no-QBR rule. |
| [`skills/rostering-data-quality/SKILL.md`](skills/rostering-data-quality/SKILL.md) | `learning-analytics-analyst`, `edtech-partner-success-manager` | Diagnosing rostering / SIS / LMS sync issues. K-12 (Clever, ClassLink, OneRoster), higher-ed (SIS / LMS), corp L&D (HRIS sync). When to escalate to product vs. when to coach the partner's admin. |
| **v0.4.2** [`skills/advocacy-program-design/SKILL.md`](skills/advocacy-program-design/SKILL.md) | `success-playbook-designer`, `ferpa-comms-translator`, `edtech-partner-success-manager`, `ravenclaude-core/documentarian` | The 5-tier advocacy ladder (logo → quote → case study → speaker → peer call); health-score eligibility gate (top-quartile only); pipeline-check + topic-match-to-strength flow; state-law overlay; FERPA overlay for student/parent quotes; pre-publication checklist. |
| **v0.12.7** [`skills/ferpa-comms-translation/SKILL.md`](skills/ferpa-comms-translation/SKILL.md) | `ferpa-comms-translator` (primary), `edtech-partner-success-manager`, `qbr-composer` | Audience-reshape + residual screen + multilingual redesign. Same playbook as `/translate-ferpa-safe-comms`; legal matrices stay in knowledge. Case-study quotes stay in `advocacy-program-design`. |
| **v0.4.2** [`skills/adoption-sequencing-k12/SKILL.md`](skills/adoption-sequencing-k12/SKILL.md) | `learning-analytics-analyst`, `success-playbook-designer`, `edtech-partner-success-manager` | Stage 1-4 sequencing rules (newly-implemented / first-year-sustaining / multi-year-mature / pre-renewal); surface-by-surface adoption rhythm; diagnostic-before-intervention discipline. **DO NOT push feature-breadth in stage 1.** |
| **v0.4.2** [`skills/implementation-90-day-arc/SKILL.md`](skills/implementation-90-day-arc/SKILL.md) | `edtech-partner-success-manager`, `learning-analytics-analyst` | The 90-day technical-onboarding arc (discovery weeks 1-2 → setup weeks 3-4 → train-the-trainer weeks 5-6 → go-live + Day-30 measurement weeks 7-8 → stabilization weeks 9-12). Calendar-dead-zone go-live check is the highest-leverage pre-flight check. |
| **v0.4.2** [`skills/partner-training-program-design/SKILL.md`](skills/partner-training-program-design/SKILL.md) | `ravenclaude-core/documentarian`, `edtech-partner-success-manager`, `success-playbook-designer` | Train-the-trainer is the only model that scales in K-12. K-12 PD modality decision rule (live-in-person / live-virtual / hybrid / async-with-followup); PD-credit alignment; teacher-union overlay in unionized states. |
| [`skills/renewal-play-design/SKILL.md`](skills/renewal-play-design/SKILL.md) | `success-playbook-designer`, `edtech-partner-success-manager` | The T-180 → T-0 renewal motion. Sponsor-confirmation arc, value-evidence pack, multi-thread the buying committee, decision-memo support, expand/maintain/contract decision rule, "no surprises" sequence. K-12 / higher-ed / corp L&D segment overlays (K-12 starts at T-180, not T-120). Anti-pattern: renewal pitched at T-7. |
| [`skills/expansion-play-design/SKILL.md`](skills/expansion-play-design/SKILL.md) | `success-playbook-designer` | The 3-gate eligibility model (top-quartile health AND demonstrable adoption AND organizational readiness). Value-trigger taxonomy; seat / module / department / multi-year / tier-upgrade patterns; internal-champion-armament; ROI-storytelling format. The "don't pitch to bottom-quartile" discipline (credibility burn + misallocation). |
| [`skills/recovery-play-design/SKILL.md`](skills/recovery-play-design/SKILL.md) | `success-playbook-designer`, `edtech-partner-success-manager` | Red-flag taxonomy; 4-hypothesis parallel diagnostic (product fit / implementation health / sponsorship / external pressure); time-bound 30/60/90-day signal targets; escalation criteria (PSM → success leadership → exec sponsor → counsel → churn-prep); the "is this recoverable?" rule; graceful exit + reference protection; post-recovery learning capture. |
| [`skills/executive-sponsor-mapping/SKILL.md`](skills/executive-sponsor-mapping/SKILL.md) | `partner-profile-curator`, `edtech-partner-success-manager` | The 6-role taxonomy (economic buyer / champion / technical buyer / user-champion / blocker / executive sponsor) with K-12 / higher-ed / corp L&D title overlays. Multi-thread coverage gap grid; "ghost sponsor" detection pattern; sponsor-change protocols (new arrives / leaves / sidelined); QBR attendance as sponsor-health signal; integration with the durable partner profile. |
...
- **v0.5.0 stack-integration layer** — 1 new knowledge file ([`cs-stack-integration-planhat-sfdc-snowflake.md`](knowledge/cs-stack-integration-planhat-sfdc-snowflake.md)) paired with 1 new skill ([`cs-platform-integration`](skills/cs-platform-integration/SKILL.md)) and 1 new template ([`partner-pickup-brief.md`](templates/partner-pickup-brief.md)). Adds the back-end integration contract (Planhat / Salesforce / Snowflake / Zapier / Granola) for the centralized CS dashboard — field-to-source map, identity spine, sync-freshness thresholds, failure-mode catalog, Granola placeholder. The dashboard rendering itself stays in [`health-report-dashboard`](skills/health-report-dashboard/SKILL.md); this layer is the contract between source systems and the data.json shape.
...
| **2026-06-04** [`knowledge/edtech-enforcement-precedents-2025-2026.md`](knowledge/edtech-enforcement-precedents-2025-2026.md) | Six-theory enforcement taxonomy from the 2025-2026 frontier: Illuminate $5.1M multistate AG (CA KOPIPA + CT student-data + NY § 2-d, Nov 2025), PowerSchool Dec 2024 breach (MTD denied Mar 2026), PowerSchool Naviance $17.25M wiretapping settlement (Feb 2026 preliminary; Jun 10, 2026 final), CDE FERPA finding (Jan 28, 2026 — first written notice of findings in FERPA enforcement process), Edmodo + IXL amicus, NetChoice v. Bonta (CAADCA). Mermaid decision tree + per-theory PSM playbook. **"No breach = no FERPA exposure" is now false** (Naviance pattern). |
| **2026-06-04** [`knowledge/coppa-2025-amendment-edtech-implications.md`](knowledge/coppa-2025-amendment-edtech-implications.md) | The Apr 22 2026 full-compliance deadline (passed). Four new STOP-needs-counsel leaves: biometric collection from <13, government-ID collection from <13, K-5 product without written cybersecurity program, third-party data flow without separate VPC. 10-point operational checklist for K-5 vendor evaluation. |
| **2026-06-04** [`knowledge/essa-evidence-tier-as-renewal-asset.md`](knowledge/essa-evidence-tier-as-renewal-asset.md) | ESSA evidence tiering (Strong / Moderate / Promising / Demonstrates a Rationale) has crossed from vendor marketing to district renewal rubric. Where to look up (WWC + Evidence for ESSA + vendor pages); 90/60/30 positioning; worked example (Lexia Core5's 8-Strong portfolio); what to do if your vendor has no ESSA evidence. |
...
The `learning-analytics-analyst`, `edtech-partner-success-manager`, `ferpa-comms-translator`, `success-playbook-designer`, `partner-profile-curator`, and `qbr-composer` agents carry compact inline priors summarizing relevant files. v0.4.0: `edtech-partner-success-manager` (both new files), `success-playbook-designer` (renewal-pricing primary, AI-feature plays secondary), `partner-profile-curator` (AI posture in profile). v0.4.1: `edtech-partner-success-manager` (operating-cadence touchpoint discipline) + `success-playbook-designer` (play-trigger suppression overlay). v0.4.2: `edtech-partner-success-manager` (5-bundle capability extension consolidated section), `success-playbook-designer` (adoption + advocacy play design), `learning-analytics-analyst` (K-12 adoption arc + SIS/SSO patterns), `ferpa-comms-translator` (advocacy-content state-law + consent overlay).
...
Owned primarily by `learning-analytics-analyst` (`utilization` / `renewal-forecast`); `edtech-partner-success-manager` uses `ttv` for the implementation-arc go-live check. Every benchmark printed is public-context only (`[verify-at-use]`) — never a substitute for the partner's own data (§3 #12).
