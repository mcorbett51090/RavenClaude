...
1. **Expansion attempt on a yellow / red account burns credibility.** The partner reads it as the vendor monetizing too hard during their pain, and the renewal slips.
2. **Expansion attempt without adoption depth produces shelfware.** The partner buys more, doesn't use it, and the next renewal has a worse-not-better story.
...
Per [`partner-health-scoring.md`](../partner-health-scoring/SKILL.md). The composite is in the top quartile *and* none of the red-flag triggers are active. If the score is yellow because a single signal dropped but otherwise stable, that's a yellow — not an expansion candidate.
...
| **New sponsor / champion arrives** | Org change brings in someone with energy to invest | Re-onboard the new sponsor (per [`executive-sponsor-mapping.md`](../executive-sponsor-mapping/SKILL.md)) before any expansion ask |
| **Comparable peer expanded** | Anonymized peer partner did the expansion + got measurable value | Reference-led expansion (loop in [`advocacy-program-design.md`](../advocacy-program-design/SKILL.md)) |
...
- Risk: low if utilization is real; high if it's roster bloat (see [`rostering-data-quality.md`](../rostering-data-quality/SKILL.md))
...
- **Comparable-peer reference** (anonymized, via [`advocacy-program-design.md`](../advocacy-program-design/SKILL.md)) — a peer school / department / team that did the expansion and got the outcome
...
1. **Credibility burn** — the partner reads it as the vendor monetizing during their pain. It puts the renewal at risk for a deal that wouldn't have closed anyway.
2. **Misallocation** — the PSM time spent crafting the ask is time *not* spent on the recovery play. The opportunity cost is the renewal itself.
...
- **Procurement-led RFP risk** at the university-wide tier. Build the incumbent-defense play in parallel (loop in `partner-profile-curator`).
...
- [ ] Gate 1 — top-quartile health confirmed, no active red-flag triggers
- [ ] Gate 2 — load-bearing workflow at 80%+ adoption, sustained 90 days
- [ ] Gate 3 — organizational readiness signal named explicitly
- [ ] A specific value-trigger from the taxonomy fired (not "we should ask")
- [ ] Internal-champion armament pack drafted
- [ ] ROI-storytelling format applied to every outcome claim
- [ ] Comparable-peer reference is segment-matched
- [ ] Implementation realism (people, time, budget) is honestly stated
- [ ] Segment overlay applied
- [ ] Not bundled into a yellow / red renewal

## When NOT to invoke

- The partner is in active recovery. Use [`recovery-play-design.md`](../recovery-play-design/SKILL.md).
...
- [`partner-health-scoring.md`](../partner-health-scoring/SKILL.md) — Gate 1 input
- [`success-plan-authoring.md`](../success-plan-authoring/SKILL.md) — Gate 2 outcome basis
- [`executive-sponsor-mapping.md`](../executive-sponsor-mapping/SKILL.md) — Gate 3 organizational-readiness signals
- [`renewal-play-design.md`](../renewal-play-design/SKILL.md) — expansion is post-renewal; don't bundle
- [`recovery-play-design.md`](../recovery-play-design/SKILL.md) — what to do when expansion isn't the right answer
- [`advocacy-program-design.md`](../advocacy-program-design/SKILL.md) — peer-reference sourcing
- [`rostering-data-quality.md`](../rostering-data-quality/SKILL.md) — seat-utilization signal trust
- [`../knowledge/renewal-pricing-conversations-edtech.md`](../../knowledge/renewal-pricing-conversations-edtech.md) — segment context
- [`../knowledge/edtech-segment-fundamentals.md`](../../knowledge/edtech-segment-fundamentals.md) — segment buying patterns
