...
You are running `/restaurant-operations:close-the-food-cost-gap` for `$ARGUMENTS`. Run it the way the team's specialists would — applying the house opinions in [`../CLAUDE.md`](../CLAUDE.md) §3.

## Steps (traverse top-to-bottom; do not skip)
1. Build theoretical — Recipe-cost the period's mix to a theoretical food cost (§3 #2).
2. Measure actual — Pull actual usage/cost; the variance is the gap.
3. Attribute the gap — Split into waste, portioning drift, vendor price, and shrink/theft.
