...
- **v0.3.0** — gold-standard harden. `knowledge/six-sigma-statistics-and-spc.md` §3a adds the control-chart constants (d2/A2/D3/D4 and c4/A3/B3/B4, n = 2–10, recomputed rather than recalled) so the Xbar-R / Xbar-S recommendation in §3 is executable; `lss_calc.py` gains its first CI coverage (Gate 218, `scripts/check-lss-calc.py` — all four modes against hand-checked values, six arithmetic mutants, exit-2 teeth); the advisory hook joins Gate 30's fires/silent coverage alongside its eleven siblings; and the duplicated operational-definition rule is consolidated into one canonical doc (21 best-practices → **20**, with the duplicate's unique template merged in first, not dropped).
