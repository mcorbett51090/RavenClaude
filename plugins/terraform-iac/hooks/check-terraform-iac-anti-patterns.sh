...
findings=()
if grep -nEi "(access_key|secret_key|password)\\s*=\\s*\\\"[^\\\"]+\\\"" "$scan_target" >/dev/null 2>&1; then
...
if grep -nEi "^\\s*count\\s*=" "$scan_target" >/dev/null 2>&1; then
...
if _rc_pcre_match "$scan_target" "(source\\s*=\\s*\\\"[^\\\"]+\\\"(?![\\s\\S]{0,80}version\\s*=))"; then
  findings+=("Module source without a pinned version nearby — pin module versions for reproducible init.")
fi
if grep -nEi "backend\\s+\\\"local\\\"" "$scan_target" >/dev/null 2>&1; then
