...
> Build command-line tools and TUIs that feel native to the shell — the **CLI sibling** of the app-craft cluster (`backend` / `frontend` / `mobile` / `desktop-app` / `api`). A team for the command surface, the output + exit-code contract, terminal UIs, and distribution.
...
- **skills** — cli-design-and-arg-parsing, output-and-exit-code-contract, tui-design, cli-distribution-and-packaging, shell-completions-and-config.
...
- **best-practices**, **templates**, **commands**, **an advisory hook**, a **scenario bank**, and an **`.lsp.json`** (TypeScript + Rust + Go + Python).
