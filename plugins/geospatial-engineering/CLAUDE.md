...
5. **Distances in degrees are a bug; area in Web Mercator is a bug.** `ST_Distance` on `geometry(…, 4326)` returns degrees — cast to `geography` or reproject. Never measure area/length in EPSG:3857 — it distorts badly away from the equator; measure in `geography` or a projected metric CRS.
6. **Use index-aware operators.** `ST_DWithin` (not `ST_Distance < d`), `&&`, the `<->` KNN operator — or the index does nothing.
7. **CRS hygiene through every pipeline hop.** Assert source SRID, set target SRID, range-validate the result (GDAL/OGR, geocoding, routing). Store one canonical SRID and `ST_Transform` on read — never keep duplicate per-CRS geometry columns.
7a. **Validate geometry at the ingest boundary.** `ST_IsValid` / `ST_MakeValid` at load, plus an extent sanity check — bad geometry fails silently in some `ST_` functions and crashes others far from the cause.
7b. **GeoJSON is `[lon, lat]`** (RFC 7946), WGS84, right-hand winding — the swap is the #1 "wrong place on the map" bug.
...
- Computing **`ST_Area` / `ST_Length` in EPSG:3857** — Web Mercator distorts area/length; measure in `geography` or a projected metric CRS (the hook flags this).
...
- Loading geometry **without validating it** (`ST_IsValid` / `ST_MakeValid`) or without an extent sanity check at the ingest boundary.
- GeoJSON written **`[lat, lon]`** instead of `[lon, lat]` — features land in the wrong hemisphere (the ocean off west Africa).
...
1. **Check the 4 skills** (`design-postgis-schema`, `write-spatial-query`, `spatial-data-quality`, `serve-vector-tiles`) plus core skills.
...
| `ST_Area`/`ST_Length` computed in EPSG:3857 (Web Mercator) | spatial files | house opinion #5 / never-compute-area-in-web-mercator |
...
| [`skills/spatial-data-quality/SKILL.md`](skills/spatial-data-quality/SKILL.md) | `geospatial-data-engineer` | Validity (`ST_IsValid`/`ST_MakeValid`), SRID + extent sanity, coordinate-order checks, validate-at-load |
...
| [`knowledge/storage-and-serving-decision-trees.md`](knowledge/storage-and-serving-decision-trees.md) | Choosing **where spatial data lives** (PostGIS vs geospatial warehouse vs tile/feature service) and **how to serve a layer** (GeoJSON vs on-the-fly MVT vs pre-rendered pyramid vs raster) — two Mermaid trees complementing the projection tree |
| [`knowledge/geospatial-stack-2026.md`](knowledge/geospatial-stack-2026.md) | Recommending tooling — Tier-1/Tier-2 across storage (PostGIS + warehouses), pipelines (GDAL/PROJ/Shapely), serving (pg_tileserv/Martin/Tegola/tippecanoe/TiTiler), visualization (MapLibre), routing (OSRM/Valhalla), formats + standards bodies, with retrieval dates |
...
| [`templates/crs-decision.md`](templates/crs-decision.md) | A standalone CRS decision record — one per layer/dataset where the coordinate-system call needs writing down on its own (lighter than the full data-model template) |
...
Named, citable single-rule docs ([`best-practices/README.md`](best-practices/README.md)) — 9 rules: [`always-store-an-srid.md`](best-practices/always-store-an-srid.md), [`reproject-on-read-store-once.md`](best-practices/reproject-on-read-store-once.md), [`never-compute-area-in-web-mercator.md`](best-practices/never-compute-area-in-web-mercator.md), [`index-geometry-with-gist.md`](best-practices/index-geometry-with-gist.md), [`validate-geometry-at-load.md`](best-practices/validate-geometry-at-load.md), [`geojson-is-lon-lat.md`](best-practices/geojson-is-lon-lat.md), [`vector-tiles-over-geojson-at-scale.md`](best-practices/vector-tiles-over-geojson-at-scale.md), [`tile-large-layers-dont-ship-giant-geojson.md`](best-practices/tile-large-layers-dont-ship-giant-geojson.md), [`simplify-per-zoom.md`](best-practices/simplify-per-zoom.md). Each takes one house opinion and makes it a standalone, exception-documented rule.
