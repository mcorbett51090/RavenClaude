...
5. **Enforce CRUD/FLS** for any user-context access — `WITH USER_MODE` (preferred) or `Security.stripInaccessible`. **At API v67.0+ (Summer '26), `WITH SECURITY_ENFORCED` is removed and no longer compiles** — use `WITH USER_MODE` (it also supports polymorphic fields and returns the full set of access errors). Note that at v67.0+ database operations default to **user mode** and classes without a sharing keyword default to **`with sharing`**; on older API versions `WITH SECURITY_ENFORCED` still compiles. `[verify-at-build]` Treat FLS as a security control, and **escalate the security verdict to `ravenclaude-core/security-reviewer`.**
