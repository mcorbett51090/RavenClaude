...
Research-sweep **addition** (Tier-A weekly news sweep; the fan-out sibling of `claude-app-engineering` 0.9.8) — **Claude Sonnet 5 went GA in GitHub Copilot on 2026-06-30.** Updated `knowledge/cross-tool-model-lineup-2026.md`: added the Sonnet 5 Copilot changelog citation to the Sources line, inserted **Sonnet 5 (GA in Copilot 2026-06-30, gradual rollout)** into the Copilot coding-agent snapshot row (ahead of Sonnet 4.6, which coexists during the rollout), added a **2026-07-07 sweep note** (GitHub Changelog primary; picker rollout to Pro/Pro+/Max/Business/Enterprise under ZDR; corroborated via WebSearch as github.blog 403s automated fetch), and re-dated the header to 2026-07-07. Per the tiebreak panel, the vendor-neutral decision tree's **`Sonnet-class` balanced-default anchor was deliberately left as a class label** (not swapped to a SKU — it does not churn with the picker, per the file's methodology-over-SKU philosophy), and **no Claude specs/pricing were imported** here (single-source-of-truth stays in the claude-app capability map; house opinion #8). Citation gate (`scripts/check-lineup-citations.py`) green. Panels: usefulness → USEFUL 3/3; detailed review → REJECT-as-premised (unverified Copilot surface + SKU-swap concerns); **tiebreak panel → PASS** after this session verified Sonnet 5 IS GA in Copilot (GitHub Changelog 2026-06-30) and re-scoped the edit to the dated Copilot picker row + sweep note only. **Migration:** none — knowledge-file content only.
