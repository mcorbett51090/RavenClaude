...
Research-sweep **correction** (Tier-A weekly news sweep) — **OpenAI GPT-5.6 Sol/Terra/Luna went PUBLIC GA on 2026-07-09**, so the 0.3.10/0.3.11 "limited preview / launch announced" framing in `knowledge/cross-tool-model-lineup-2026.md` had become a factual defect (a "preview" tag on a GA model, and GPT-5.6 absent from the Copilot section). Updated: the three Codex GPT-5.6 rows moved **preview → GA** with pricing added (Sol ~$5/$30, Terra ~$2.50/$15, Luna ~$1/$6 per Mtok — **tech-press-secondary, openai.com 403s automated fetch; carried with `[verify-at-use — price secondary-sourced]`, not vendor-confirmed**); a new **GitHub Copilot GPT-5.6 bullet** (GA 2026-07-09, admin-gated off by default, scoped by surface+plan+date per house opinion #3 — Sol on Pro+/Max/Business/Enterprise, Terra/Luna on Pro/Pro+/Max/Business/Enterprise) + the Copilot coding-agent snapshot row; the Copilot **CLI** task-based auto model selection (2026-07-01); the **Codex-into-ChatGPT-desktop-app merge** (2026-07-09) noted as a surface change; the **GPT-5.5 row re-scoped** to point at GPT-5.6 Terra/Sol as the newer default; header re-dated to 2026-07-11 + a **2026-07-11 sweep note**. Mermaid decision-tree leaves intentionally left at GPT-5.5 (framework-level, not churned) with an explicit reconciliation note. Closed-world: GPT-5.6 real, GPT-5.7 remains the fictional example. Citation gate (`scripts/check-lineup-citations.py`) green. **Panels:** usefulness → USEFUL 2/2 (0.95, 0.82); detailed review → APPROVE_WITH_CHANGES 2/2 (0.85, 0.72) — no disagreement, no tiebreak needed; all required changes applied (header past-tense recast, tree/table reconciliation, Copilot snapshot row). Sources: [OpenAI GPT-5.6](https://openai.com/index/gpt-5-6/), [GitHub Copilot GPT-5.6 changelog 2026-07-09](https://github.blog/changelog/2026-07-09-openais-gpt-5-6-sol-terra-and-luna-are-now-available-in-github-copilot/), [TechCrunch 2026-07-09](https://techcrunch.com/2026/07/09/openai-launches-its-new-family-of-models-with-gpt-5-6/). **Migration:** none — knowledge-file content only.

## [0.3.11] — 2026-07-08

Research-sweep **addition** (Tier-A weekly news sweep) — **Claude Sonnet 5 went GA in GitHub Copilot on 2026-06-30**, a gap the same-day 0.3.10 sweep of this file left open (it added GPT-5.6 / Kimi K2.7 / Copilot-Auto but not Sonnet 5, and the sibling `claude-app-engineering` 0.9.10 added Sonnet 5 only to the Claude capability map). Updated `knowledge/cross-tool-model-lineup-2026.md`: added the Sonnet 5 Copilot changelog citation to the Sources line, inserted **Sonnet 5 (GA in Copilot 2026-06-30, gradual rollout)** into the Copilot coding-agent snapshot row (ahead of Sonnet 4.6, which coexists during the rollout), added a **2026-07-08 Sonnet-5-in-Copilot sweep note** (GitHub Changelog primary; rollout to Pro/Pro+/Max/Business/Enterprise under ZDR; corroborated via WebSearch as github.blog 403s automated fetch), and noted it in the header. The vendor-neutral decision tree's **`Sonnet-class` balanced-default anchor was deliberately left as a class label** (not swapped to a SKU), and **no Claude specs/pricing were imported** (single-source-of-truth stays in the claude-app capability map; house opinion #8). Citation gate (`scripts/check-lineup-citations.py`) green. Panels (from PR #599, before rebase): usefulness → USEFUL 3/3; detailed review → REJECT-as-premised (unverified Copilot surface); **tiebreak → PASS** after this session verified Sonnet 5 IS GA in Copilot (GitHub Changelog 2026-06-30). Rebased onto main after the concurrent 2026-07-08 sweeps landed — the redundant claude-app-map half of the original PR was dropped as superseded; only this net-new ai-coding addition remains. (0.3.10 was the 2026-07-08 GPT-5.6 Sol/Terra/Luna + Kimi K2.7 + Copilot-Enterprise-Auto sweep.) **Migration:** none — knowledge-file content only.
