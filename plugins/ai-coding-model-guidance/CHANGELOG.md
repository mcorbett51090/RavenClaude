...
Research-sweep **update** (scheduled news routine) — **Claude Sonnet 5 went GA in GitHub Copilot 2026-06-30** ([GitHub Changelog](https://github.blog/changelog/2026-06-30-claude-sonnet-5-is-generally-available-for-github-copilot/)). Added Sonnet 5 to the Copilot coding-agent row in `knowledge/cross-tool-model-lineup-2026.md` as the current Sonnet-class Claude option `[verify-at-use]` (retrieved 2026-07-06), re-dated the header to 2026-07-06, and added a 2026-07-06 sweep note. Sonnet 5's capability/right-sizing stays in the sibling `claude-app-engineering` (this plugin's non-Claude lane); this file tracks only its **Copilot-picker availability** — a scoping split resolved by a tiebreak panel (the row already lists Claude SKUs as Copilot-availability facts, so it is in-scope). Also flagged, **not yet folded in**: **Kimi K2.7 Code**, Copilot's first selectable open-weight model — a cheapest-fast-tier candidate for next sweep. Panels: usefulness → USEFUL; detailed review → APPROVE; tiebreak (Copilot-row scope) → UPDATE + bump. **Migration:** none — knowledge-file content only.
