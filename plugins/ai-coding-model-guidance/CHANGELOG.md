...
Research-sweep **addition** (Tier-A weekly news sweep; the fan-out sibling of `claude-app-engineering` 0.9.8) — **Claude Sonnet 5 (`claude-sonnet-5`) is GA in GitHub Copilot as of 2026-06-30** ([Changelog 2026-06-30](https://github.blog/changelog/2026-06-30-claude-sonnet-5-is-generally-available-for-github-copilot/)): Copilot Pro / Pro+ / Max / Business / Enterprise, **selectable in the model picker with a gradual rollout** (may not appear on a given tenant yet), runs under ZDR. Updated `knowledge/cross-tool-model-lineup-2026.md`: added Sonnet 5 to the **coding-agent** picker row, a dedicated **Sonnet 5 Copilot bullet**, the header `Last reviewed` (→ 2026-07-05) + sources, and a **2026-07-05 sweep note**. Per house opinion #8 this plugin covers Claude models **only as they appear in the Copilot picker** — Copilot availability is verified against the changelog (403 automated fetch → `WebSearch`), while the model-level facts (id, $3/$15 + intro $2/$10-through-2026-08-31, ~+30%-tokenizer, adaptive-thinking-default) are grounded on the first-party [Sonnet 5 doc](https://platform.claude.com/docs/en/about-claude/models/whats-new-sonnet-5) that fetched cleanly 2026-07-05; the authoritative Claude lineup stays in `claude-app-engineering`'s capability map. Every added claim carries a date + citation + `[verify-at-use]` per the `check-lineup-citations.py` gate. Panels: usefulness → **USEFUL**; detailed review → **APPROVE-WITH-FIX** (fixes applied). **Migration:** none — knowledge-file content only.
