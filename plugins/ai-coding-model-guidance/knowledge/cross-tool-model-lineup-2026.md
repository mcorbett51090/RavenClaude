...
**Last reviewed:** 2026-06-14 · **Confidence:** medium — these three vendors ship **weekly-to-monthly**, faster than the Claude platform; treat this as the freshness anchor the Researcher sweep re-dates. Every numeric / availability claim carries a retrieval date — **verify against the cited primary source before quoting it to anyone.**
...
- **Claude Fable 5 is GA in Copilot** (rolling out from 2026-06-09; [Changelog 2026-06-09](https://github.blog/changelog/2026-06-09-claude-fable-5-is-generally-available-for-github-copilot/)) — Anthropic's first public **"Mythos-class"** model (vendor framing, `[verify-at-use — single-source]`), built for long-horizon autonomous coding. Plans **Pro+ / Max / Business / Enterprise**; surfaces VS Code, Visual Studio, JetBrains, Xcode, Eclipse, the Copilot CLI, the cloud agent, github.com, and mobile. **Included free through 2026-06-22**, then usage-credit billing — `[verify-at-use — dated window expires 2026-06-22]`. **Business/Enterprise admins must enable the Fable 5 policy (off by default)** — same pattern as the Opus 4.8 policy. Also on **AWS Bedrock** and the Anthropic API (`claude-fable-5`, **$10 / $50** per Mtok — **2× Opus 4.8** [$5/$25]); Microsoft Foundry: the authoritative Foundry _Learn_ model table ([`claude-models`](https://learn.microsoft.com/azure/foundry/foundry-models/concepts/claude-models)) **now lists `claude-fable-5` (preview)** — **re-verified 2026-06-14 via the Microsoft-Learn MCP** (it appears in the model-family, comparison, quota, capability, and region tables; Global Standard deployment, regions East US 2 / Sweden Central). Real quota is **Enterprise/MCA-E-only today** (pay-as-you-go and Free-Trial RPM/TPM both **0/0**; Enterprise/MCA-E 2,000 / 2,000,000), and the whole Foundry Claude surface is still **preview**. *(The Azure blog + catalog page announced GA weeks earlier; the Learn deployable-model table lagged until ~2026-06-14 — the earlier 2026-06-10/06-11 sweeps correctly found it absent, and that gap has now closed.)* `[verify-at-use]` The Copilot changelog 403s automated fetch — read via `WebSearch` + cross-referenced across the GitHub/Microsoft posts + dev guides (the route ladder, below). `[verify-at-use — surface/plan-specific; pricing/inclusion window]`
...
>
> **Re-verify note (2026-06-14):** the **Microsoft Foundry Fable 5** sub-claim **flipped** — the authoritative `learn.microsoft.com` Foundry Claude-model table **now lists `claude-fable-5` (preview)** (model-family, comparison, quota, capability, and region tables; Global Standard, East US 2 / Sweden Central; pay-go + Free-Trial quota still 0/0, so Enterprise/MCA-E-tier in practice). Re-verified via the Microsoft-Learn MCP; the Copilot bullet above is updated. This sweep also spot-checked the Copilot/Anthropic primaries (Fable 5 GA in Copilot, usage-based billing live 2026-06-01) — **no other net-new** for this file.
>
> **[SUPERSEDED 2026-06-14] Re-verify note (2026-06-11):** _(kept for provenance)_ the Foundry Learn table at that time **did not** list `claude-fable-5` (it listed Opus 4-8/4-7/4-6/4-5/4-1, Sonnet 4-6/4-5, Haiku 4-5 previews + the gated `claude-mythos-preview`) despite the Azure blog + `ai.azure.com` catalog page announcing GA — a worked example that a vendor blog/catalog announcement can lead the deployable-model-table listing by weeks. That gap closed on/around 2026-06-14 (see the note above).
>
