...
findings=()
if grep -nEi "image:\\s*\\S+:latest|image:\\s*[^@\\s]+\\s*$" "$scan_target" >/dev/null 2>&1; then
  findings+=("Container image not pinned by digest (':latest' or floating tag) — pin by digest for reproducibility.")
fi
if grep -nEi "runAsNonRoot:\\s*false|privileged:\\s*true|runAsUser:\\s*0\\b" "$scan_target" >/dev/null 2>&1; then
  findings+=("Privileged/root container — run as non-root and unprivileged; drop capabilities.")
fi
if _rc_pcre_match "$scan_target" "kind:\\s*Deployment(?![\\s\\S]*resources:)"; then
  findings+=("Deployment without resources block nearby — set requests AND limits (scheduling + cap).")
fi
if _rc_pcre_match "$scan_target" "kind:\\s*(Deployment|StatefulSet)(?![\\s\\S]*readinessProbe)"; then
  findings+=("Workload without a readinessProbe nearby — readiness gates traffic; add it.")
fi
if _rc_pcre_match "$scan_target" "clusterrolebinding[\\s\\S]*cluster-admin"; then
