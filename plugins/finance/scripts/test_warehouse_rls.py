...
SYNTH = {
    ...
    "statements": {
        ...
        "reasoning_trail": {
            "income_statement": [
                {"account": "4000", "line": "Product revenue",
                 "section": "Revenue", "amount": 1000000.0},
                {"account": "5000", "line": "Cost of goods sold",
                 "section": "COGS", "amount": 400000.0},
            ],
            "balance_sheet": [
...
def test_close_package_to_rows():
    ...
    check("emits all 7 fact/dim tables",
          set(tables) == {"dim_entity", "dim_period", "fct_close_statement_line",
                          "fct_recon_exception", "fct_flux_movement",
                          "fct_close_kpi", "fct_close_state"})

    de = tables["dim_entity"]
    check("dim_entity has exactly one row", len(de) == 1)
    eid = de[0]["entity_id"]
    check("entity_id is UUID-shaped (8-4-4-4-12)",
          len(eid) == 36 and eid.count("-") == 4)
    check("entity_id is deterministic (same name -> same id)",
          eid == cpr.entity_uuid("Nimbus Widgets LLC (FICTITIOUS)"))
    check("--entity-id override wins over the derived id",
          cpr.to_rows(SYNTH, entity_id=ENT_A)["dim_entity"][0]["entity_id"] == ENT_A)

    dp = tables["dim_period"]
    check("dim_period carries the 30-day count for 2026-06",
          dp[0]["day_count"] == 30 and dp[0]["fiscal_year"] == 2026 and dp[0]["fiscal_month"] == 6)

    sl = tables["fct_close_statement_line"]
    check("statement lines are stamped with entity_id + period (grain)",
    ...
    check("statement line amounts flow through presentation-signed (AP=200000)",
          any(r["line"] == "Accounts payable" and r["amount"] == 200000.0 for r in cl_lines))
    check("IS + BS lines both present in the fact",
          any(r["statement"] == "IS" for r in sl) and any(r["statement"] == "BS" for r in sl))

    re_rows = tables["fct_recon_exception"]
    check("recon fact carries all 3 accounts with their status",
          len(re_rows) == 3 and {r["status"] for r in re_rows} ==
          {"PASS", "FLAG", "self-supported"})
    check("recon fact preserves the FLAG break (35000 difference)",
    ...
    check("close-state fact carries state + traceability badge + self_certified",
...
def test_kpi_parity():
    ...
    check("every derive_kpis key is present as a fact row",
          set(reconstructed) == set(golden))
    check("fct_close_kpi values == derive_kpis VERBATIM (KPI parity)",
    ...
    check("golden current ratio = 2.0 (CA 500000 / CL 250000 from trail)",
          golden["current_ratio"] == 2.0)
    check("golden DSO = 3.6 (AR 120000 / rev 1000000 * 30 days)",
          golden["dso_days"] == 3.6)
    check("is_na flag is False for a derived KPI",
    ...
    check("current_ratio lands as value=None / is_na=True with no trail (n/a, not plugged)",
          cr["value"] is None and cr["is_na"] is True)
    check("n/a parity: rows2 still equals derive_kpis(no_trail)",
...
def test_entity_rls_resolve():
    ...
    check("empty-object {} grant (not a list) denies all -> []",
          rls.resolve([ENT_A], {}) == [])
    # TYPE GUARD (fail closed): a bare STRING must NOT iterate char-by-char into a grant.
    check("string 'allowed' claim -> [] (no char-by-char grant fabrication)",
          rls.resolve([ENT_A], "ABC") == [])
    check("string 'requested' -> [] (fail closed, no char explosion)",
          rls.resolve("ABC", [ENT_A, ENT_B]) == [])
    check("dict grant {'entity': ...} -> [] (not a collection of ids)",
...
def test_entity_rls_bind_split_brain():
    ...
    ident = ci.VerifiedIdentity(subject=f"user-1@{iss}", email=None, roles=frozenset(),
                                issuer=iss, verified=True, token_fingerprint="deadbeef00000000")
    good_claims = {"iss": iss, "sub": "user-1", "allowed_entities": [ENT_A, ENT_B]}
    granted, reason = rls.bind_entitlement_to_identity(good_claims, ident)
    check("matched iss+sub -> entitlement bound, allowed_entities returned",
          granted == [ENT_A, ENT_B] and reason.startswith("ok"))

    other_iss = {"iss": "https://evil.example.com/", "sub": "user-1", "allowed_entities": [ENT_A]}
    g, r = rls.bind_entitlement_to_identity(other_iss, ident)
    check("different issuer -> [] with split-brain reason", g == [] and "split-brain" in r)

    other_sub = {"iss": iss, "sub": "user-2", "allowed_entities": [ENT_A]}
    g, r = rls.bind_entitlement_to_identity(other_sub, ident)
    check("same issuer, DIFFERENT sub -> [] with split-brain reason", g == [] and "split-brain" in r)

    unver = ci.VerifiedIdentity(subject="config:carol", email=None, roles=frozenset(),
                                issuer="config-asserted", verified=False, token_fingerprint="n/a")
    g, r = rls.bind_entitlement_to_identity(good_claims, unver)
    check("UNVERIFIED identity -> [] (entitlement cannot bind to an unverified identity)",
          g == [] and "unverified" in r)

    empty_grant = {"iss": iss, "sub": "user-1", "allowed_entities": []}
    g, r = rls.bind_entitlement_to_identity(empty_grant, ident)
...
def test_entity_rls_claim_envelope():
    ...
    check("MISSING allowed_entities claim -> deny-all ([])",
          g == [] and "missing" in r)
    ...
    over_ttl = {"allowed_entities": [ENT_A], "iat": now - 60,
                "exp": now - 60 + (31 * 60)}  # 31-minute lifetime
    ...
    check("not-yet-valid token (now < iat) -> deny-all ([])",
          g == [] and "not yet valid" in r)
...
def test_reference_models_invariants():
    ...
    if os.path.exists(rls_sql_path):
        ...
        check("RLS FORCEs row level security (owner is not exempt)",
              "FORCEROWLEVELSECURITY" in flat)
        check("RLS predicate uses ANY(current_setting('app.entity_ids')::uuid[])",
    ...
    if os.path.exists(cube_path):
        ...
        check("Cube scopes entity_id to securityContext.allowed_entities",
              "securityContext.allowed_entities" in cube and "access_policy" in cube)
        check("Cube pins operator: in for explicit array-claim IN semantics",
              "operator: in" in cube)
        check("partner-facing view carries its OWN access_policy (>=3: 2 cubes + view)",
              cube.count("access_policy") >= 3)
        check("view filters portfolio_close.entity_id (belt-and-suspenders on the VIEW)",
