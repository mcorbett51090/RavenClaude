...
| [`skills/multi-currency-translation/SKILL.md`](skills/multi-currency-translation/SKILL.md) | `controller` | **Autopilot · live tier.** ASC 830 / IAS 21 remeasurement (temporal) + translation (current-rate) with a CTA plug / remeasurement G/L via `scripts/remeasure.py`; per-account `rate_class`, CTA self-check, hyperinflation refusal; a translation pre-stage to `consolidate.py`. Decision-support, not an audited remeasurement |
| [`skills/live-connectors/SKILL.md`](skills/live-connectors/SKILL.md) | `controller` | **Autopilot · live tier.** OAuth2 GL extractor reference impl (QBO/NetSuite/Sage Intacct/Xero) in `scripts/connectors/` — atomic rotating-refresh, per-entity lock, error-cause routing, record/replay harness — + drill-through GL lineage feeding `statement_engine --gl-detail`. Reference impl + mock harness; NOT live-verified |
| [`skills/warehouse-dashboard/SKILL.md`](skills/warehouse-dashboard/SKILL.md) | `controller`, `board-pack-composer` | **Autopilot · live tier.** Warehouse-backed multi-tenant dashboard: a close-package→fact/dim semantic model, entity-level array-claim RLS (`scripts/entity_rls.py` + `close_package_to_rows.py`), reusing `data-platform`'s RLS/JWT/Cube/dbt. Isolation gated on Postgres FORCE-RLS + Cube denial tests (specified, not executed) |
| [`skills/idp-segregation/SKILL.md`](skills/idp-segregation/SKILL.md) | `controller`, `audit-prep-specialist` | **Autopilot · live tier.** Evolves `close_state.py`'s config-asserted `--actor` into an IdP-verified identity via `scripts/close_identity.py` — OIDC claim + signature validation (HS256 dev/fixture; RS256/JWKS via optional PyJWT, refuse-loudly), token-level preparer≠approver check, step-up-to-LOCK. Tamper-evidence, not auditor-grade (consumer WORM/IdP required) |
| [`skills/netsuite-close/SKILL.md`](skills/netsuite-close/SKILL.md) | `controller` | **Autopilot · NetSuite (gold-standard).** Layperson-wireable NetSuite source: OAuth2 M2M (cert-signed JWT via an injected signer seam, no refresh token) + time-boxed TBA fallback; a **BS-cumulative / IS-period** SuiteQL trial balance (`scripts/connectors/suiteql.py`) that ties out; a COA-draft generator + plain-English `netsuite_doctor.py` triage; `close_state.py` **changed-after-sign-off** drift detection; one-command front door `scripts/connectors/netsuite_connect.py` (`--replay` offline). Reference-impl + offline harness; landscape in `knowledge/netsuite-integration-landscape.md` |
...
- **v0.17.0** (2026-07-06) — **controller-autopilot live-integration tier** (FORGE `fca-live-integration-tier`, all four workstreams parallel-built, adversarially reviewed, security-reviewed). Adds 4 skills (`multi-currency-translation`, `live-connectors`, `warehouse-dashboard`, `idp-segregation` → 22 total) + engines `remeasure.py` (ASC 830/IAS 21 remeasurement/translation + CTA, goldens independently reconfirmed by the FORGE critic: current-rate CTA +200, temporal loss −80), `scripts/connectors/` (OAuth2 GL extractor reference impl + record/replay harness + drill-through lineage), `entity_rls.py` + `close_package_to_rows.py` (warehouse semantic model + array-claim RLS reusing data-platform), `close_identity.py` (OIDC identity + token-level SoD evolving `close_state.py`). **Honest scope:** W1 multi-currency is fully-tested real logic; W2/W3/W4 are **assumption-complete reference implementations with mock/replay harnesses + consumer first-light checklists — NOT live-verified** (no creds/warehouse/IdP in this env). A mandatory **security-reviewer** pass fixed a 🔴 Cube-view RLS-omission + hardened the crypto/step-up/split-brain binding before merge. Consolidated suite: **11 test files, 272 acceptance tests, all green**; ruff-clean; stdlib-first (optional PyJWT refuse-loudly). Next (genuinely consumer-side): live OAuth tenants, warehouse stand-up, IdP wiring.
