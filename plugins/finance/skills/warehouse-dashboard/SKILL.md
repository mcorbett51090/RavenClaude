...
- **SPLIT-BRAIN (read with `idp-segregation`).** The RLS `allowed_entities[]` array claim (entity *entitlement*) and the close's *SoD identity* MUST come from **one verified token / issuer**. Two separate tokens let a viewer hold entitlement as one principal while the close is approved under another. Bind them with `entity_rls.bind_entitlement_to_identity(entity_claims, identity, now)`, which fails closed unless the entitlement token's `iss` + `sub@iss` match the verified `VerifiedIdentity`.
