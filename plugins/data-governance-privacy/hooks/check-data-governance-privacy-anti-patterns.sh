...
findings=()
if grep -nEi "(ssn|social_security|national_id|passport|credit_card|card_number|dob|date_of_birth)\\b" "$scan_target" >/dev/null 2>&1; then
  findings+=("Possible PII field — ensure it's classified, minimized (do you need it?), access-controlled, and in DSR/retention scope.")
fi
if _rc_pcre_match "$scan_target" "(anonymi[sz]ed)(?![\\s\\S]{0,80}(k-anonymity|aggregat|irreversible))"; then
...
if grep -nEi "(retention|ttl|delete_after)\\s*[=:]\\s*(never|none|0|indefinite|-1)" "$scan_target" >/dev/null 2>&1; then
...
if grep -nEi "(select\\s+\\*\\s+from\\s+\\w*(user|customer|person|patient))" "$scan_target" >/dev/null 2>&1; then
