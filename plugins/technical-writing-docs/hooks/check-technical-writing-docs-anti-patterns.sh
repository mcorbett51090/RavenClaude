...
findings=()
if grep -nEi "(TODO|TBD|FIXME|coming soon|lorem ipsum|XXX)" "$scan_target" >/dev/null 2>&1; then
...
if _rc_pcre_match "$scan_target" "(localhost:[0-9]+|127\\.0\\.0\\.1|YOUR_API_KEY|example\\.com/api)(?![\\s\\S]{0,40}#\\s*example)"; then
...
if grep -nEi "\\b(click here|read more|this link)\\b" "$scan_target" >/dev/null 2>&1; then
