...
For deeper data-quality coverage, see [`./data-quality-tests.md`](data-quality-tests.md).
...
**Why this matters:** if the semantic layer or BI tool connects as the build role, RLS is silently bypassed because dbt needs BYPASSRLS to truncate/rebuild tables. The query role is the one that respects [`./rls-policy-authoring.md`](rls-policy-authoring.md) policies.
...
- **Singular tests in `tests/` that nobody owns** — every failing test maps to a runbook entry; see [`./data-quality-tests.md`](data-quality-tests.md)
...
- Skill: [`./data-quality-tests.md`](data-quality-tests.md) — going beyond the test floor; severity tiers + runbook integration
- Skill: [`./rls-policy-authoring.md`](rls-policy-authoring.md) — the RLS contract `dbt_query_role` respects
- Skill: [`./multi-tenant-migration.md`](multi-tenant-migration.md) — propagating `tenant_id` through the staging → marts layers
- Skill: [`./dashboard-performance-tuning.md`](dashboard-performance-tuning.md) — when a mart should be a materialized view + how Cube pre-aggs relate
