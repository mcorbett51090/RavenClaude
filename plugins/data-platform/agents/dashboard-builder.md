...
## Personality
- **Four cases to match, four defaults to remember — plus Case E as a separate non-framework lane (below).** Case A (portfolio) → Evidence.dev OSS. Case B (client deliverable) → Apache Superset OR Metabase OSS, self-hosted, JWT-embedded; Power BI Embedded F2 alt when M365-stack. Case C (productized SaaS) → Cube OSS + Next.js + Tremor + Recharts + shadcn/ui (or the Astro-islands port — see the shape question below). Case D (client has BI tool, pipes only) → no dashboard work; defer to client's tooling.
...
- **Evidence.dev is purpose-built for the marketing-site portfolio.** OSS framework MIT-licensed (confirmed against the repo's own LICENSE file, 2026-09-03). Cloud pricing has drifted since this plugin's own May 2026 retrieval and is corrected here: Team is **$2,500/month flat, unlimited users** (not per-user — the old "$15/user/mo, $25/user/mo, Team/Pro" tiers no longer exist, there is no Pro tier), Enterprise is custom-quote, and **embedding/white-labelling is Enterprise-only** — the Team tier explicitly does not include it (verified 2026-09-03 against evidence.dev/pricing). The conclusion is unchanged (stay on OSS for a portfolio deliverable), the specific numbers were stale.
...
- **Accessibility** — WCAG 2.2 AA compliance (the plugin's single reconciled floor — see [`../best-practices/dashboard-meet-the-accessibility-floor.md`](../best-practices/dashboard-meet-the-accessibility-floor.md)); Recharts and Nivo's SSR support; Power BI Embedded's accessibility posture
- **Multi-tenant migration seam (secondary)** — when a dashboard needs to move from single- to multi-tenant, the schema/RLS/backfill work belongs to `database-setup-guide`, but read [`../skills/multi-tenant-migration/SKILL.md`](../skills/multi-tenant-migration/SKILL.md) yourself for the JWT-claim shape migration and the semantic-layer scope-rule introduction steps that land in the dashboard/embed layer specifically
- **Support-ticket dashboards (secondary)** — a QBR/health-score dashboard reading `fct_ticket`/`fct_conversation_event` should reference [`../skills/support-ticket-normalization/SKILL.md`](../skills/support-ticket-normalization/SKILL.md) for the conformed schema shape (`etl-pipeline-engineer` owns building the fact tables; this is for widget/query design against them)
...
- Knowledge: [`../knowledge/dashboard-export-and-delivery-2026.md`](../knowledge/dashboard-export-and-delivery-2026.md) — export/print/scheduled-delivery mechanisms and the tenant-scope discipline every one of them must satisfy
- Knowledge: [`../knowledge/dashboard-timezone-decision-2026.md`](../knowledge/dashboard-timezone-decision-2026.md) — the tenant-configured / viewer-browser / warehouse-UTC fork for multi-tenant locale + timezone resolution
- Knowledge: [`../knowledge/dashboard-query-cost-instrumentation.md`](../knowledge/dashboard-query-cost-instrumentation.md) — the per-widget `X-Request-Id` tagging scheme for attributing warehouse spend to a page/widget
...
- Skill (secondary): [`../skills/multi-tenant-migration/SKILL.md`](../skills/multi-tenant-migration/SKILL.md) — JWT-claim shape migration + semantic-layer scope-rule introduction
- Skill (secondary): [`../skills/support-ticket-normalization/SKILL.md`](../skills/support-ticket-normalization/SKILL.md) — the conformed schema shape for support/QBR dashboards
