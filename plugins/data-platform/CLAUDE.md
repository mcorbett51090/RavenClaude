...
1. **Check available skills first** — the 12 skills in this plugin (`stack-selection`, `cloud-database-comparison`, `connector-configuration`, `jwt-embed-issuance`, `rls-policy-authoring`, `cube-schema-scaffolding`, `embed-csp-and-iframe-sandboxing`, `dbt-project-scaffolding`, `dashboard-performance-tuning`, `multi-tenant-migration`, `data-quality-tests`, `cross-system-identity-resolution`) plus the core skills (`structured-output`, `grounding-protocol`, etc.).
