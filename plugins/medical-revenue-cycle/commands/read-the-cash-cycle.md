...
You are running `/medical-revenue-cycle:read-the-cash-cycle` for `$ARGUMENTS`. Run it the way the team's specialists would — applying the house opinions in [`../CLAUDE.md`](../CLAUDE.md) §3.

## Steps (traverse top-to-bottom; do not skip)
1. Compute net collection rate — Collected ÷ allowed (not gross); target 95–98%+ (§3 #4).
2. Compute first-pass resolution — Claims paid without rework; benchmark ~90%, high performers 95–98% (§3 #2).
3. Read days-in-A/R by bucket — Days-in-A/R under 30 is high-performing; read by aging bucket and payer (§3 #3).
