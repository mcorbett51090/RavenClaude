...
findings=()
if grep -nEi "(AKIA[0-9A-Z]{16}|secret_key\\s*=|password\\s*=\\s*[\\\"'][^\\\"']+[\\\"']|api[_-]?key\\s*=\\s*[\\\"'][^\\\"']+[\\\"'])" "$scan_target" >/dev/null 2>&1; then
  findings+=("Possible hardcoded secret/credential — a committed secret is compromised; vault it and rotate.")
fi
if grep -nEi "(verify\\s*=\\s*False|InsecureSkipVerify\\s*:\\s*true|rejectUnauthorized\\s*:\\s*false)" "$scan_target" >/dev/null 2>&1; then
  findings+=("TLS verification disabled — this defeats transport security; do not ship.")
fi
if grep -nEi "(\\\"Resource\\\"\\s*:\\s*\\\"\\*\\\"|\\\"Action\\\"\\s*:\\s*\\\"\\*\\\"|iam:\\*|AdministratorAccess)" "$scan_target" >/dev/null 2>&1; then
  findings+=("Wildcard IAM permission — least-privilege violation; grant the minimum needed.")
fi
if grep -nEi "(0\\.0\\.0\\.0/0).*(22|3389|admin)" "$scan_target" >/dev/null 2>&1; then
  findings+=("Network open to 0.0.0.0/0 on an admin/SSH/RDP port — unintended exposure.")
fi
if grep -nEi "(md5|sha1)\\s*\\(" "$scan_target" >/dev/null 2>&1; then
