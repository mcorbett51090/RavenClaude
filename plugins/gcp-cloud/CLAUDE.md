...
6. **Pick compute by operational burden.** Stateless containers/HTTP → Cloud Run; need k8s → GKE (Autopilot to cut ops); event functions → Cloud Run functions (renamed from Cloud Functions in August 2024); legacy → GCE. Cloud Run is the right default for most services.
