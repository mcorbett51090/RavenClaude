...
10. **Pin a `model:` tier alias on every agent, and pick the cheapest tier the job survives.** Each `agents/*.md` MUST carry `model:` with one of `haiku` / `sonnet` / `opus` / `fable` / `inherit` — never a full model id, which goes stale when the SKU rotates. An omitted `model:` resolves to the main conversation's model, so on an Opus session a grep-shaped worker becomes an Opus worker. Default the roster by job shape, not by prestige: read-a-lot/return-a-little workers → `haiku`; bounded edits, tests against a stated contract, first-draft prose → `sonnet`; design, adjudication, and any gate that holds merge → `opus` (never de-escalate a gate). Doctrine: [`plugins/ravenclaude-core/knowledge/model-tier-delegation.md`](plugins/ravenclaude-core/knowledge/model-tier-delegation.md). **This is gated twice:** `scripts/check-frontmatter.py` fails the build on a missing/invalid `model:`, and `scripts/check-model-tier-ratchet.py` (Gate 287) fails a PR that raises the roster-wide `opus`/`fable`/`inherit` share or lowers the `haiku` count against `tests/fixtures/model-tier-ratchet.json` — re-stamp with `--stamp --allow-loosen` and say why in the PR when the loosening is deliberate. Cross-host: the alias is honoured natively only on Claude Code; the Copilot and Codex projections state it in the generated header for the consumer to pin, because those hosts take a picker/model id rather than a tier alias `[docs-verified 2026-09-14 — Copilot custom-agents-configuration reference; per-host truth in plugins/ravenclaude-core/knowledge/host-support.json components.agents.<host>.model_tier]`.
