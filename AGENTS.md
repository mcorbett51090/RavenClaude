...
4. Update consumers via `/plugin marketplace update ravenclaude`. `/reload-plugins` is **often no longer required** — since **v2.1.221 (2026-08-04)** plugins installed from `/plugin` activate immediately when safe, and the same release made `/plugin install` refresh a stale marketplace catalog and retry before reporting a plugin not found; run `/reload-plugins` only if a change doesn't take effect. `[verified 2026-08-19 against the Claude Code changelog]`
