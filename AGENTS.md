...
| **Account** | `~/.ravenclaude/usage/` | **this machine, every repo** — never committed | state that belongs to the account, not to any repo (the routine token reserve's weekly reading, projection and override) |
| **Data branch** | an orphan, never-merged branch in a **private** repo the user picks (`ravenclaude/usage-meter`) | that repo's collaborators | telemetry a cloud Routine must persist between ephemeral containers (the usage meter's allow-listed samples) — never a working-tree file |
...
10. **Pin a `model:` tier alias on every agent, and pick the cheapest tier the job survives.** Each `agents/*.md` MUST carry `model:` with one of `haiku` / `sonnet` / `opus` / `fable` / `inherit` — never a full model id, which goes stale when the SKU rotates. An omitted `model:` resolves to the main conversation's model, so on an Opus session a grep-shaped worker becomes an Opus worker. Default the roster by job shape, not by prestige: read-a-lot/return-a-little workers → `haiku`; bounded edits, tests against a stated contract, first-draft prose → `sonnet`; design, adjudication, and any gate that holds merge → `opus` (never de-escalate a gate). Doctrine: [`plugins/ravenclaude-core/knowledge/model-tier-delegation.md`](plugins/ravenclaude-core/knowledge/model-tier-delegation.md). **This is gated three ways:** `scripts/check-frontmatter.py` fails the build on a missing/invalid `model:`; `scripts/check-model-tier-ratchet.py` (Gate 287) fails a PR that raises the roster-wide `opus`/`fable`/`inherit` share or lowers the `haiku` count against `tests/fixtures/model-tier-ratchet.json` — re-stamp with `--stamp --allow-loosen` and say why in the PR when the loosening is deliberate; and `scripts/check-model-tier-fit.py` (Gate 288) fails a PR whose agent's tier does not fit the role its own frontmatter declares — an implementer (name ends `-implementation-engineer` / `-implementer` / `-coder` / `-developer`, or the description *opens* with `Use to BUILD` / `Use for BUILDING` / `IMPLEMENT` / `Use for X implementation`) may not sit on a frontier alias, and the three core merge gates may not sit below one. A genuine mis-read is exempted by name **with a reason** in `tests/fixtures/model-tier-fit-exemptions.json`, never by loosening the classifier; a stale exemption fails. The shape the classifier deliberately does not rule on — a frontier `*-engineer` beside its plugin's architect/lead, written in lower-case prose — is *listed* by `check-model-tier-fit.py --report` (the pair-review queue) for a human to tier, and the tie-breaker is **sibling-plugin parity**: the same role shape gets the same tier in every plugin (the `aws-cloud` / `gcp-cloud` engineers on `sonnet` decide the `azure-cloud` engineers), and a difference needs a reason that names the role, in the plugin's CHANGELOG. Cross-host: the alias is honoured natively only on Claude Code; the Copilot and Codex projections state it in the generated header for the consumer to pin, because those hosts take a picker/model id rather than a tier alias `[docs-verified 2026-09-14 — Copilot custom-agents-configuration reference; per-host truth in plugins/ravenclaude-core/knowledge/host-support.json components.agents.<host>.model_tier]`.
11. **No agent's `tools:` may grant dispatch — `Agent`, `Agent(...)`, the pre-rename alias `Task`, or the wildcard `"*"` — unless it is exempted by name, with a reason.** "Sub-agents do not spawn other sub-agents" is a house rule, not a platform limit: Claude Code nests three layers deep by default (`CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH`), and the only thing that stops a given subagent is its `tools:` allow-list omitting `Agent` `[docs-verified 2026-09-14, sub-agents § "Let subagents spawn their own subagents"]`. Two facts decide the shape of the rule: in a **subagent definition** the `Agent(type)` type list is **ignored** — `Agent(scout)` grants unscoped dispatch of anything at any tier — and `"*"` inherits `Agent`. Every nested hop pays the handoff tax (brief written, report re-read) at the *caller's* tier where the Team Lead never sees it, and a mid/fast caller makes the tier decisions the doctrine reserves for the frontier orchestrator. **This is gated:** `scripts/check-nested-dispatch.py` (Gate 289) fails the build on any `agents/*.md` whose `tools:` grants dispatch without an entry in `tests/fixtures/nested-dispatch-exemptions.json`; a stale or reasonless exemption fails too. The one sanctionable shape — a frontier-tier parent fanning out to fast-tier, read-only leaves that cannot themselves dispatch — is exempted per agent, and the reason must name the leaf type(s) and tier. The full determination (possible / enabled / desirable) is [`docs/decisions/2026-09-14-nested-dispatch-determination.md`](docs/decisions/2026-09-14-nested-dispatch-determination.md); the doctrine section is `model-tier-delegation.md` § "Multi-hop delegation". What the gate cannot reach — the built-in `general-purpose` / `claude` types (they hold every subagent tool), a fork, a consumer's project-local agent — is recorded by `handoff-tax-meter` as `nested_dispatch`; the consumer's off-switch is `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH=1`.
...
This marketplace ships **~184 plugins / 600+ agents**, so two levers keep the budget affordable — they're complementary, not either/or:
