...
Today this marketplace ships **106 plugins**:
...
- **[`geospatial-gis-engineering`](plugins/geospatial-gis-engineering/)** — Geospatial / GIS engineering: 3 agents (gis-architect / spatial-data-engineer / geospatial-app-engineer) — storage choice (PostGIS vs warehouse vs tiles), CRS/SRID done right, index-using spatial SQL (ST_DWithin, KNN), and map tiling (MVT/MapLibre). PostGIS-leaning, portable to BigQuery GIS / GeoParquet. 5 skills, 10 best-practices, advisory hook.
- **[`wordpress-cms-engineering`](plugins/wordpress-cms-engineering/)** — WordPress build/extend/operate: 3 agents (architect / developer / ops) — build-approach decision (classic vs block/FSE, plugin vs theme, headless), Gutenberg blocks & hooks, secure data handling, caching/security/safe-updates. 5 skills, 8 best-practices, advisory hook.
- **[`event-management`](plugins/event-management/)** — Event planning & production: 3 agents (strategist / operations-lead / marketing-revenue) — format & budget & sponsorship strategy, run-of-show & logistics & contingency, promotion/registration/ROI. Distinct from project-management. 5 skills, 8 best-practices, advisory hook.
- **[`physical-therapy-rehab-clinic`](plugins/physical-therapy-rehab-clinic/)** — Outpatient PT/rehab clinic ops: 3 agents (operations / documentation-compliance / billing) — scheduling & capacity, defensible plan-of-care docs & certification, therapy billing (8-minute rule, units, modifiers, denials). Advisory, not medical/billing advice. 5 skills, 8 best-practices, advisory hook.
- **[`optometry-eyecare-practice`](plugins/optometry-eyecare-practice/)** — Optometry / eye-care practice ops: 3 agents (operations / optical-dispensary / front-office-billing) — exam flow & recall, optical capture rate, the medical-vs-vision-plan billing split. Advisory, not medical/billing advice. 5 skills, 8 best-practices, advisory hook.
