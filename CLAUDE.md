...
**Name:** RavenClaude
**Purpose:** A production-grade Claude Code template that bootstraps the *Team Lead + Specialized Agents* manager pattern (Architect, Coder, Tester, Reviewer, Security) with git worktrees, hooks, and strict quality gates.
...
The Team Lead (top-level Claude session) **delegates** work to specialized sub-agents. Sub-agents operate in isolation and return structured reports. The Team Lead is the only role allowed to make user-facing commitments.

### Team roster
| Agent | When to spawn | Definition |
|-------|---------------|------------|
| Architect | Multi-file design, API shape, schema changes, "how should we structure X" | [`.claude/agents/architect.md`](.claude/agents/architect.md) |
...
| [`spawn-team`](.claude/skills/spawn-team.md) | Team Lead helper: brief and dispatch a multi-agent team |
