...
| Step | Actor | Output |
|---|---|---|
| 1 | Team Lead (`/spawn-team`) | Picks the **document** playbook — `deep-researcher` → `documentarian` (`spawn-team/SKILL.md:140-142`); adds a `code-reviewer` pass on the draft, since any non-trivial diff gets one per the skill's dispatch table |
