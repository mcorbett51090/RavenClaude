...
Adaptive thinking on Sonnet 4.6 (replaces `budget_tokens`); extended thinking requires `temperature` unset/1; **1M context** + context editing/compaction; the **memory tool** (**GA on the Messages API, no beta header** — corrected 2026-08-06; the earlier reading here conflated it with **Managed Agents memory stores**, a separate beta surface whose memory-store endpoints use `agent-memory-2026-07-22` and whose session endpoints use `managed-agents-2026-04-01`. See [memory surfaces (2026)](../plugins/memory-engineering/knowledge/memory-surfaces-2026.md)).
