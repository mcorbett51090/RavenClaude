...
> **Status (updated 2026-05-29):** Phase 0 (Stop-mode bridge) + Gap 1 (DoD gate) + Gap 2 (runaway brake) shipped in **v0.56.0**; Gap 6 (task-scope gate) + Gap 7 (disclaimer) shipped alongside. **Gap 5 (containment posture) shipped in v0.57.0** — portable tool-layer denies + the container/worktree boundary + the honest "OS sandbox is Claude-only" caveat (no OS-sandbox JSON written; the verifiable portable layer + docs only). **Gap 4 deterministic core shipped** — `scripts/thing-golden-eval.py` + `scripts/thing-golden-set.jsonl` + **audit-gates Gate 33**: a CI-cheap regression lane that runs a {dangerous, benign, injection, secret, scope} corpus through the REAL engine (`thing-decision.py preview` / `classify-payload`) and asserts each deterministic disposition (pre-LLM deny / hard-rule / clean-allow / routes-to-panel). The runner also carries the two **owner-verify** lanes the plan calls for — `--probe` (does `claude -p` resolve + return a parseable verdict from inside a Copilot session?) and `--live --model M` (per-vendor seat quality) — which need credentials / a live Copilot session and so are NOT in CI. **Remaining:** Gap 3 (cross-vendor seats), gated on the per-vendor `--live` lane clearing for each backbone.
