...
> **✅ Substrate verification status (2026-05-25 — VERIFIED; T2 unblocked).** Every load-bearing hook-API claim in this section was re-verified against the current Claude Code docs (hooks reference, hooks guide, and the Agent SDK hooks page) and the repo's own [`knowledge/claude-code-permissions.md`](../plugins/ravenclaude-core/knowledge/claude-code-permissions.md) was reconciled to match. **The 2023-era "both cannot be true" alarm was a false alarm** — the exit-code protocol and the JSON `hookSpecificOutput` protocol coexist (you pick one *per hook*: exit 2 blocks and ignores any JSON; JSON is read only on exit 0). Findings: **`updatedInput` is real** (the EDIT lever stands — it rewrites `tool_input` and must pair with `permissionDecision: "allow"` or `"ask"`; `"defer"` ignores it); `permissionDecision` accepts `allow`/`deny`/`ask`/`defer` (with `defer` **headless-`-p`-only**); `prompt` and `agent` hook types exist (`agent` experimental); and a hook `deny` **does** block even under `--dangerously-skip-permissions`/`bypassPermissions` — now *explicitly documented*. One correction folded in below: the platform **fails open on timeout**, so the §B.3.5 "never fail open" guarantee is an *orchestrator responsibility*, not a platform default.

Per code.claude.com/docs/en/hooks + /hooks-guide + /agent-sdk/hooks (re-verified 2026-05-25):

- `PreToolUse` hooks fire **before** any permission-mode check. A hook returning `permissionDecision: "deny"` blocks the tool **even in `bypassPermissions` mode or with `--dangerously-skip-permissions`** — "policy that users cannot bypass by changing their permission mode." This means the tribunal is a true gate, not a bypassable one. **Verified verbatim.**
- The reverse is **not** symmetric: a hook returning `"allow"` does **not** override a `deny` rule from settings — "hooks can tighten restrictions but not loosen them." **Safety consequence for the tribunal: an ALLOW verdict structurally cannot relax the `security_deny` floor**, even if a panel voted to. This is a guarantee, not a hope.
- Hooks receive `tool_input` on stdin as JSON (`{tool_name, tool_input, session_id, cwd, permission_mode, ...}`).
- Hooks can return a JSON `hookSpecificOutput` (on **exit 0** — exit 2 blocks and ignores JSON) with:
  - `hookEventName: "PreToolUse"` (required to identify the output)
  - `permissionDecision: "allow" | "deny" | "ask" | "defer"` — priority when multiple hooks/rules apply is **deny > defer > ask > allow**; `"defer"` is available **only in non-interactive `-p` mode** (it preserves the tool call for an Agent-SDK wrapper to resume).
  - `permissionDecisionReason: "<string>"`
  - **`updatedInput: { ... }`** — replaces the original `tool_input` before execution. **VERIFIED real** (Agent SDK "Modify tool input" example rewrites `tool_input.file_path`). Must be returned **inside `hookSpecificOutput`** alongside `permissionDecision: "allow"` (auto-approve the rewrite) or `"ask"` (show it to the user); with `"defer"` it is ignored; always return a *new* object rather than mutating `tool_input`. **This is the EDIT lever, and it is supported — the EDIT verdict path does not collapse.**
- Hook types: `command` (shell), `http` (POST to a URL), `mcp_tool` (call an MCP tool), `prompt` (single-turn LLM judge), and `agent` (multi-turn subagent, **experimental — may change**). All five are valid for `PreToolUse`.
- Default timeouts: `command` / `http` / `mcp_tool`: **600 s (10 minutes)**. `prompt`: **30 s**. `agent`: **60 s**. Configurable per hook. **Critical**: on timeout (or connection failure), Claude Code treats the hook as a *non-blocking error* and **the tool proceeds — fail-open by default.** The Lawspeaker therefore must enforce its own internal deadline (well under the hook `timeout`) and emit an explicit `ask`/`deny` *before* the platform's fail-open fires; see §B.3.5. The platform will not fail closed for us.
...
| **T2** — Single-seat tribunal (reviewer B only) | after T1 | Orchestrator script that invokes ONE panelist (`code-reviewer`-shaped) via `claude -p`, renders ALLOW/DENY (no EDIT yet), writes audit trail. Toggle live for `shell_readonly` in the dashboard (lowest-stakes category). **✅ SHIPPED v0.24.0 (2026-05-25).** Build note: the doc's `claude -p --bare` invocation **does not work under subscription/OAuth auth** — `--bare` refuses OAuth and demands `ANTHROPIC_API_KEY`. The seat defaults to plain `claude -p` (run from a scratch dir so the consumer's CLAUDE.md isn't auto-loaded); `--bare` is opt-in via `THING_SEAT_BARE=1` for API-key users. See [[knowledge/claude-code-permissions.md]] / the `thing` skill. | 12-16 h | ✅ done |
...
| **T4** — AlignmentCheck + injection hardening | after T3 | Reviewer C (AlignmentCheck) seat; adversarial-content envelope; pre-LLM injection regex; full §B.9 implementation. **✅ SHIPPED v0.26.0 (2026-05-26).** Much was already standing from T2/T3 (Heimdall/AlignmentCheck seat convened; basic `<untrusted>` envelope; `xc.injection-attempt` + `xc.secret-in-command` already `pre_llm_deny`; `injection_detected` unilateral DENY). T4 closed the genuine §B.9 gaps: (1) **§B.9.5 self-disable invariant** — new cross-cutting `xc.tribunal-self-disable` (critical, `pre_llm_deny`, `always_screen`) denies any Bash mutation of the Lawspeaker / seat / concern-decision scripts / `hooks.json` / `.ravenclaude/thing.yaml`, or a `thing: off` write into `comfort-posture.yaml`. The new `always_screen` mechanism evaluates it **category-independently** — even a self-disabling command shaped to classify into a toggled-OFF category is denied (closes the obvious evasion). (2) **Hardened `xc.injection-attempt`** with JudgeDeceiver / role-tag / special-token / verdict-flip shapes. (3) **§B.9.4 egress secret backstop** in the seat — a secret reaching a seat is denied locally, never transmitted to the model API. (4) **Envelope framing** tightened to the canonical AlignmentCheck wording. **Build finding:** the seat secret-scan `grep` first parsed `-`-leading patterns as options and silently fell through to a *live* model call — Gate 15 caught it; fixed with `grep -e`. | 10-14 h | ✅ done |
...
**Tests that have to pass per phase:** T0 — gate-audit proves the migrated `guard-destructive.sh` blocks (exit 2). T2 — gate-audit proves a known-bad Bash payload is denied by a single-panelist tribunal. T3 — gate-audit proves a known-bad-but-fixable Bash payload is EDITed correctly. T4 — **Gate 15 (shipped)** proves curated JudgeDeceiver-class injection payloads are denied, self-disable commands are denied even in a toggled-OFF category, a legit substrate read is *not* over-blocked, and the seat egress secret backstop denies locally. T5 — gate-audit proves a bypass-listed pattern skips the orchestrator. T6 — gate-audit proves override slash commands work end-to-end.
...
- **v3 (2026-05-25, Claude Opus 4.7):** Substrate-verification pass — resolved the §B.5.1 BLOCKING callout. Re-verified every load-bearing hook-API claim against the current Claude Code docs (hooks reference + hooks guide + Agent SDK hooks page) and reconciled the repo's `knowledge/claude-code-permissions.md` to match (a new "Advanced JSON output protocol" section). **The 2023-era "both cannot be true" alarm was a false alarm** — exit-code and JSON `hookSpecificOutput` protocols coexist (one per hook). Confirmed: `updatedInput` is real (EDIT path stands); `permissionDecision` allow/deny/ask/defer (defer is `-p`-only); `prompt`/`agent` hook types exist (agent experimental); a hook `deny` blocks even under `--dangerously-skip-permissions` (now explicitly documented). Surfaced a new safety property: a hook `allow` cannot override a settings `deny`, so the tribunal structurally cannot relax `security_deny`. Corrected one overclaim: the platform fails *open* on timeout, so §B.3.5's "never fail open" is an orchestrator responsibility, not a platform default. Net effect: **T2 is unblocked.**

- **v4 (2026-05-25, Claude Opus 4.7):** **T2 shipped (v0.24.0).** Built the single-seat orchestrator end-to-end: `hooks/thing-orchestrator.sh` (Lawspeaker — PreToolUse(Bash), fast-grep short-circuit when no toggle is set, pre-LLM critical screen, single-seat convene, Sága-log write, fail-closed verdict), `scripts/thing-decision.py` (classifies a command via the EMISSIONS table — one source of truth — and reads the per-category `thing:` toggle from `comfort-posture.yaml` + optional `thing.yaml` seat config), `scripts/thing-seat.sh` (the lone Mímir/code-reviewer seat via `claude -p`, with a `THING_SEAT_MOCK_VERDICT` test hook so CI never calls a live model), `skills/thing/SKILL.md`, `templates/thing.yaml`. Dashboard: the `shell_readonly` Command-review toggle is now live (writes `thing: on`); others stay Preview. Gate-audit **Gate 14** proves the orchestrator denies a known-bad payload and allows a known-good one (CI-safe). Storage decision: the on/off toggle rides as a per-category `thing:` key inside `comfort-posture.yaml` (§B.7.4 shape; the translator ignores the extra key) so it uses the existing dashboard save path; `thing.yaml` holds seat config only. **Correction to the design:** `claude -p --bare` (specified in §B.1/§B.5.3/§B.5.6) is unusable under subscription/OAuth — it requires `ANTHROPIC_API_KEY`. The seat defaults to plain `claude -p` from a scratch directory (avoids loading the consumer's CLAUDE.md); `--bare` is opt-in for API-key users. Latency observed ~10–15 s/seat (within budget; open question #1 stands). Open question #4 (credit-pool cost) is now concrete: ~$0.05–0.10 per reviewed command on a subscription — material enough that `shell_readonly` review is a validation switch, not a daily setting.

- **v5 (2026-05-26, Claude Opus 4.7):** **T4 shipped (v0.26.0) — §B.9 hardening.** Found T4 was mostly already standing from T2/T3 (Heimdall/AlignmentCheck seat, a basic `<untrusted>` envelope, `xc.injection-attempt`/`xc.secret-in-command` already `pre_llm_deny`, the `injection_detected` unilateral-DENY override, Gate 14's pre-LLM injection case). Built the genuine gaps. **(1) §B.9.5 — the Thing cannot disable itself (the load-bearing invariant), previously NOT implemented.** Added cross-cutting `xc.tribunal-self-disable` (critical, `pre_llm_deny`, new `always_screen` flag) matching Bash mutations (redirect/tee/rm/mv/cp/sed/chmod/…) of the Lawspeaker, seat wrapper, concern/decision scripts, the plugin `hooks/` + `scripts/` **directories** (matching the directory, not just the filenames, defeats glob / `$var` / whole-dir `rm -rf` evasions), or `.ravenclaude/thing.yaml`, plus `thing: off` writes into `comfort-posture.yaml`. The `always_screen` mechanism (in `thing-concerns.py::screen_always` → `thing-decision.py::_screen_always` → orchestrator pre-`enabled` check) evaluates it **category-independently** so a self-disabling command shaped to land in a toggled-OFF category is still denied — closing the obvious evasion the category-gated screen left open. **(2)** Hardened `xc.injection-attempt` (6→18 patterns: instruction-override, role-tag/`<|im_*|>`/`[INST]` spoofing, jailbreak, verdict-flip), FP-tuned against real commands (`gh pr review --approve` etc.). **(3) §B.9.4** egress secret backstop in `thing-seat.sh` — a secret reaching a seat is denied locally, never sent to the model API. **(4)** Envelope framing tightened to the canonical LlamaFirewall AlignmentCheck wording; the whole command is wrapped as untrusted (a conservative superset of the §B.9.2 heredoc-body split). **Gate 15** added (curated adversarial fixtures + category-independent self-disable + negative control + seat backstop) and **Gate 16** (every catalog trigger regex must compile); 47/47 gates green. **Build finding:** the seat's secret-scan `grep` first parsed `-`-leading patterns (`--password`, `-----BEGIN…`) as *options*, silently failed, and fell through to a **live** `claude` call — Gate 15 caught the leak; fixed with `grep -e`. **Security review (pre-merge, `security-reviewer` agent)** found two confirmed shell-level bypasses of the first-cut literal-filename self-disable guard — `tee -a` append (the `\S*` ate the flag; `tee` was absent from the verb list) and path obfuscation (`rm -rf <hooks dir>`, globs, `$var`) — both fixed by matching the substrate **directories** + adding `tee`, and pinned as Gate 15 fixtures. Also hardened in response: bounded the trigger-3 lookaheads (ReDoS), `screen_always` now fails **closed** on an uncompilable `always_screen` trigger (vs `_matches`' soft skip), and the egress backstop + catalog gained GitHub-fine-grained/GitLab/Slack/Google/JWT/`--token` families. Deferred to a later phase: §B.9.2 separate heredoc-body extraction (whole-command-untrusted is the safe superset), and reviewing `Edit`/`Write`/`MultiEdit` tools (Bash-only orchestrator today; that is the deferred T3.5 / open-question #5 track).

*End of v5. Tribunal design document — the Thing.*
