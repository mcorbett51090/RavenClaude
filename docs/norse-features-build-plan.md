...
> **RECONCILIATION (2026-05-29, core v0.54.0): NOT built as a themed program, and the v0.18–0.28 window is long past — re-scope to an OPTIONAL backlog, don't run as a sequenced roadmap.** Several features now have de-facto equivalents shipped under other names: **Fenrir** (un-overridable deny / "cannot disable itself") = DONE via the tribunal `security_deny` floor + `xc.tribunal-self-disable` (`always_screen`, `pre_llm_deny`) + the MCP allowlist; **Víðarr** (security/event-log panel) = mostly satisfied by the **Saga tab** (`/__saga` reads `.ravenclaude/runs/thing/`); **Sleipnir/Mjölnir** = partially covered by existing worktree skills + `plugin-release-checklist`/`audit-gates.sh`; **Yggdrasil/Trees** = the (still-stub) Trees tab. **Genuinely unbuilt:** Heimdall (perimeter-alarm), Norns, Bifröst, Ragnarök, Níðhöggr. **Real blocker for any event-driven panel:** the P0.2/P0.4 structured event-emission substrate (`hook-events.jsonl`/`posture-events.jsonl`) was never built — grep finds no `_emit_hook_event`/`_emit_posture_event`. Cherry-pick features only on demonstrated need.
