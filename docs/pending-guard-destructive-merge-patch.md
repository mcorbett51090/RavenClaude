...
# Case 13 (round-4 review finding — a legitimate prefix token before `git` itself must not be
# mistaken for the branch; the tail-after-keyword isolation closes this category, not just these
# three examples)
echo '2>/dev/null git checkout main; git merge feature-branch' # expect BLOCKED (leading redirect)
echo 'FOO=bar git checkout main; git merge feature-branch'     # expect BLOCKED (env-var assignment)
echo 'sudo git checkout main; git merge feature-branch'        # expect BLOCKED (command wrapper)
# ...and the prefix must not confuse create-target tracking either
