...
- **`NOTICE.md`** — every third-party server gets an attribution entry: source repo, package, license, what it does, the pinned prerequisite, and a PATH fallback. Keep the fallback command **identical** to the one in CLAUDE.md (a mismatch is a real, shipped bug — see Provenance).
...
- **A machine-readable attribution field** on the `mcpServers` entry (e.g. `x-attribution`) would let CI verify every third-party server is attributed. Attractive, but **unverified** whether Claude Code's loader tolerates unknown keys in an `mcpServers` entry without breaking server launch — verify before adding one to a live manifest; until then attribution lives in `NOTICE.md` and is checked by presence/convention.
