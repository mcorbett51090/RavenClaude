...
- **`git pull` blocked by *uncommitted* changes on the clone** — the more common stall, and a different failure from the one above. The clone doubles as the live runtime surface, so `.ravenclaude/comfort-posture.yaml` (posture tuning) and `.claude/settings.json` (rewritten by `apply-comfort-posture.py` at session start) are **tracked files that normal use modifies** — and upstream edits them too. `git pull --ff-only` then refuses before anything updates. Commit them to a side branch (robust) rather than `stash`/`pop`, which conflicts precisely when upstream touched the same file. Untracked files (e.g. a draft in `docs/staging/incoming/`) never block a pull. See the [`update-ravenclaude`](../../plugins/ravenclaude-core/skills/update-ravenclaude/SKILL.md) skill § "When the pull stalls on local changes".
...
- [`plugin-lifecycle-off-ask-auto.md`](./plugin-lifecycle-off-ask-auto.md) — unused-plugin sweep / ask-first install (not this update path). Do not use `/reset-plugin-cache` as cleanup.
