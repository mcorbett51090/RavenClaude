...
**Splitting a mixed gate (derivable vs. structural).** A gate can enforce *both* a self-healing property and a non-self-healing one. `check-marketplace-claims.py` is the worked example: its **derivable counts** ("`<N>` skills/agents" prose, README "ships `**N plugins**`") are a repo-wide moving target that self-heals post-merge (`--fix`), but its **structural** claims (required files present, descriptions under the char cap, every plugin in the architecture.md table) are *not* derivable and must still block the PR. The pattern: give the checker a `--structural-only` mode (PR CI runs that), a `--fix` mode (the post-merge workflow runs that), and keep the default all-checks mode for the audit fixtures. In `audit-gates.sh` the count `must_fail` fixtures stay in default mode (detector teeth), the clean-tree `must_pass` switches to `--structural-only` (so a PR with not-yet-healed counts doesn't fail the meta-test), and a `--fix`-repairs fixture is added as the relocated count `must_pass`.

**Generate-then-test: keeping a gate that *consumes* a self-healed artifact honest.** When you move an artifact off PR-gating, any *other* gate that reads the **committed** copy of it can silently go stale — it would test last-merge's output, not the current PR's. `dashboard.html` is the worked example: it self-heals post-merge (it inlines the concept/decision-tree SVGs, which need mermaid-cli + Chromium to render — an authoring burden no PR should carry), but seven render-test gates (Heimdall / Víðarr / Norns / Níðhöggr / Bifröst / Mímir / roundtrip) extract functions from the committed file. The fix is **generate-then-test**: before those gates run, `audit-gates.sh` regenerates `dashboard.html` *in place* (pure-Python, inlining the existing committed SVGs — no browser), so the render tests always exercise the CURRENT generator's output. The render tests already accept the dashboard path as `argv[2]`, so no test needed editing. The dashboard freshness `must_fail` detector stays (append → `--check` flags stale); its clean-tree `must_pass` is replaced by "the generator runs clean (and leaves a fresh file for the render gates)", and the exact-byte "committed == regenerated" assertion relocates to the post-merge workflow's `--check`. The SVG-sync gates follow the plain self-healing pattern (keep `must_fail` mutated-manifest detector; drop the clean-tree `must_pass`, since a PR that edits a diagram without rendering its SVG must not fail).

### Splitting a required workflow into parallel suite jobs behind one aggregator

A required workflow's `pull_request` trigger can never carry a `paths:`/`paths-ignore:` filter (see the header comment on [`.github/workflows/validate-marketplace.yml`](../../.github/workflows/validate-marketplace.yml) and Gate 242) — a filtered-out run leaves the required check Pending forever. Below that trigger-level floor, two sanctioned patterns exist to trim a required workflow's cost, and they compose:

- **Step-level `if:`** (PR-A / #1108) — a single job runs every step; expensive standalone steps carry a per-step skip condition (fed by a `dorny/paths-filter` `detect` job) so a docs-only PR skips them while still reporting a conclusion for the job.
- **Job-level `if:` behind an always-on aggregator** (PR-B) — the same `detect`-fed skip condition is promoted from a step inside one job to the `if:` on several **parallel** jobs, each running one lint suite. A final aggregator job — the one that carries the required check's exact `name:` — `needs:` every suite job, sets `if: always()` so it runs regardless of any dependency's outcome, and rolls the results up explicitly: every dependency must report `success` or `skipped`, never `failure`/`cancelled`. Splitting into jobs (rather than steps) buys real wall-clock parallelism, at the cost of each job needing its own checkout (and any other setup the steps it carries need).

**The aggregator's rollup is the load-bearing piece — get it wrong and the split silently weakens the required check.** It must check every gating dependency's `result`, not just wait on them (GitHub's own "if a dependency fails, dependents are skipped unless they use `always()`" behavior only handles scheduling, not correctness — an aggregator that merely runs after a failed suite and exits 0 would report the required check green while a real suite failed). A best-effort upstream job (like `detect` itself) can be `needs:`-listed for ordering without being included in the pass/fail check, provided every job gated on its output is independently fail-safe toward running (never toward skipping) when that upstream job fails — otherwise its failure is silently laundered into "everything downstream passed."
