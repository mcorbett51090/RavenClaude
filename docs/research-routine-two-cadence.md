...
6. **Tribunal gate before the PR is final** (added 2026-09-11). Once the gate suite is green, run the diff through the [`routine-review-tribunal`](../plugins/ravenclaude-core/skills/routine-review-tribunal/SKILL.md) skill — the same two-panel, cross-model review `/forge-pipeline` gives a plan before code exists, applied here to the knowledge-file diff this routine just produced. `approved` with `merge_authority: "auto"` → open the PR and **merge it directly** per the skill's "Auto-merge procedure" (mark ready, wait for `main`'s required checks, squash-merge via the GitHub MCP `merge_pull_request` tool), then notify with what shipped; `approved` with `merge_authority: "human"` → open/ready the PR, note which seat flagged `design_decision` and why, notify, and stop — a person merges; `needs_revision` → apply the tribunal's required edits and resubmit (bounded to 2 rounds); `escalate` → open a flagged draft PR and notify rather than merging unreviewed corrections into a claim-grounded knowledge file. This is the routine's second opinion in place of the human reviewer it doesn't have, running unattended.

   **Authorization for the `"auto"` branch:** repo-level GitHub auto-merge is deliberately OFF and the platform's standing rule is "don't merge without explicit user instruction" (`CLAUDE.md` § "Admin bypass is deliberate"). This step, as written and dated, is sourced directly from Matt's 2026-09-11 directive ("I want as much auto-merged as possible; the only thing I should review is a design or architecture decision") and is intended to serve as that explicit instruction for this routine's `merge_authority: "auto"` PRs going forward — not yet verified against an actual unattended end-to-end run, so treat early auto-merges here as worth a spot-check.
