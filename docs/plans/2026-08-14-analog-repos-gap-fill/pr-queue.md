...
|---|---|---|---|---|---|---|---|---|---|---|
| F1 | WebFetch-only result sanitizer | L1 | C06 C15 | [#928](https://github.com/mcorbett51090/RavenClaude/pull/928) `feat/ravenclaude-core-webfetch-sanitize-hook` | `hooks/sanitize-webfetch-output.{sh,py}`, `hooks.json`, `RC_BASELINE.hooks` **33→34**, manifests, CHANGELOG, dashboards | poisoned payload → `updatedToolOutput` + strip>0; clean → identity; crash → fail-open exit 0; `RC_BASELINE.hooks===34` | ~12 files / hook+regen (Yellow/Red split accepted on regen) | [] | `[verify-at-implement]` **0.267.0** (0.266.0 taken by #931; branch **CONFLICTING**) | true |
...
Do not open another PR that edits `plugins/ravenclaude-core/.claude-plugin/plugin.json` while #928 is open. F2 and F3 do not touch it. Rebase #928 onto current `main` before merge — #931 already took 0.266.0 and hook 33.
