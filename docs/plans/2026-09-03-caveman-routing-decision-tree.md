...
| C5 | `transcript_path` is a common input field on every hook event. `session_id` is on every payload; `source` is on `SessionStart`. | `docs/plans/archive/2026-08-08-premise-gate/plan.md:8`; host-verified by P0.4 below. |
