...
**Gitignore policy:** the buildout plan (B.1) already specifies that `.ravenclaude/runs/` is gitignored by default (run artifacts are local). Inherit that. The exception is `findings.json` for accepted-but-not-yet-acted findings, which the user may want to commit for cross-machine continuity — that's covered in [§15 Open Questions](#15-open-questions) Q5.
...
| E3 | **Multi-machine drift** — Matt's laptop sees a different dawn report than Matt's desktop | v1: per-machine, no sync. v2: optional `.ravenclaude/runs/dawn/` cross-machine sync via the user's choice of mechanism (rsync, OneDrive, git stash, custom). The data is local-first; sync is the user's call. |
...
| E8 | **Conflicting Huginn dismissals** across machines (the user dismissed a finding on laptop, didn't on desktop) | `huginn-preferences.json` is gitignored and per-machine. Each machine learns independently. The user is the integrator. |
...
| R4 | **Multi-machine drift** confuses the maintainer about what's "real" — laptop says X stale, desktop says X fresh. | M | M | v1: per-machine, no sync. Document the model. v2 could ship optional sync. |
...
| Q5 | **Cross-machine continuity for accepted findings.** When Matt accepts a finding on laptop but does the actual refresh from desktop, how does desktop know the finding was accepted? | The user reasonably expects "I accepted this yesterday" state to follow them. | v1: it doesn't. The dismissal is per-machine. v2: support optional commit of `dismissals.json` to the marketplace repo (not the per-machine `huginn-preferences.json`). **Decide before Phase 4.** |
