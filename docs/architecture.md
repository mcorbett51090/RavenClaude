...
| [`web-design`](../plugins/web-design/) | 0.1.0 | Web design & build: 7 specialist agents (web architect, UX, visual, frontend implementer, content strategist, accessibility auditor, performance engineer), 4 skills (design-system audit, accessibility review, Core Web Vitals tuning, technical SEO audit), 8 templates, 1 advisory hook (oversized rasters, missing alt, hardcoded hex outside tokens, missing meta tags, accidental noindex). Inherits `ravenclaude-core` protocols. |
