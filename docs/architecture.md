...
|---|---|---|
| [`ravenclaude-core`](../plugins/ravenclaude-core/) | 0.3.0 | Domain-neutral: 14 specialist agents (incl. new data-engineer), dispatch playbook, gates, hooks, contribution-staging workflow (security sweep + expert routing), templates |
| [`power-platform`](../plugins/power-platform/) | 0.6.0 | Microsoft Power Platform: 11 specialist agents (incl. new power-platform-tester) + 13 skills (9 imported Daniel Kerridge MIT + 4 in-house) + advisory house-opinion hook (8 grep-able checks) + bundled pbix-mcp MCP server |
