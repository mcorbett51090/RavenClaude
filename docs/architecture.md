...
| [`power-platform`](../plugins/power-platform/) | 0.8.0 | Microsoft Power Platform: 11 specialist agents (includes `power-platform-tester`; all with active cross-plugin Structured Output Protocol blocks + licensing_impact field), 13 skills (9 imported Daniel Kerridge MIT + 4 in-house) + advisory house-opinion hook covering **8 checks** + knowledge bank capturing production lessons (currently: programmatic cloud-flow creation via Dataverse Web API when the PA Management API is blocked) + bundled pbix-mcp MCP server |
...
| [`regulatory-compliance`](../plugins/regulatory-compliance/) | 0.1.0 | Financial-regulatory: 6 specialist agents (AML/KYC, regulatory reporting, risk-and-controls, policy & procedure writer, examination prep, Bermuda-insurance), 4 skills (AML program review, regulatory mapping, SAR narrative drafting, examination readiness), 8 templates, 1 defensive PreToolUse PII-scrub hook. Field-experience positioning (BMA). Inherits `ravenclaude-core` protocols. |
| [`web-design`](../plugins/web-design/) | 0.2.0 | Web design & build: 7 specialist agents (web architect, UX, visual, frontend implementer, content strategist, accessibility auditor, performance engineer), 4 skills (design-system audit, accessibility review, Core Web Vitals tuning, technical SEO audit), 8 templates, 1 advisory hook (oversized rasters, missing alt, hardcoded hex outside tokens, missing meta tags, accidental noindex), and a knowledge bank seeded with a 2026 curated "cutting edge yet simple" reference set (Linear, Vercel, Raycast, Resend, Cursor, v0, Tldraw, Cal.com). Inherits `ravenclaude-core` protocols. |
...
**Planned plugins** (on the roadmap): EdTech (built around the partner-success-manager pattern), Salesforce. See [`./plugin-roadmap-analysis.md`](./plugin-roadmap-analysis.md) for the prioritization analysis behind the finance / regulatory-compliance / web-design selection.
