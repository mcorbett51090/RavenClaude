...
- **v17 (2026-05-24, Claude + Matt):** Two shipped changes, not plan edits — recorded here for the trail. **(1) Permission vocabulary collapsed from five levels to three** (`deny` / `ask` / `allow`). The five levels (`deny`, `always-ask`, `mostly-ask`, `mostly-allow`, `autopilot`) always collapsed to the same three buckets at emission time (`level_to_bucket`); the v0.2.0 safe/risky split that would have made `always-ask` ≠ `mostly-ask` never shipped, so the extra levels were cosmetic. The dashboard now offers Deny/Ask/Allow buttons and four presets (Recommended/Deny/Ask/Allow); the translator still accepts the old names so pre-0.19.0 consumer YAMLs translate unchanged. Supersedes the five-level references in §1.1 and the glossary (§5.3.2 "Level"). **(2) Per-permission, per-layer overrides shipped** — the dashboard's category cards now expose a per-permission control with independent User/Local/Project selectors, serialized into the schema's v5 `overrides:` map and resolved by the translator (`category_overrides_map` / `pattern_layer_value`). Plus a `defaultMode: default` pin + SessionStart `ensure-default-mode.sh` advisory hook so the emitted rules aren't silently bypassed by a sticky bypass mode. Released as ravenclaude-core v0.19.0.
