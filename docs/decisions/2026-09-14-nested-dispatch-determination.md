...
| Fact | Source |
| --- | --- |
| A subagent **can spawn subagents of its own, up to three layers below the main conversation**, by default. | sub-agents § "Let subagents spawn their own subagents" |
| The cap is `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH`; `1` turns nesting off; at the limit Claude Code withholds `Agent` from every subagent except a fork (whose `Agent` call then errors). | same |
