...
See PR / `plugins/ravenclaude-core/CLAUDE.md` v0.283.0 milestone. Four Claude Code platform-fact
corrections (stale subagent nesting depth; native concurrency cap; `/reload-plugins` now often optional;
new `archive`/`command` marketplace sources). These were fetchable and first-hand verified.
...
|---|---|---|---|---|
| 1 | microsoft-fabric | **Fabric Data Agents' OpenAI Assistants API retires 2026-08-26** — migrate query→MCP endpoint, SDK→Responses API | learn.microsoft.com/fabric/data-science/data-agent-mcp-server | **P0 (hard deadline, days out)** |
