...
| 11 | Cloud flows are `workflow` rows with `category=5`; `statecode=1`=Activated, `0`=Draft; reactivate via PATCH statecode/statuscode | BLOCK | [unverified — training knowledge; widely-documented Dataverse schema, BTCSI verified empirically]. Justification: schema enum not surfaced in this session's searches; route to verify = Dataverse `workflows` metadata or a live `pac`/Web API query | G5/G8 — note as `[unverified]`; script must tolerate schema variance |
| 12 | `ConnectionAuthorizationFailed` (403) occurs when activating a flow whose connection reference isn't yet bound/propagated; retry-after-propagation succeeds | BLOCK | [unverified — training knowledge + BTCSI empirical]. Consistent with claim 7 (connection-permission gating). Route to verify = reproduce in a real env | G5 — red-team treats as the primary retry trigger |
...
G1 caught a **correlated error in the source material**: BTCSI's recipe hardcodes `--publish-changes`
